# ARCHIVED (2026-07-13, S83) — this is the historical OD/crypto/physics master context.
# The LIVE operating doc is now `CLAUDE.md` (lean, Kalshi-only). Nothing below was deleted or edited —
# this file is the verbatim pre-split master (canonical through S82's header). Consult it for the
# session-by-session record: OD/info-dipole crypto research (S20-S37), the Information-Layer /
# four-forces / gravity-time physics program (S3-S25, INFO-0xx ledger), capability demos, defense
# pipeline. The still-live pieces (OD toolkit, trading rules) were carried forward into `CLAUDE.md`.

# CLAUDE.md — DavisAI Master Context (Updated 2026-07-13 Session 82 — Kalshi: the PER-TRADE LEVEL-HIT dataset built (200k events, leakage-clean); level-hits mean-revert at 1¢, NO cell pays even at maker fees → confirms S81 size-vs-fee at trade granularity + internal flow context is a WEAK predictor → the edge is EXTERNAL (futures lag). Thu 7/16 natgas STAGED. Pyth feed UNSTUCK (deleted 11 legacy crypto workflows hogging the runners → ticks accrue tonight). WEATHER scoreboard characterized (`WEATHER_BASELINE_S82.md`: per-regime bar, edge on transition days, per-bucket-never-pooled — forecaster stays Greg's spec). READ `SESSION_HANDOFF_2026-07-13_S82.md` first, then S81/S80, not this whole file)

## S82 UPDATE (2026-07-13) — the PER-TRADE LEVEL-HIT dataset (priority-1 for 3 sessions) FINALLY built; it confirms the edge is EXTERNAL not internal (read `SESSION_HANDOFF_2026-07-13_S82.md`; branch `claude/kalshi-s79-kickoff-ij8t9o` = repo DEFAULT, push there)
- **Branch trap again:** harness dropped me on a fresh branch at the stale S70 tip `3c70ff5` (the recurring trap); switched to s79 tip `816f621`. Work on s79.
- **THE LEVEL-HIT DATASET DONE (`research/kalshi/level_hit_dataset.py`; `LEVEL_HIT_FINDINGS_S82.md`):** re-pulled the full **496,306-trade** WTI tape (46 events × top-12), built **200,421 level-hit events** — one row per 1¢ transition, pre-hit context {moneyness, side, velocity, herd/whale SCORED, exhaustion, tod, release} + forward trailing-exit outcome {continued, big-run≥2, net taker/maker}. Per-cell (moneyness×side×velocity×release), distributions not means, **leakage gate PASS (0 fails)**, settle window excluded, zero synthetic.
- **THE FINDING (honest, Result-Discipline negative that sharpens the program):** (1) level-hits **MEAN-REVERT at 1¢** — continue_rate **0.383 < 0.5**, big-run rate 0.24, run median 0. (2) **NO cell pays on average even at MAKER fees** — best cell maker-mean −0.63¢, cross-cell −1.45¢; the toll dominates all 60 cells. **Confirms S81 (size-vs-fee, direction is the easy part) at per-TRADE granularity.** (3) Cell splits barely move net (rel≈norel, fast≈slow; deep-moneyness loses least = tiny fee but continues least). (4) **The pre-hit ORDER-FLOW context is a WEAK predictor of a big run** (herd/whale, exhaustion, dipole expect lift +0.00…+0.03; strongest is `aggr`, the hitting trade's side, +0.05 but near-tautological with dir).
- **META-CONCLUSION:** the Kalshi-INTERNAL continuation predictor is weak + fee-bound → **the tradeable edge is EXTERNAL (the futures→Kalshi LAG), not the book's own flow.** The level-hit that runs is most plausibly the one trailing a fresh NYMEX move — NOT in this tape. **NEXT: join the Pyth `futures_move_bps`/`lag_seconds` onto each level-hit** (once `data/pyth-ticks` accrues) → turns this scaffold into the real continuation predictor.
- **Thu 7/16 EIA natgas (14:30 UTC) STAGED + turnkey:** `release_book_signal.py --selftest` leakage PASS 30/0; `--test` runs on accrued `data/kalshi-bins` (rel=0 only because the release-spanning window hasn't accrued yet — fires when it does). `consensus_poll.py` confirms Thu Natural Gas Storage prev=61B (forecast fills in near release) + Wed 7/15 EIA crude → KXWTI.
- **WEATHER scoreboard characterized (`research/kalshi/WEATHER_BASELINE_S82.md`; forecaster stays Greg's spec, HANDS OFF — this is scoreboard only):** daily-high `KXHIGH*` markets = 6-bucket ~2°F partition ladder, re-centered daily around the expected high, same-day settle. Ran market self-calib + walk-forward persistence/climatology baselines via `kalshi_score.py`: pooled Brier NY 1.12 / CHI 1.31 / DEN 1.11 — but **the pooled mean is the WRONG lens.** Per-regime split (Greg): persistence calm vs transition = NY 0.73 vs 1.45, CHI 0.91 vs 1.58, DEN 0.92 vs 1.38 → **the tradeable edge lives in the TRANSITION (frontal) cell** where the naive baseline + the market leaning on it are weakest; LAX is marine-clamped (|Δ|~1°F, efficient — skip). **PER-BUCKET-NEVER-POOLED applies to weather too** (cells = regime × city × synoptic season × bucket-moneyness × swing-dir; **2 regimes is the MINIMUM — refine toward synoptic cells** ridge/cold-frontal/warm-front/marine/post-frontal/convective). Tradeable version needs a PRE-hoc transition classifier. Market baseline still post-hoc placeholder until `KXHIGH*` bins accrue over a settled event. Doc carries a worked trade example (real KXHIGHNY-26JUN29, realized 88°F, ≥86 bucket won) + Kalshi fee/payout economics.
- **PYTH FEED UNSTUCK (end of S82):** the durable run had sat `queued` because **11 legacy crypto collector workflows were hogging the Actions runners** (worst: `bybit_perp_history` every 15 min + 8 coin collectors on 6h crons). Greg dispatched the Pyth run (now `in_progress`); then we **DELETED all 11 crypto workflows** on the default branch (commit `a9d590e`; recoverable via git) → crons stop, runners freed. Only `kalshi_collectors_durable.yml` + `pyth_collector_durable.yml` remain. **Real ticks accrue once energy futures reopen (~Sun 22:00 UTC) — the 6h cron spans the reopen, so `data/pyth-ticks` fills tonight, before Thu natgas.**
- **NEXT (S83):** (1) sub-second lag on accrued `data/pyth-ticks` (priority-1, feed now unblocked) — which strikes lag by how many seconds, does the reprice clear the toll (`score_hold`); (2) join Pyth `futures_move_bps`/`lag_seconds` onto the level-hit dataset (the external-driver hypothesis); (3) Thu 7/16 natgas live re-run + join actual via EIA; (4) roll re-point Jul 21+.

## S81 UPDATE (2026-07-12) — the futures→Kalshi LAG made tradeable-or-not; sub-minute pivot to Pyth (read `SESSION_HANDOFF_2026-07-12_S81.md`; branch `claude/kalshi-s79-kickoff-ij8t9o` = repo DEFAULT, push there; `KALSHI_TRADING.md` = the new file index)
- **Lag exploitation built + worked end-to-end** (`research/kalshi/lag_exploit_backtest.py`; per-trade, per-cell, leakage-gated, no averaging). **Mode B cross-strike = NULL** (strikes synchronous with each other → the lag is futures-vs-Kalshi only; needs an external feed). **Mode A futures→Kalshi:** direction IS predictable and SHARPENS with move size (lagging contracts, next-min hit-rate 0.55@$0.05 → **0.77@$0.40+**); the obstacle is **SIZE-vs-FEE, not direction** (median 1-min reprice ~1¢ < ~2.4¢ fee). Net-of-fee positive only on **lagging × ≥$0.40 move = +91¢/26 trades**; rare at 1-min. GROSS reprice positive all 5 days. Corrected two over-negative reads (over-charged toll; pooled small-moves + synchronous contracts).
- **⭐ SUB-MINUTE PIVOT (Greg: "1 min is an eternity; NYMEX moves a lot in a minute"):** Yahoo 1-min high/low was BROKEN (under-sampled). Pyth real 1-min OHLC: intra-minute range **11¢** vs 6.4¢ close-to-close (1.6× hidden), **15–23¢ around the 14:30 EIA release**. Historical **sub-second NYMEX EXISTS** (Pyth Benchmarks `/v1/updates/price/{ts}`; rate-limited ~2 req/s). Direct event-study (10s NYMEX + ms Kalshi tape): on a +17¢ NYMEX jump the liquid strike's first reprice came ~7s later, still climbing at ~20s → the "full-minute lag" was **coarse-bar aliasing**, BUT **7–20s is a HUGE exploitable window** (Greg), not near-synchronous: with a sub-second NYMEX feed you act with seconds of margin = a **persistent latency edge on EVERY meaningful move**; less-liquid strikes lag even longer. Time-to-act is NOT the constraint — reprice SIZE vs toll is. One event so far; live feed gives the distribution.
- **PYTH FEED BUILT + LIVE:** `research/kalshi/pyth_collector.py` (stdlib SSE, front-month WTIQ6/NGDQ6/BRENTU6 = Kalshi's own settlement source; dedup on advancing publish-time) + `.github/workflows/pyth_collector_durable.yml` (6h cron → `data/pyth-ticks`, run #1 live). Ticks accrue when energy futures reopen (Sun eve ET), before Thu natgas. **Roll dates: WTIQ6 7/21, NGDQ6 7/29, BRENTU6 7/31 — re-point after expiry.**
- **`KALSHI_TRADING.md` NEW** — the file index Greg asked for: 2 sections (current Kalshi files / old-completed pieces). Keep it current.
- **NEXT (S82):** (1) sub-second lag on LIVE ticks — which strikes lag by how many seconds, does the whole reprice clear the toll (use `score_hold` at real resolution); (2) Thu 7/16 EIA natgas live book + busy-day lag; (3) the PER-TRADE LEVEL-HIT dataset (priority-1, still NOT started); (4) roll re-point; paper-loop RSA creds.
...
- **Thread (a) consensus accrual LIVE.** `research/kalshi/consensus_poll.py` (stdlib) polls the FREE ForexFactory weekly JSON (`nfs.faireconomy.media/ff_calendar_thisweek.json`; carries forecast/previous for Crude/NatGas/CPI/NFP/FOMC), maps title→series, upserts `data/kalshi/consensus.jsonl`. Feed has forecast+previous but NOT actual + current-week only → poll forward for forecast, join actual from EIA/BLS at release. Wired into the durable workflow (polls + persists each cycle) + retargeted to s79; push fired durable run #4.
- **Event-weight study (agent; committed `EVENT_WEIGHT_STUDY.md`/`event_weight_study.json`/`source_map.json`).** Per-bucket, never pooled. 4-regime natgas split DATA-DISCOVERED from degree-days (winter/summer withdrawal humps + spring/fall injection; double-hump confirmed; summer = build-MINIMUM not net draw). **Weather→storage STRONG (winter R²=0.85, 91% sign) → the OD-weather forecaster IS a storage-print forecaster = non-arbed edge on any storage-NUMBER-settled contract.** BUT storage-surprise→PRICE ≈ NULL (hit 0.52, big sell-the-news bucket): the number is weather-predictable so it's priced by release; the seasonal 5yr proxy is blindest in winter. Natgas intraday spike real (1.57× same-hour placebo); crude weak (1.12×).
- **⭐ THE MERGED SIGNAL ARCHITECTURE (Greg, S79, load-bearing):** **news release = CATALYST + coarse size hint** (a lot/a little — a range at most; DO NOT build a precise surprise→move regression, it's null anyway; also the PAUSE gate); **book imbalance + dipole exhaustion on the contract probability series = the DIRECTION + magnitude-class read** (imbalance SIGN→direction incl. sell-the-news; imbalance MAGNITUDE+exhaustion→little-or-a-lot + hold-or-fade). This is the crypto FILTER+TIMING stack FIRED BY a release; it resolves the surprise→price null (direction was never in the surprise). Build the release-triggered book signal, NOT a surprise regression.
- **Other standing rules set this session** (detail in handoff): score events per-bucket (surprise×reaction), never pooled |move|; release-day quiet→spike→decay playbook (measure intraday not daily; pre-quiet = PAUSE trigger; stand aside through the spike); two-phase model (underlying = event impact prior / Kalshi bins = transfer-function calibration); weather = ROOT signal for weather+electricity+natgas (double-humped demand; honest scope = beat climatology/persistence/MOS, not out-forecast ECMWF).
- **NEXT:** (1) **Greg: make s79 the DEFAULT branch** so the 6h cron fires (continuous bins+consensus accrual; the one unblock). (2) Build the release-triggered BOOK signal (imbalance+exhaustion, calendar-gated) on the accruing order-book series. (3) Accrue bins+consensus across real releases → coupling test with REAL street-consensus residual surprise + lead-time score baselines. (4) AGA pre-2002 natgas history extension (bonus). (5) OD-weather → `kalshi_score.py`. (6) paper loop on demo API (RSA creds).


## S78 UPDATE (2026-07-12) — MARKETS PIVOT: crypto (proving ground) → KALSHI prediction markets. Full news+collector+coupling+settlement-scoring pipeline built (read `SESSION_HANDOFF_2026-07-12_S78.md` + `KICKOFF_S78_KALSHI.md` + `KALSHI_BUILD_SCOPE.md`; branch `claude/kalshi-s78-kickoff-jb5oyx`, based on S77 tip `claude/book-swing-s77-kraken-meomsh`)
- **THE PIVOT (S77→S78):** crypto book-swing is a REAL but FEE-FLOOR microstructure edge, capped by capital + a maker-fee regime we don't control (S77 Hummingbot live check: naive MM ≈ flat). Crypto was the **proving ground**; **Kalshi fits the stack better** — (a) news→outcome coupling is **direct/causal** on event contracts (vs weak on BTC), (b) CFTC-regulated, all-50-states, real REST+WS API, (c) **wide-spread, less-HFT** books (a maker edge survives), (d) Greg's **energy/macro/weather domain edge** is real here. Same infra, better-fit venue. All S25-S37 crypto work stands (below); it is not discarded, it is the validated track record.
- **BUILT THIS SESSION (all on branch, pushed; scope = `KALSHI_BUILD_SCOPE.md` 4 steps + infra):**
  1. **`research/kalshi/kalshi_collector.py`** — public v2-API snapshot collector. **28-series watchlist**: 12 weather daily-high cities + 7 macro prints (NFP/CPI/PCE/Fed) + 4 energy (WTI/Brent/natgas/gasoline) + 5 electricity (KXPOWERKWH live; ERCOT/PJM/NYC/SoCal auto-capture when they open). Binary YES/NO book folded to a unified YES book (NO bid@q = YES ask@100−q) so `research/shape_s71/early_signal.book_imbalance()` consumes it UNCHANGED. Mid stored in cents = implied probability. Depth-verified live (clean 2-sided implied distributions).
  2. **News pipeline** merged from `origin/claude/run-pass-14-classifier-nTViL` + **REPOINTED** (`news_ingest_rss.py`): feeds → EIA/Fed/NHC (live; BLS 403s datacenter IPs), `BTC/ETH_TERMS` → `CONTRACT_KEYWORDS` map (each Kalshi series a regex), categories → ENERGY/INFLATION/JOBS/MONETARY/HURRICANE/WEATHER. **Verified live: EIA crude items tag `KXWTI`/`KXBRENTD`; Fed→`KXFEDHIKE`; NHC→`KXTROPSTORM`** = the direct causal coupling the thesis is built on.
  3. **`research/kalshi/kalshi_coupling_adapter.py`** + `news_coupling_research.py --source kalshi` — feeds Kalshi mid-probability into the existing signed-edge-vs-placebo engine UNCHANGED (asset=series, venue=market, close=mid; no trade flow in snapshots → flow/volume cols N/A, signed-bps-vs-placebo gate intact). Verified end-to-end (8 obs on `KXCPIYOY`).
  4. **`research/kalshi/kalshi_score.py`** (Option A) — settlement + forecast SCORING harness = **the scoreboard the ⭐ weather-via-OD thread plugs into**. Fetches REALIZED settlement (result/floor-cap/`expiration_value` = the exact number), builds the market-implied ladder distribution (partition ladders=weather mids; cumulative=energy "Above X" differenced), scores ANY forecast (bucket dist OR point value+sigma via Gaussian-over-buckets) vs realized AND vs market (Brier/log-loss/edge). **Proper market baseline = implied dist from bins at `--lead-minutes` before close** (settled last-price is post-hoc near-certain → flagged; activates as bins accrue). Verified on real NYC data.
  5. **`.github/workflows/kalshi_collectors_durable.yml`** — durable GHA cron (6h, ~5h50m/run) mirroring the coin collectors: restore prior gzipped per-series JSONL → append → gzip+push to a **`data/kalshi-bins`** branch (line-count guardrail, ~6% gz ratio). **First run auto-triggered live on push.**
- **TWO THREADS (both live):** (1) **news→contract COUPLING** (macro/energy) — the KALSHI_BUILD_SCOPE thesis, ready, awaiting accrued bins spanning real EIA/CPI/NFP releases. (2) **⭐ WEATHER via real OD** (Greg's own spec, he's running it separately) — turn the OD engine on raw NOAA/NWS/GHCN data to forecast the exact settlement variable → **edge on the OUTCOME not microstructure (does NOT get arbed)**; `kalshi_score.py` is its scoreboard. **CONVERGENCE: electricity prices are weather-driven, so the OD-weather forecaster is also an electricity-price forecaster.** Honest scope (Result Discipline): OD's proven wins RECOVERED KNOWN laws; vs operational NWP (ECMWF/GFS) the defensible first claim is **OD as a local operator / bias-correction on the exact station settlement variable**, tested to beat climatology/persistence/MOS — not "out-forecast ECMWF from raw obs."
- **STANDING KALSHI RULES:** per-CONTRACT deploy (like per-cell); provisional-until-live (only the demo/paper loop confirms fills); **capital locked to resolution → model ANNUALIZED return, not $/hr** (the crypto $/hr frame does NOT transfer); maker fees ≈ 25% of taker; news-jump gap risk → `PAUSE_NEW_ENTRIES` wired to the release calendar; breadth (many contracts) not depth is Kalshi's capacity answer.
- **NEXT:** (1) let `data/kalshi-bins` accrue days across real releases → run `news_coupling_research.py --source kalshi` (the thesis test) + `kalshi_score.py` with lead-time baselines; (2) **Greg: sync the workflow onto the DEFAULT branch for recurring cron** (my token can't trigger runs; push-trigger fired run #1); (3) plug the OD-weather forecaster into `kalshi_score.py` when ready; (4) Kalshi microstructure thread (`early_signal` on the probability series — wide spreads = the maker edge crypto lost); (5) paper loop on `demo-api.elections.kalshi.com` (RSA-key auth). Infra reality unchanged from S37: no AWS connector; Render present but no authed workspace; GitHub token can't trigger workflow runs.
