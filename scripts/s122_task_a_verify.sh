...
git switch -C s122-task-a "origin/$TARGET"
mkdir -p data
...
class RawActionsMemberRowS122Test(unittest.TestCase):
    """F-30: the member row carries the frame's per-record raw actions whole."""
...
import json
from pathlib import Path
from research.kalshi.frankie_raw_mbo_benchmark.tests.test_native_replay_driver import at, make_driver, record
driver = make_driver(total_mbo_records=1)
driver.consume([record(seq=0, event_ns=at('2021-10-04T13:00:00'), order_id=901)])
driver.finalize()
row = driver.counters.member_rows[0]
action = row['raw_actions'][0]
observed = sorted(action)
Path('data/task_a_action_keys.json').write_text(json.dumps(observed), encoding='utf-8')
print('OBSERVED_RAW_ACTION_KEYS', observed)
PY
...
text = p.read_text(encoding='utf-8')
assert text.count('structurally_absent=("raw_actions') == 12
observed = set(json.loads(Path('data/task_a_action_keys.json').read_text(encoding='utf-8')))
removable = {'raw_actions', 'raw_actions[]'}
for leaf in ('source_dbn_sha256', 'source_dbn_object', 'is_snapshot', 'book_effect'):
    if leaf in observed:
        removable.add(f'raw_actions[].{leaf}')
print('REMOVE_STRUCTURAL_ABSENCE', sorted(removable))
changed = 0
out = []
for line in text.splitlines(keepends=True):
    if 'structurally_absent=(' not in line:
        out.append(line)
        continue
    prefix, rest = line.split('structurally_absent=(', 1)
    body, suffix = rest.split(')', 1)
    tokens = re.findall(r'"([^"]+)"', body)
    if not tokens or 'raw_actions' not in tokens:
        out.append(line)
        continue
    ...
    if not kept:
        ...
        continue
    tuple_body = ', '.join(f'"{token}"' for token in kept)
    if len(kept) == 1:
        tuple_body += ','
    out.append(prefix + 'structurally_absent=(' + tuple_body + ')' + suffix)
assert changed == 12
text = ''.join(out)
...
S122 Item 4 Task A / F-30 carries each compact frame's raw_actions list whole onto the exact member row instead of dropping it after member_clock_row consumes it for clock derivation. The structural-absence pins are recomputed from an observed fixture row, and the differential proves streamed and inline retention still carry identical rows.
