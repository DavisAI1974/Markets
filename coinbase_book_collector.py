"""
coinbase_book_collector.py — GENERIC Coinbase L2 order-book collector (any product).

Symbol-parameterized generalization of coinbase_btcusd_book_collector.py (S43),
built to unblock the OD-BOOK / maker-edge bottleneck: the maker-fill model
(odcore/maker_book.py) showed the binding constraint on btc_coinbase is the
ABSENT SPREAD (1-tick book, half-spread ~0.0008 bps), not the signal. Lower-
liquidity coins (SOL/DOGE/XRP) carry materially wider relative spreads, so the
same signal-as-filter + QuietFloor gate should clear net there. This collector
produces those wider-spread cells while reusing the exact battle-tested WS
protocol and row schema of the btc collector.
...
spread and signed trade flow. It runs alongside the trade collectors and writes
a separate file.
...
Run:
  python coinbase_book_collector.py --product SOL-USD \
      --out sol_coinbase_book.jsonl.gz --duration 21000 --grid-ms 100 --depth 10
"""
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Generic Coinbase L2 book collector (OD-BOOK)")
    ap.add_argument("--product", type=str, required=True,
                    help="Coinbase product id, e.g. BTC-USD, ETH-USD, SOL-USD, DOGE-USD, XRP-USD")
    ap.add_argument("--duration", type=float, default=21000.0,
                    help="collection seconds (default 21000 = 5h50m)")
    ap.add_argument("--out", type=str, required=True,
                    help="gzipped JSONL output path, e.g. sol_coinbase_book.jsonl.gz")
    ...
    args = ap.parse_args()
    asyncio.run(collect(args.duration, args.out, args.grid_ms, args.depth, args.product))
