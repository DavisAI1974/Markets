...
#
# These thresholds restored to the pre-Pass-9 baseline 2026-05-10 to
# remove interpretation-driven adjustments. Earlier passes had:
#   - Pass-9: tightened BOTH_SIDES_MIN 0.30 → 0.35 (motivation: exclude
#     η-high subset that predicted momentum; effect was near-zero in
#     practice because per-side η is bimodal at 0/0.50).
#   - Pass-10: added COMBINED_MAX = 0.55 ceiling (motivation: same as
#     above, more effective; reclassified ~290 KR-BTC chunks).
# Both changes were normative model tweaks driven by sub-cell findings
# rather than by the chunk-level data itself. Restoring to baseline so
# the classifier reports labels as originally specified, with the
# sub-cell findings (η-high WASH × momentum r=+0.106; EQ × η-high
# r=+0.150) reported as MEASUREMENTS in the analysis output, not
# folded back into the labels.
