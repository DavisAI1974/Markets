...
BTC-USDT *linear perpetual* (USDT-margined) WS collector on Bybit.
...
Data source: Bybit V5 public linear WS, no auth, no API key.
  stream.bybit.com/v5/public/linear
    - publicTrade.BTCUSDT  -> taker side ('S': "Buy"|"Sell"), qty, price
    - tickers.BTCUSDT      -> bid1Price/ask1Price for mid

Run: python bybit_btcusdt_perp_collector.py --duration 14400 \\
...
def _load_existing_bins(path: str) -> dict[float, dict]:
    ...
    try:
        ...
        print(f"[by-btc-perp] could not load existing bins: {e}", flush=True)
        return {}
...
async def collect(duration_s: float, save_path: str) -> dict[float, dict]:
    ...
    print(f"[by-btc-perp] starting {duration_s:.0f}s on {SYMBOL}"
        ...
        try:
            async with websockets.connect(WS_URI, ping_interval=20, close_timeout=5) as ws:
                ...
                if reconnect_count > 0:
                    print(f"[by-btc-perp] reconnected ({reconnect_count})", flush=True)

                while time.time() - t0 < duration_s:
                    try:
                        raw = await asyncio.wait_for(ws.recv(), timeout=30.0)
                    except asyncio.TimeoutError:
                        print(f"[by-btc-perp] recv timeout t={time.time()-t0:.0f}s", flush=True)
                        continue
                    ...
                    if now - last_save >= 30.0:
                        ...
                        print(f"[by-btc-perp] t={now-t0:.0f}s bins={len(bins)} trades={n_trades}",
                              flush=True)
            ...
            reconnect_count += 1
            print(f"[by-btc-perp] WS issue ({type(e).__name__}: {e}); retry 5s (count={reconnect_count})",
                  flush=True)
            _save_bins(bins, save_path)
            try:
    ...
    _save_bins(bins, save_path)
    print(f"[by-btc-perp] done. bins={len(bins)} reconnects={reconnect_count}", flush=True)
    return bins
...
def main():
    ...
    p.add_argument("--bins-path", type=str, default="btc_bybit_perp_bins.json")
    args = p.parse_args()
