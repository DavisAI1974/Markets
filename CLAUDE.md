# CLAUDE.md — DavisAI Master Context (Updated 2026-06-22 Session 36b — body canonical through S25; S36b+S36+S35+S34+S33+S32 delta at top. READ `S36_NETCOST_BACKTEST_FINDINGS.md` + `SESSION_HANDOFF_2026-06-22_S36.md`, not this whole file)
...
- **QUANTUM question RESOLVED (Architect, S36b — precision NOT speed):** the DavisAI Hilbert-Space-Unification / merge-math says classical-∞-dim ML and quantum ML are the SAME mathematics on different substrates (difference is how the inner product is computed, not the operator) → it FORBIDS a quantum-execution edge rather than enabling one: any "step ahead of price" would exist classically too. So **build the operator OFFLINE, run it CLASSICALLY in the hot path; NO quantum hardware between tick and order** (a turn-detector on an order-flow+price window fails every prong of the quantum-advantage test — low-dim, cheap classical eval). Independently CONFIRMS our split: OD's edge is on turn FILTERING (which reversals are real), fine-resolution PRICE pins the TIMING near the fee floor. Latency stays classical HFT physics (colo/FPGA/kernel-bypass).
- **Actionable edges (Architect) + done:** (1) **per-leg ASYMMETRIC fee floor** (`_info_dipole_fee_floor.py`) — because the dipole predicts the turn you can REST a maker limit there → floor drops 22bps(taker)→13(maker-in)→**4bps(maker/maker)**, **1.6–3.5× the oracle opportunity** (fill-risk caveat). (2) **regime master-gate** via the dipole C ratio (H_self/H_cross → toxic-flow / MM-withdrawal / liquidity-crisis → go FLAT) on top of filter+timing — worth more than another entry bp. (3) **incremental/sliding operator update** (recursive rolling state, not per-tick recompute) = cheapest latency win, no new hardware. (4) **pre-entry-window leakage check on EVERY new signal before any backtest** (the algebraic-dipole 0.993 OOF/FN=0 is unvalidated and exactly the shape leakage takes — do NOT enter the harness until it survives). **Falsification harness (D):** OD operator vs a strong CLASSICAL champion (order-flow imbalance + tuned reversal filter), both classical, on frozen fine-res data; metric = bps-to-turn + FN-rate at matched FP, net of the per-leg floor, OOS. Architect's soft prediction: OD wins on FILTERING, not TIMING.
