...
- **⭐ SUB-MINUTE PIVOT (Greg: "1 min is an eternity; NYMEX moves a lot in a minute"):** Yahoo 1-min high/low was BROKEN (under-sampled). Pyth real 1-min OHLC: intra-minute range **11¢** vs 6.4¢ close-to-close (1.6× hidden), **15–23¢ around the 14:30 EIA release**. Historical **sub-second NYMEX EXISTS** (Pyth Benchmarks `/v1/updates/price/{ts}`; rate-limited ~2 req/s). Direct event-study (10s NYMEX + ms Kalshi tape): on a +17¢ NYMEX jump the liquid strike repriced **within ~7–20s, NOT a minute** → the "full-minute lag" was **coarse-bar aliasing**; the exploitable lag is in LESS-liquid strikes, measurable cleanly only sub-second.
