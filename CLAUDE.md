...
- **BLOCKER for priority-1 (sub-second Pyth lag):** the Pyth durable workflow run #1 (`29185118959`) has been **stuck `queued` ~since 07:59 UTC and never executed** — `data/pyth-ticks` branch does not exist, zero ticks collected. Plus it's Sunday: energy futures don't reopen until ~Sun 22:00 UTC. **Greg: click "Run workflow" on `pyth_collector_durable.yml` / check why run #1 is jammed** (likely concurrency behind the 6h kalshi collector). Sub-second lag test is next-session once ticks accrue.
- **NEXT (S83):** (1) join Pyth futures move onto the level-hit dataset (the external-driver hypothesis); (2) sub-second lag on accrued ticks (priority-1, once feed unstuck); (3) Thu natgas live re-run + join actual via EIA; (4) roll re-point Jul 21+.
