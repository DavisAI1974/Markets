# CLAUDE.md — DavisAI Master Context (Updated 2026-06-30 Session 45 — body canonical through S25; S45+S44+S43+S42+S41+S40+S39+S38+S37+S36b+S36+S35+S34+S33+S32 delta at top. READ `SESSION_HANDOFF_2026-06-30_S45.md` + `SESSION_HANDOFF_2026-06-29_S44.md`, not this whole file. BRANCH = `claude/crypto-liquidity-signals-5c5vg9` (== `-kb2i5c` == `-s45-y2ni2m`, synced).)

## S45 UPDATE (2026-06-30) — decisive maker test ran on the multi-coin book: per-cell operators flip DOGE NET+, fill autopsy pins the bleed on adverse selection, the validated dipole FLIP detector is NOT wired in (the fix) (read `SESSION_HANDOFF_2026-06-30_S45.md`)
...
- **ARE WE USING THE DIPOLE FLIP DETECTOR? NO.** The maker path imports only `quiet_floor.gated_signal`; the validated S36 FLIP detector (`odcore/flip_detector.py` / `odcore/info_dipole.py` divergence+exhaustion, 64% reversal) is NOT wired in (`incremental.py` only mentions it in a comment). **THE FIX (per-cell wiring, NEXT #1): gate the maker quote on the FLIP/turn detector** — post the bid only at a valley/flip, the ask only at a peak/flip — converting "bought-on-the-way-down" fills into "bought-at-the-bottom." doge already cleared on depth-K alone; the flip gate is the lever for sol/xrp/eth. Chat takes the per-coin OD run for the turn/toxicity operator (default run is VAL-only/SAFE; the frozen taker sentinel does not bear on the maker thread).
