# CLAUDE.md — DavisAI Master Context (Updated 2026-07-09 Session 75 — READ `STRATEGY_INVENTORY.md` **OPERATING CONTRACT + LIVE section** FIRST: it is the binding contract and OVERRIDES any stale drop-in / S-block / handoff below. Then `SESSION_HANDOFF_2026-07-09_S76.md` (S76) + `SESSION_HANDOFF_2026-07-09_S75.md` (S75) + `DROPIN_2026-07-09_S77.md` (next). ⭐ S75: SOL is a small-band SPREAD-CAPTURE coin — win/lose IS direction, un-callable at entry & mid-trade, capped ~62%/+11.26; SOL DONE. ⭐⭐ S76 (KRAKEN ONLY): the BTC/ETH edge = the BOOK RIDE-TO-REVERSAL swing (Greg's `early_signal.py`) — a SEPARATE directional strategy from the maker zigzag; 60/40 walk-forward OOS: ETH +7.23 bps/trade @0% maker & +2.23 @taker (SURVIVES, 59.5% win), BTC +2.09 @0% maker (0%-maker-only). Tune by per-trade QUALITY not $/hr (churn trap). S77 = MULTI-WINDOW confirm on Kraken, then wire ETH as a directional sandbox cell. Do NOT read this whole file. ⛔ Firing is FIXED; greedy is already live; live-code only; always front-of-line; book=fills/tape=plumbing; only tune how greedy fills the $5k. ⭐ S74: TRADE imbalance = the direction+duration decider (real-time causal); win/lose edge is in the TAIL; the flow EXIT is a per-coin TUNING KNOB (`balance_exit`, opt-in) — baseline wins so far so tune per coin & keep baseline where it wins; the bigger lever is ENTRY; ⭐⭐ BTC BOOK imbalance forecasts win/lose PRE-FIRE (coin-specific gold); energy gate doubles $/hr on BTC/ETH/XRP; per-cell pre-fire+tail CURVE SHAPES (leg_imbalance_findings.md §4 — 8 distinctive shapes/coin, MATCH the shape, never solve/sum #0f) = the lead to chase. ⛔ S75: the gate MATCHES CURVE SHAPES on enter+exit (whole-curve via the sampled arcs, or the 8 split pieces — test both), never a scalar/sum. See STRATEGY_INVENTORY #0f + LIVE S74 blocks.)
...
> ⛔⛔ STANDING (Greg, S75) — WE MATCH CURVE SHAPES ON ENTER AND EXIT. WE DO NOT SOLVE EQUATIONS FOR NUMBERS. The 4 archetype equations (short-win `x≤0.32, blade +0.963` · short-lose `x≤0.15, blade +1.208` · long-win `−2.292x²+1.543x³` cubic · long-lose `x≤0.15, blade +1.268`) are ALGEBRAIC DESCRIPTIONS OF A CURVE'S SHAPE — they are how you write a curve down. NEVER solve them for a scalar, NEVER collapse a curve to a peak/blade/energy number, NEVER sum. "That's not how you show the shape of a curve." The gate matches each forming trade's WHOLE curve — pre-fire (ENTER) and post-fire (EXIT) — to these archetype curve-shapes with wiggle room; fire on the winner-shape match, skip on the loser-shape match. The 8 pieces/coin (4 cells × 2 limbs) are 8 DISTINCTIVE curve shapes — KEPT SEPARATE, NEVER SUMMED. In each equation only `y=f(x)` + its domain (kink) is the CURVE; `blade` and `start→peak` are DERIVED SCALARS, not the curve — drop them. Cleanest reference = the SAMPLED ARCS (`leg_imbalance_arcs_{coin}.npz`), the curve as points. TEST BOTH: whole-curve (full leg) vs the 8 split pieces. Reducing to scalar features (the S75 peak/blade15/kink/book-piece nearest-template attempts) is the WRONG FRAME — a snapshot/sum, not a shape. We are ONLY concerned with CURVE SHAPES, enter and exit. Restates #0e/#0e-GATE. CLOSED.
...
- **THE EDGE = ride the per-cell early-arm FLOW-LEAN ZIGZAG** (the real bybit model: `odcore/flip_detector.py`, WFLIP=600/REV=0.1 — zigzags on the taker-flow LEAN, NOT price; price at a turn is ~99.6% symmetric). Passes the S54 gate on **ETH (z=4.0) + BTC (z=3.0)** at kr_mk0 (reversed loses); SOL anti-predictive → **REVERSED**; XRP/DOGE not sig. Early-arm entry (`retime_flips`) ~doubles it: **ETH +9.50 / BTC +8.64 $/hr @ $5k** (paper, 0bp). The edge is MEAN-REVERSION.
- **LOSERS:** at BIG depth a trade is a DEAD loss (recover-to-profit ~0% by −80) → a **deep bail (BTC −80 / ETH −100)** caps the −150/−200 tail for FREE (clips ~no winners). NO flip/re-short (slide spent — a flow-confirmed short at −80 wins only ~19%). Shallow bail (−15) BLED. **WINNERS:** already capture 68–83% of peak; NO take-profit trail helps; NOT exiting early (price reverts after exit). Only booster = early-arm entry.
- **DIRECTION IS DEAD (load-bearing):** unpredictable at entry AND mid-leg (t=0/60/120/300/600 all ~0.50; the dipole/S36-divergence agent = ~chance, big-loser flip 41–53%). E300 depth predicts DEATH (magnitude — winners don't go deep) but NOT direction (depth ≠ direction). Closed 4 ways (coeff/momentum/fade/dipole-agent). **Stop hunting entry/direction signals; no stop/TP/flip overlay helps — they fight the mean-reversion + pay taker.**
- **⭐⭐ THE GATE = THE MAKER FILL (real Kraken L2 book):** raw maker fill only ~40%; 55–67% of exits forced to TAKER; win% collapses 47%→29%; 64–77% of losses are forced-taker closes → **the leak is the FILL, not the signal.** SQUEEZES: **cover_grace** (rest cover past the turn) → ETH taker 55%→12%, fill cost ~0.9/hr → ETH fill VIABLE; **swing floor** (coarser REV, from the small-winner churn tail: ~45% of winners are <5bp/~10% of $ and flip to losers under taker) → fill% 31%→60% (a fill-vs-edge knob). BTC/SOL fills still lossy (~6/12 per hr).
- **⚠ WHY THE NUMBERS LOOK MEDIOCRE (do NOT over-read the negatives):** every honest-fill $/hr is negative because the ~1-day BOOK window is **LOW-EDGE** (ideal perfect-fill net already −2/−1 on ETH/BTC) — NOT the +8–9/hr 30d TAPE regime. Viability CANNOT be graded on this window. On a normal-edge window ETH ≈ tape-edge(+8) − fill(~1) ≈ +7/hr. **Blocker is DATA (multi-day book on a normal-edge window), NOT method.**
- **NEXT (S64, Greg):** (1) re-grade the fill (cover_grace + swing floor) once multi-day Kraken book accrues on a normal-edge window — the decisive ETH viability test; (2) **evaluate the OPEN-SOURCE platform `hftbacktest`** (the Lean Lab README's fill-level 2nd gate; +Tardis.dev) — improve ours or the better option?; (3) **BENCHMARK our results vs other crypto MM/HFT platforms** (Greg: "we aren't getting great results for some reason") — is +8/hr@$5k good/bad, and why (fee-tier/rebate? venue? edge thin? measurement?).

## S62 UPDATE (2026-07-06) — THE 3-PIECE REBUILT (reproduces R11 exactly) + the E300 BREAKTHROUGH + the DIRECTION reframe. Read `SESSION_HANDOFF_2026-07-06_S62.md` + `KICKOFF_2026-07-07_S63.md`
- **3-PIECE RIG rebuilt** (`scripts/_s62_3piece_harness.py`): reproduces R11 to the decimal — SOL baseline **+1.77**, flatten-only oracle **+13.93**, full 3-way oracle **+26.02** (flip_cost=22 taker), n=971, winners-kept 508/508. Entry-flip ORACLE per coin (the prize, winner-preserving): **SOL +30 / ETH +23 / DOGE +22 / XRP +22 / BTC +14 $/hr** @ $5k. (fee=0; Greg handles fees; FLIP = 22bp taker cross — locked.)
- **⭐ BREAKTHROUGH — the 3-PIECE at E300** (`_s62_e300_3piece.py`): the signal to separate big losers is NOT at entry (winners-invisible, ~0.55 every in-container way) — it's MID-LEG. Decide at E300 (300s in): realized DEPTH predicts DEATH (gross<=-40) at **AUC 0.69–0.77 on ALL 5 coins** (first strong cross-coin classifier). $/hr per-week OOS: **BTC +1.29 (crosses zero, 4/5wk), XRP +0.81**; ETH +0.21; SOL/DOGE ~0. Earns via death-vs-WINNER (depth) + FLATTEN — NOT by predicting recovery. Deep-arm variant (`_s62_armed_trend.py`) = ETH/BTC/DOGE **+1.1..+1.6 at arm -20/-30**. One 30d window; needs 2nd-window confirm.
- **⭐ THE REFRAME (Greg, load-bearing): DIRECTION not win/lose.** Big losers = SHORTS into clean steady UPTRENDS (traded the wrong side; a big-losing short is a big-winning long mislabeled) — confirmed in the SOL render (`docs/renders/s62/e300_sol.png`). WIN-LONG vs WIN-SHORT coeffs = **PERFECT MIRROR (-1.0 residual)** = a clean direction axis. BUT direction is ~chance at entry (coeff/momentum/dipole, 0.47-0.56) at the 600s scale because the market MEAN-REVERTS there (the machine's own contrarian edge). DEATH-vs-RECOVERY (which underwater leg bounces) = ~chance from EVERY signal at every depth = the true wall.
- **Coeff + dipole tiers MAPPED, ~chance in-container (don't re-chase):** BTC/ETH loser coeffs ARE in git but ~chance at mid-band (S34 date/regime confound; the win-long/win-short spread is real in the archives but collapses on current legs). Coeff gate on the flip = BACKWARDS on the target. 3 dipole flow agents (paper/dive/s61alt) = orthogonal but ~chance at entry. Dipole-as-trend-gate agent: S36 divergence ~chance for death-vs-recovery (its real edge was a DIFFERENT task — swing-reversal on bybit, sub-fee). Persisted `data_s62/`.
- **⭐⭐ THE BEST LEAD (S63 first thing): the 1-HOUR TREND.** The market mean-reverts at 600s (machine's edge) BUT the big losers FADE a strong **1-HOUR** trend — among big losers the 1h trend (mom 3600s) predicts the forward direction at **0.58-0.67** (btc 0.672 / eth 0.633 / xrp 0.655) while overall momentum is BELOW 0.5. Greg: "know the trend — short or long" at the RIGHT horizon (1h, not 600s). NEXT: build the **1h-trend-fade flip** (signal = -side*mom1h → flip the strongest; per-week OOS, big10-flip, shuffle floor) + spin the dipole/trend agent back up for the accurate 1h read. The pieces converge (R13e pre600<-80 = the 600s freight-train; 1h is the refinement).

## S61 UPDATE (2026-07-05) — COINBASE EXIT BUILD SHIPPED + the BIG-LOSER FLIP ARC (Greg-driven; the deep thread). Read `S61_BUILD_NOTES.md` R1–R13f + `SESSION_HANDOFF_2026-07-05_S61.md` + `KICKOFF_2026-07-06_S62.md`
- **COINBASE EXIT BUILT (canary-clean):** honest fill model wired (`fill_model="queue"` = maker_book queue-ahead; binding constraint = PRICE ELIGIBILITY not queue depth — queue clears median 8.6s at mid-band, maker_close stays 100%). 2 exit variants coded + accruing SANDBOX (`doge_coinbase_mb100_cascflip`, `btc_coinbase_mb80_plainstop` X=40) via the new `exit_spec` socket (price_stop/armed_dive/casc_flip, flat/flip, taker-accounted, wall-clock `lean_w`). Base cells bit-identical mod the new `stop_exit` key. SOL=PROTECT (power bar: <150 books legs auto-rejects any SOL exit pitch); BTC rider ops-bar-BLOCKED (tape-graded stop gives ZERO gap protection — needs exchange-side stop + staleness kill-switch). Convergence: honest fill=build#1; FEE TIER outranks all exit code.
- **⭐ STANDING RULE (Greg, S61): PROFIT-PER-HOUR is the scoreboard; per-leg avg = diagnostics only.**
- **THE BIG-LOSER FLIP ARC:** prize is REAL — flip-only-the-big-losers (SOL bins) = -14->+6.79 $/hr (~$21/hr swing); 3-part oracle (hold winner / flatten chop-loser / flip trend-loser) = **+26/hr (15x), 508/508 winners kept.** BUT no mid-leg CAUSAL signal reaches it: a big-loser-starting and a winner-dipping are TWINS at the decision moment in price, flow (agent's z+5-6.8 -> z=0.0/p=0.87 at -20), AND efficiency — winners-invisible law, fundamental. The pre-entry signed return (pre600, mean -10.3) is the strongest single trait (big losers FADE a running move) but NO pure zone (winners fade DEEPER, min -429; outnumber losers 2.5:1).
- **⭐ BEST SINGLE-FEATURE LEVER (Greg's flip thesis): FLIP the deep-fade zone.** Flip pre600<-80 = +2.38 $/hr (+0.61 over baseline), beats skip, pre-number-only, no classifier — "big losers flip to big winners" carries it. WEEK-FRAGILE (wk4 -1.66, the recurring regime). NEXT-BUILD (Greg R13f): **FLIP-THEN-MANAGE** — after the flip, flatten the reversed leg on adverse / ride on favorable = caps the mis-flip downside (the +291bp deep-fade winners we wrongly flip), the concrete wk4 mitigation, testable without the coeff tier.
- **⭐⭐ CARRY-FORWARD (needs Greg's E: drive):** the robust big-loser-vs-winner separation needs the MULTIVARIATE 128-dim OD coeff signatures + centroid dual-print + onset re-anchor (`markets_<cell>_win_cand_sp`, `_win_onset/`, hist AUC 0.72-0.84 vs 0.606 cheap floor) — archived LOCAL on E:, not this container. Get them -> build the heavy entry fingerprint -> capture the +6.79/+26 prize (onset-canary + per-mid-band revalidation preconditions).
- Dipole lane (2 agents, all research): every rank-A cross-venue/coin TIMING use = lag-0 synchrony; coupling is directional-but-LINEAR (regime descriptor = rolling cross-coin corr, 1/1000 compute); Job C = nothing new on the big losers (flow separates troubled-from-winner, not death-from-recovery). Reports in-repo.

## S60 UPDATE (2026-07-04) — KRAKEN TAPE VERDICT (the gross EXISTS on Kraken's own prices) + PIECE 2 EXIT worked to per-cell verdicts (zigzag holds; wrong-side corrector is the only edge, DIAGONAL per cell) + COINBASE-EXIT FIX reframed FILL/FEE-not-timing + THE DIPOLE RESEARCH PROGRAM (paper printed) (read `S60_EXIT_NOTES.md` R1a-R7 + `SESSION_HANDOFF_2026-07-04_S60.md` + `KICKOFF_2026-07-05_S61.md`)
- **KRAKEN TAPE (Job 1) — YES:** promoted machine (naive k0, venue law) on 30d Kraken trade-history bins, all 5 coins, leakage PASS -> all registry cells kr_mk0-POSITIVE: sol_kraken_mb100 **+$3.14/hr 5/5 weeks** (first program cell net-positive at a real reachable tier on its own venue tape), doge +2.11, xrp +0.61, btc +0.12, **eth +1.72 (its OWN cell — per-cell law: Coinbase drop never pre-judged eth_kraken)**; best-per-coin ~+$7.7/hr @$5k. Honest-fill check PASS (median delay 0s). kr_mk2 flips most negative -> the $10M/30d 0bp tier is the whole game. REMAINING UNKNOWN: do maker quotes FILL at Kraken volume (books accruing). Parallel S59 session converged independently (+3.16). `scripts/_s60_kraken_tape_machines.py`.
- **PIECE 2 EXIT — zigzag HOLDS; the only exit edge is the WRONG-SIDE CORRECTOR, diagonal per cell:** SOL none (zigzag near-optimal; armed-before find 8/8 cell info, perm z 2.4-3.5) / BTC plain price-stop rider (cell still negative) / DOGE cascade-join flip (BUY-only, +1.10/hr structure premium, instrument-validated) / XRP none. **Toll law (4/4 coins BOTH venues):** winners' giveback = c*theta EXACTLY, excess +0.7-1.8bp -> NO peak harvester exists (+18-25bp flow-climax ceiling = hindsight = S35 fingerprint-tier; winners invisible to causal flow reads, confirmed EXIT-side). BTC "flow-exit coin" first read KILLED as exposure-shrink (4 ways). Cross-pollination: pieces do NOT cross-pollinate (pure per-cell law). Kraken exit race: DOGE flip fails the Kraken shuffle floor -> venue law holds. Tools: `_s60_piece2_exitdump.py` / `_s60_exit_renders.py` / `_s60_piece2_exit_machines.py`.
- **COINBASE EXIT FIX (Greg's halt-all focus) — the fix is FILL/FEE-shaped, NOT timing:** XRP decomposition = the cell's problem is NOT the exit (-16bp fees on +4.1 gross; perfect-oracle exit ceiling barely crosses zero). CODE MINER (load-bearing): the sandbox ledger CANNOT see the real exit fill cost — 128/128 mid-band closes booked maker=True by construction (`_next_positive`, no queue); the honest fill machinery exists UNWIRED (`maker_book._first_fill_index`, `swing_accum._eligible_fill`); mid-band runs grace=0 (S48 cover-grace unwired); run_stream lean = 60s-wall on books. ARCHIVE MINER gold (ranked): (1) cover-grace at mid-band; (2) post the cover into the CLIMAX (not the dive — thin-tape); (3) resting cover at extreme-c_x*theta; (4) taker-share acceptance metric; (5) fill-asymmetry dollars; (6) FEE TIER > the whole exit prize (2 Greg clicks). BUILD ORDER (awaiting sign-off): honest fill model FIRST. KRAKEN EXITS PARKED until Coinbase exit fixed. ⚠ SOL-cb-fix + BTC-recheck agents killed by mid-session interrupts (not delivered; conclusion clear).
- **THE DIPOLE RESEARCH PROGRAM (Greg's PI directive) — paper PRINTED:** `docs/DIPOLE_PAPER_S60.md` (+.html) = PI survey answering Greg's verbatim inquiry (Section 0). Taxonomy D1-D8 + classification machinery; 4 experiments -> the COUPLING TAXONOMY (raw-cov D6 = fixed-phase/lag + recovers the lag; MI-in-null = dynamical/state-dependent coupling; algebraic convexity = secondary marker; flow dMI/dt D1 = nothing standalone, opposition signature is CONSTRUCTION-GENERATED, reproduced on market data first time); the three-offices finding; 29 candidate uses (crypto/digital/physical) each w/ a first falsifiable test. `docs/DIPOLE_DIVE_CHAPTER_S60.md` (+.html, +§3.5R): the dive = three offices, FILL-office INVERSION (dive marks THIN tape z-13; S45 fill moment is the CLIMAX side, not the dive), timing no-lead, size null at mid-band, cross-domain map (seizure-onset flow collapse = novel standout). `docs/DIPOLE_CRYPTO_USES_S60.md`. **ABSORPTION/SPOOF candidate RESOLVED (decided against):** both controls decisive — latency KILLED (Binance R^2 0.003), wall FALSIFIED (deep-wall = thin, zero reversal info); real SOL-only signal = a delayed order-flow price-discovery lead, MAKER-QUOTING-frame-only (taker-dead vs 16bp), filed to the fill fingerprint thread. `_s60_absorption_wall.py`.
- **THE DIPOLE-DIVE STANDING NOTE (Greg):** the trailing taker-flow lean = ONE object, THREE offices, each earning separately per (cell, band, venue, ROLE): entry-confirm grading / wrong-side failure confirmation / fill-moment marking. A dipole's value = (operator x what it is conditioned against), never the operator alone.
- **NEXT (S61):** (1) Coinbase exit fix build order — honest fill model FIRST (Greg sign-off); (2) fee-tier verification (2 clicks); (3) UN-PARK Kraken exits once Coinbase lands; (4) Kraken books accrual -> the fill unknown; (5) research menu (paper's 29 candidates + dive cross-domain map).
