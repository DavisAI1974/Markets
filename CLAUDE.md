# CLAUDE.md — DavisAI Master Context (Updated 2026-07-02 Session 52 — body canonical through S25; S52+S51+S50+S49+S48+S47+S46+S45+S44+S43+S42+S41+S40+S39+S38+S37+S36b+S36+S35+S34+S33+S32 delta at top. READ `SESSION_HANDOFF_2026-07-02_S52.md` + `S52_SIZING_MATRIX.md` + `SESSION_HANDOFF_2026-07-02_S51.md`, not this whole file. BRANCH = designated `claude/crypto-liquidity-signals-*` (== canonical `5c5vg9`; keep synced).)

## S52 UPDATE (2026-07-02) — JOB 1 DONE: the sizing-on-winners accounting is AIRTIGHT (entry sizing correctly credited; winner-side extensions FALSIFIED); canonical $/hr-with-sizing matrix delivered. Job 2 (Bybit) data-gated (read `SESSION_HANDOFF_2026-07-02_S52.md` + `S52_SIZING_MATRIX.md`)
- **Greg's S51 concern RESOLVED — "you aren't factoring in the size up on the winners."** Full accounting done, falsification-first. Deliverable = `S52_SIZING_MATRIX.md` (the canonical "$/hr with sizing factored" every future citation uses; flat vs entry-sized vs winner-sided, per cell × fee scenario × v1/v2 fill).
- **Job 1a — does `_dollars()`'s `min(size×S, flow)` under-credit sizing? NO.** `corr(size,cap)>0` on all 5 cells (+0.05..+0.10) → high-conviction legs sit on fatter-flow turns, so the min() model DOES capture the fat-leg concentration (not thrown away). `corr(size,net)≈+0.03` (entry sizing loads |move|, not wins — S47 re-confirmed); `corr(cap,net)≈0-to-negative`. Sizing lift →0 at the flow-capped ceiling is the PHYSICAL flow wall, not a modeling artifact. At $1k/leg the cap already binds 54–87% of legs → sizing acts on the fat-flow minority (exactly where corr(size,cap) puts it). Capital-MATCHED lift (isolates allocation from the small mean_size≈1.07 deploy-more effect): SOL +21%→+17%. Tool: `scripts/_s52_sizing_audit.py`.
- **Job 1b — winner-side sizing BEYOND entry conviction = both mechanisms FALSIFIED.** (i) Sequence anti-martingale ("size after recent winners") needs leg-outcome PERSISTENCE — there is none (lag-1 net autocorr ~0 on every cell, never sig-positive; ETH mildly anti-persistent z=−3.2; prior-k→next-net corr ≤ |0.035|, E-spread ≈0 bps). Leg outcomes are ~independent (regime resets each turn). (ii) Within-leg green-adds ("add only when green") needs green legs to offer maker flow — they offer LESS: winners' fillable $ = 0.32–0.65× losers' on 4/5 cells (DOGE the thin/noisy exception). The market force-feeds fill to LOSERS (S45/S51 adverse selection, re-confirmed on the cap model) → a green-only add STRUCTURALLY can't load winners. Tools: `_s52_winner_persistence.py`, `_s52_winner_fillability.py`. Winner-side sizing re-testable only on the Bybit venue book + queue model (fill-share + reversed-control) — not off Coinbase.
- **The surviving sizing lever = ENTRY conviction** (already deployed, `odcore.swing_maker.size_legs` hi_clip=4.0): +8–21% as-deployed at $1k (mostly SOL/ETH), SHRINKING with the rebate (uniform per-leg add lifts the flat baseline faster), →0 at the flow ceiling. A capital-constrained-regime lever — real, kept, NOT an order-of-magnitude multiplier. The OOM levers stay the REBATE (mk0→−1bp ≈3.9× SOL, super-linear) + venue FLOW (Bybit ~10× tape).
- **Job 2 (Bybit venue-cell) DATA-GATED:** `data/{sol,eth}-bybit-book` NOT yet on origin (cron `0 */6` started 07-02; 5h50m runs push at end; token can't GHA-dispatch → Greg may need to trigger the first run). Confirmed absent this session; deferred to next session once ≥1 window lands. All 5 Coinbase books re-materialized to `/tmp` from `data/<coin>-book` this session.
- **NEXT (S53):** (1) Bybit venue-cell measurement once books land (Job 2 — the decisive test; full stack per venue-cell + winner-side add re-test under the queue model); (2) Greg: Bybit MM application (institutional_services@bybit.com) + Backpack VIP. DEAD adds (S52): sequence anti-martingale sizing; within-leg green-adds on Coinbase (falsified). Everything DEAD from S51 stands.
