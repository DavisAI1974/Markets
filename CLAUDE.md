...
- **2026-09-20 22:35Z: CYCLE 0 IS RE-RUN WHOLE, FROM THE BEGINNING (Greg: "a full rerun from the beginning and not steps"; "Rerun classroom also").** The 22:06Z piecewise dispatch (run 35540633095: retained machine half, new Frankie request only) was CANCELLED before its host job started. The cycle's order is fixed and sequential: native BOSS -> Granite critic -> export -> Frankie's session (request + same-session classroom teach-back + correction) -> native learning -> readback -> completion; nothing on the machine computes exhaustion chains, D's or families (delivered as the 49 registry input layers; the derivations are Frankie's step, after the machine result reaches him, never concurrent). Built with receipts, nothing deleted (f0910e6c): `feedback_cycle._cycle_supersede` (declared, OLD binding hash, refused past any retained Frankie output; archives binding/controller/export/attachment/intent so `run()` starts over), `declare_identity_supersede.py --supersede-cycle`, declaration input `supersede_cycle`, `frankie_host_supersede_cycle.yml` (moves `execution/cycle-00` whole + `handoff-<sha256(request id)>` aside). Round: host advance f0910e6c -> cycle-state supersede -> declare (`supersede_cycle=true`) -> ONE pipeline dispatch -> readiness re-delivered for the NEW critic request sha -> re-dispatch -> HOLD with the new Frankie request -> export -> Root. Record: `CLAUDE_HANDOFF_20260920.md` 22:30Z; drop-in "State at 22:35Z".
- **2026-09-20 22:10Z: CYCLE 0's FRANKIE HALF IS BEING RE-ISSUED ON A NEW REQUEST (Greg: "We are applying this to cycle 0 and re-running cycle 0").** Root's earlier reports (recorded, fork, PR) were not real (`rootdavis` is not a GitHub account); the Frankie half of cycle 0 never ran, the machine half is retained. Two changes to how Frankie runs, on Greg's word, both package code (family green each time): (1) **nothing hidden from Frankie** -- `knowledge/RUN_FINDINGS.md` (append-only ledger) and his own prior lessons are rendered whole into every prompt, witness pinned in the attachment (47dd6e57); (2) **the calculations are Frankie's, not the runner's, and the required set is the registry** -- the request names the 49 calculation layers of the August 28 A_MEMORY recalculation (crosswalk of run 33746436209) verbatim in seven groups, requires one `calculation_accounting` lesson entry per layer (derived / compared / could_not with reason) and the ten append-only output ledgers as lesson entries (56111bf1, a60f0b1b; the constant is tested against the crosswalk JSON). Round run with receipts, nothing deleted: host advanced to a60f0b1b; code-bound state superseded (`superseded-code-bound-state-20260920T220441Z.json`); the old principal request e0c461d7 moved aside (`principal-request-superseded-20260920T220438Z.json`); declaration with `supersede_principal=true` (old attachment eeaf4afc); pipeline run 35540633095 dispatched 22:06Z on the launch branch to re-prepare cycle 0 and return to the HOLD with a NEW request. Then: export (`frankie_host_export_principal_request.yml`) -> Root's task updated -> Root's session pushes to `root/cycle-00-response` -> `frankie_host_record_principal_response.yml` -> ONE pipeline dispatch -> verify, learning, readback, completion, classroom turn -> cycle 1. Record: `CLAUDE_HANDOFF_20260920.md` 22:05Z; drop-in `DROP_IN_CLAUDE_20260921.md` "State at 22:10Z".
- **2026-09-20 20:10Z: CYCLE 0 IS AT THE DESIGNED HOLD.** Pipeline run 35533855801 ended `cycles exited 3`, status `actual_frankie_session_pending` (run directory `C:/Codex/Frankie-BOSS-20260919/actual-feedback-run`, principal request `execution/cycle-00/principal/session-request.json` of 16:39:23Z, prompt `principal/prompt.md`). The coordinator accepted the declared supersede (code hash, spent arm, retained export `boss_commit`) and reached `recover`. NEXT IS HUMAN: Root's Frankie session records the response with `operations/record_actual_frankie_response.py` (configuration + sha256, cycle 0, response + sha256, host attestation + sha256); then ONE re-dispatch of `frankie_journal_stack.yml` on the launch branch resumes into verify -> native learning -> readback -> completion -> cycle 1 (which needs its own readiness delivery). **No package code changes until both cycles are done (Greg, 20:15Z)**; the notes for after are `NOTES_FOR_CLAUDE_CHAT_20260920.md` + `SIMPLIFICATION_NOTES_20260920.md`. Host RUNNING `KeepRunning=true` at 2b069fc2, Pod RUNNING, nothing deleted. **Per-cycle report, no limit
  (Greg, 20:35Z): `frankie_host_cycle_report.yml` writes the whole report to a file on the host, uploads it by
  presigned PUT and attaches it as a workflow artifact (run 35536537442); Frankie's own analysis has no cap anywhere
  in the code. Priority once reports exist (Greg, 20:46Z): keep the next cycle running; glance, deep-dive later.**
- **2026-09-20 19:45Z: cycle 0 was ONE provenance pin from the designed HOLD; the receipted supersede covered it (696f2275, on Greg's word).** Since 16:28Z every refusal was the same coupling: `identities.code_hash` spans every `.py`, so each host advance during the open cycle re-mints the training identity, the checkpoint digest, the request plan's `arm_hash`, the coordinator's saved binding and the export manifest's `boss_commit`. Fixed with receipts, never relaxed: the re-prepare path (842ec2ee), the declared cycle identity supersede (`feedback_cycle._binding_supersede` + `operations/declare_identity_supersede.py` + `frankie_host_declare_identity_supersede.yml`, bad519b7), extended to the spent controller arm (963ee275, after the read-only `frankie_host_binding_diff.yml` probe showed exactly two differing keys) and to the retained export's `boss_commit` pin (696f2275; run 35530475076 had ACCEPTED the binding and refused at `_export_verified`, content still hash-verified). Sequence from here: family + CI green -> advance -> code-bound supersede -> declare -> re-dispatch -> `actual_frankie_session_pending`. Record: `CLAUDE_HANDOFF_20260920.md`. Host RUNNING `KeepRunning=true`, Pod RUNNING, nothing deleted.
