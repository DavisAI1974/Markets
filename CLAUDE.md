# CLAUDE.md — DavisAI Master Context (Updated 2026-06-30 Session 49 — body canonical through S25; S49+S48+S47+S46+S45+S44+S43+S42+S41+S40+S39+S38+S37+S36b+S36+S35+S34+S33+S32 delta at top. READ `S49_SECOND_WINDOW_FINDINGS.md` + `S48_TAKER_CUT_FINDINGS.md` + `S47_SIZING_FINDINGS.md`, not this whole file. BRANCH = `claude/crypto-liquidity-signals-s49-rqtcfk` (reset onto canonical `5c5vg9`; keep `5c5vg9` synced).)
...
- **NEXT (S50):** (1) **confirm maker fee ≤0 / rebate is real on Coinbase** for these cells (job #2 — a business fact, the deploy set is provisional until known); (2) wire `size_legs` into the per-cell **emit path** (a sizing analogue of `odcore/quiet_registry.py`: per-cell alpha/roll + deploy flag) once a maker≤0 venue is confirmed; (3) keep watching the auto-accruing ledger (3rd/4th window). DEAD: timing retiming, wrong-tail entry-gates, spread/dive as timing.
