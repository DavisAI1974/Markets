...
- **STATE 2026-09-21 16:2xZ (chat 5 CLOSED, Greg: "We have to start new chat"): READ FIRST the 16:2xZ block atop
  ...
  version 2) pushes. Greg at close: chat 5's account of the KEYS is not accurate; the key question is deferred to the
  next session; ask him before acting on any key statement. First probe: `ACTION=status`.**
- **STATE 2026-09-21 13:5xZ (chat 5): THE ROOT READ IS RUNNING ON THE RENDER (Greg: "get the root read going";
  ...
  no key rotation for now. 16:1xZ: the git PAT is in SSM (written with the `Claude` IAM key Greg pasted; both values
  are in the chat by his choice) and the box heartbeat PUSHES with it (`root/cycle-00-progress` 4a2044785,
  15:35:54Z): the git chain is closed. Open calls: the three trunk registrations, RUNPOD_API_KEY in the
  environment. Box probes PAUSED.**
...
- **STATE 2026-09-21 08:00Z (superseded above): cycle 0's MACHINE HALF DONE on retained Pod g7y3g2w1kor4l3; the run HOLDS for Frankie's
  ...
  of the next chat = the harness on it.** Branch `claude/cycle-0-full-rerun-lr6e14`. Read
  `research/kalshi/frankie_boss/DROP_IN_CLAUDE_20260921.md` READ FIRST (to-do items 0-7), then `CLAUDE_HANDOFF_20260920.md`
  22:55Z onward. Native host is r7i.4xlarge (16 vCPU), not 8xlarge; the 8xlarge is the ingest runner.
