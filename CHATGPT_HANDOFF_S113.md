...
**6 tasks.** They are independent - run one per conversation.
...
## TASK 2 - Seasonal station weights + the Ohio/Baltimore holes

*Registry item M-5 (size M).*
...
## TASK 3 - ISO day-ahead and 7-day wind + solar -> NET LOAD (we serve the LOAD half already; the renewable half is entirely absent)

*Registry item G-4 (size M).*
...
## TASK 4 - THE WEATHER STATION SET IS 16 HAND-SET METROS AND ONE OF THEM COVERS THE ENTIRE SOUTHEAST - and Greg says the metros no longer sit where the load is
...
## TASK 5 - TEST GREG'S HYDRO CARRY - does TVA's curtailed state predict SOCO/SCEG/DUK? If it does, TVA's FORWARD water becomes a forward signal for three BAs that publish none
...
## TASK 6 - TRIAGE THE 1,129 UNREAD DATA POINTS - find the ones that should be read and are not
