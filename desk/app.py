"""HTMX operations desk for the unattended NG collector and SHADOW refiner.

The desk is read-only. It shows the underlying CME/NYMEX feed and accepts coach
messages only after the exact causal source gate has validated them. CME event
contracts remain SHADOW and execution remains disabled.
"""
...
from desk.ng_coach_panel import router as coach_router
...
def status_class(payload: dict[str, Any]) -> tuple[str, str]:
    ...
    stale = False
    try:
        stale = float(health_age) > 20 or (
            record_age is not None and float(record_age) > 30_000
        )
    except (TypeError, ValueError):
        pass
...
def ng_top_stack() -> str:
    ...
    total_records = sum(
        int(value) for value in counts.values() if isinstance(value, (int, float))
    )
    return f"""
<section class="stack-card">
  ...
  <div class="lower-grid">
    ...
    <div class="book-panel signal-panel">
      <div class="panel-title"><span>SIGNAL AUTHORITY</span><b>single causal path</b></div>
      <div class="call"><span>Underlying telemetry</span><strong>SHADOW ONLY</strong></div>
      <p>The exact-source coach is displayed in the separate read-only card below. Missing data, incomplete MBO state, and unmeasured product lag remain visible stand-downs. No dashboard component may calculate a second posterior.</p>
    </div>
  </div>
