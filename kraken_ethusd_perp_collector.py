"""
kraken_ethusd_perp_collector.py

ETH-USD *perpetual futures* (Kraken Futures, USD-margined inverse) WS
...
Data source: Kraken Futures public WS, no auth, no API key.
  futures.kraken.com/ws/v1
    - trade  feed: PI_ETHUSD  -> taker side + qty + price (n_trades)
    - ticker feed: PI_ETHUSD  -> bid/ask for mid

Symbol note: PI_ETHUSD is the inverse-USD perpetual (the longstanding
flagship). Kraken also runs PF_ETHUSD (linear, quote-margined). PI_ is
historically the more liquid contract; switch to PF_ if Kraken's
liquidity migrates.

Run: python kraken_ethusd_perp_collector.py --duration 14400 \\
        --bins-path eth_kraken_perp_bins.json
"""
...
def _load_existing_bins(path: str) -> dict[float, dict]:
    ...
    try:
        ...
        print(f"[kr-eth-perp] could not load existing bins: {e}", flush=True)
        return {}
...
async def collect(duration_s: float, save_path: str) -> dict[float, dict]:
    ...
    print(f"[kr-eth-perp] starting {duration_s:.0f}s on {PRODUCT}"
        ...
        try:
            async with websockets.connect(KRAKEN_FUTURES_WS, ping_interval=20, close_timeout=5) as ws:
                ...
                if reconnect_count > 0:
                    print(f"[kr-eth-perp] reconnected ({reconnect_count})", flush=True)

                while time.time() - t0 < duration_s:
                    try:
                        raw = await asyncio.wait_for(ws.recv(), timeout=30.0)
                    except asyncio.TimeoutError:
                        print(f"[kr-eth-perp] recv timeout t={time.time()-t0:.0f}s", flush=True)
                        continue
                    ...
                    if now - last_save >= 30.0:
                        ...
                        print(f"[kr-eth-perp] t={now-t0:.0f}s bins={len(bins)} trades={n_trades}",
                              flush=True)
            ...
            reconnect_count += 1
            print(f"[kr-eth-perp] WS issue ({type(e).__name__}: {e}); retry 5s (count={reconnect_count})",
                  flush=True)
            _save_bins(bins, save_path)
            try:
    ...
    _save_bins(bins, save_path)
    print(f"[kr-eth-perp] done. bins={len(bins)} reconnects={reconnect_count}", flush=True)
    return bins
...
def main():
    ...
    p.add_argument("--bins-path", type=str, default="eth_kraken_perp_bins.json")
    args = p.parse_args()
