...
Out: _alt_labels/coeffs/alt_coeff_index.json.gz  (same schema as fingerprint_dataset coeff_index;
     lineage="onset_1s") + per-cell counts to stdout.

Usage:  python _run_alt_coeffs.py [--cap 100] [--cells doge_bybit_perp_buy,...]
"""
...
def coef_for_window(lr: list[float]) -> list[float] | None:
    """The cs100_v2 deterministic pipeline on one pre-entry window's log-returns -> 128-dim coef."""
    ...
    for rank, (idx, _) in enumerate(hits):
        if rank < EXPAND_BUDGET:
            ...
            prefill.append(embeds[idx]); mask.append(1)
    dec = OperatorDecoder()
...
def run_cell(cell: str, cap: int) -> dict:
    ...
    for w in winners:
        ...
        coef = coef_for_window(lr)
        if coef is None:
            n_skip += 1; continue
        out[w["source_id"]] = {"coef": coef, "label": "win", "cell": cell,
...
def main() -> int:
    ...
    index = {}
    for cell in cells:
        index.update(run_cell(cell, args.cap))
    ...
    with gzip.open(out_fp, "wt", encoding="utf-8") as f:
        json.dump(blob, f)
    print(f"\nwrote {len(index)} alt coeffs -> {out_fp}", flush=True)
    return 0
