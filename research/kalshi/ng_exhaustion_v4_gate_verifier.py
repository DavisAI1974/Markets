...
from research.kalshi.ng_exhaustion_v4_mechanics import (
    ExecutionHandoff,FirstLock,PredecessorLifecycle,ProbabilityEntry,StateMovieRow,V4ContractError,
    recompute_first_lock,validate_execution_binding,validate_probability_movie,validate_state_movie,
)
...
class DetectorIntensityResolution:
    ...
    def validate(self):
        if self.mode=="OMITTED":
            ...
            if not self.source_sha256 or self.proxy_namespace: raise GateVerificationError("PROVEN_NATIVE requires source only")
...
def verify_prelaunch_gates(*,
    ...
    gates={}
    discovery.validate()
    validate_availability_chain(discovery,feature_available_at=feature_available_at,model_evaluated_at=model_evaluated_at,decision_available_at=decision_available_at)
    gates["1_causal_exhaustion_discovery_clock"]=True
    # Gate 2: source event/receive/feature clocks live in fields; model/decision clocks live above and ledger.
    ...
    recomputed=recompute_first_lock(probability_entries,threshold=lock_threshold,persistence=lock_persistence,lock_policy_sha256=lock_policy_sha256)
    ...
    validate_execution_binding(handoff,probability_entry=probability_entries[-1] if claimed_first_lock.entry_hash==probability_entries[-1].entry_hash else next(e for e in probability_entries if e.entry_hash==claimed_first_lock.entry_hash),first_lock=claimed_first_lock,state_movie_hash=movie_hash)
    gates["8_sealed_prediction_to_execution_handoff"]=True
    sparse_policy.validate(); gates["9_sparse_stage_scope_preservation"]=True
    core={
        "schema":SCHEMA,"gates":gates,"state_movie_hash":movie_hash,"probability_movie_hash":prob_hash,
        "first_lock_hash":claimed_first_lock.lock_hash,"execution_handoff_hash":handoff.handoff_hash,
        "intensity_mode":intensity.mode,
        "mechanical_prelaunch_contracts_pass":all(gates.values()),
        "trusted_v4_empirical_launch_authorized":False,
        "promotion_authorized":False,
        "p0_six_real_receipts_satisfied_by_this_receipt":False,
    }
