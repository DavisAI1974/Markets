#!/usr/bin/env python3
"""Bind branch-guarded execution to plan-bound target derivation readiness v19."""
...
import ng_historical_refinement_preflight_v14 as v14
import ng_historical_refinement_readiness_v19 as readiness

SCHEMA = "ng_historical_refinement_preflight.v15"
EXECUTOR_CONTRACT = "ng_historical_refinement_executor_v15"
...
class HistoricalRefinementPreflightV15Error(RuntimeError):
    pass


@contextmanager
def _v19_context() -> Iterator[None]:
    saved = (
        v14.executor,
        v14.readiness,
        v14.EXECUTOR_CONTRACT,
        v14.READINESS_CONTRACT,
        v14.STAGE_CONTRACT,
        v14.STAGE_ORDER,
        v14.STAGE_CONTRACT_FINGERPRINT,
    )
    v14.executor = executor
    v14.readiness = readiness
    v14.EXECUTOR_CONTRACT = EXECUTOR_CONTRACT
    v14.READINESS_CONTRACT = READINESS_CONTRACT
    v14.STAGE_CONTRACT = STAGE_CONTRACT
    v14.STAGE_ORDER = STAGE_ORDER
    v14.STAGE_CONTRACT_FINGERPRINT = STAGE_CONTRACT_FINGERPRINT
    try:
        yield
    finally:
        (
...
def _base_receipt(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    prior = base.pop("v14_preflight_fingerprint", None)
    for field in (
        "execution_plan_v19_validated",
        "target_slice_derivation_required",
        "target_slice_derivation_pre_outcome",
        "target_derivation_rebuilds_from_definition_bound_broad_bytes",
        "target_derivation_binds_target_inspection_plan",
        "basis_regeneration_blocked_until_target_derivation",
        "basis_target_plan_substitution_rejected",
        "metadata_only_target_route_rejected",
    ):
        base.pop(field, None)
    base.pop("fingerprint", None)
    base["schema"] = v14.SCHEMA
...
def _check_plan(plan: Mapping[str, Any]) -> None:
    ...
    if keys != STAGE_ORDER:
        raise HistoricalRefinementPreflightV15Error(
            "embedded plan does not use readiness-v19 stage order"
        )
    required = (
        "corpus_coverage",
    ...
    try:
        ...
        raise HistoricalRefinementPreflightV15Error(
            "required plan-bound target derivation stage is missing"
        ) from error
    if not (
        ...
        raise HistoricalRefinementPreflightV15Error(
            "plan-bound target derivation is not ordered before basis and G15 replay"
        )
    ...
    for key in (
        ...
        if by_key[key].get("requires_fixed_outcomes") is not False:
            raise HistoricalRefinementPreflightV15Error(f"{key} must remain pre-outcome")


def _finalize(plan: Mapping[str, Any], base: Mapping[str, Any]) -> dict[str, Any]:
    _check_plan(plan)
    with _v19_context():
        v14.validate_receipt(base)
    result = copy.deepcopy(dict(base))
    result["schema"] = SCHEMA
    result["v14_preflight_fingerprint"] = base["fingerprint"]
    ...
    result["execution_plan_v19_validated"] = True
    result["target_slice_derivation_required"] = True
    result["target_slice_derivation_pre_outcome"] = True
    result["target_derivation_rebuilds_from_definition_bound_broad_bytes"] = True
    result["target_derivation_binds_target_inspection_plan"] = True
    result["basis_regeneration_blocked_until_target_derivation"] = True
    result["basis_target_plan_substitution_rejected"] = True
    result["metadata_only_target_route_rejected"] = True
...
def validate_receipt(receipt: Mapping[str, Any]) -> None:
    ...
    if value.get("schema") != SCHEMA or observed != legacy._fingerprint(value):
        raise HistoricalRefinementPreflightV15Error(
            "preflight v15 schema or fingerprint mismatch"
        )
    if value.get("executor_contract") != EXECUTOR_CONTRACT:
        raise HistoricalRefinementPreflightV15Error("executor contract mismatch")
    if value.get("readiness_contract") != READINESS_CONTRACT:
        raise HistoricalRefinementPreflightV15Error("readiness contract mismatch")
    if value.get("readiness_stage_contract") != STAGE_CONTRACT:
        raise HistoricalRefinementPreflightV15Error("stage contract mismatch")
    if value.get("readiness_stage_contract_fingerprint") != STAGE_CONTRACT_FINGERPRINT:
        raise HistoricalRefinementPreflightV15Error("stage fingerprint mismatch")
    for field in (
        "execution_plan_v19_validated",
        "target_slice_derivation_required",
        "target_slice_derivation_pre_outcome",
    ...
    if not isinstance(plan, Mapping):
        raise HistoricalRefinementPreflightV15Error("exact plan snapshot is required")
    _check_plan(plan)
    if value.get("plan_fingerprint") != plan.get("fingerprint"):
        raise HistoricalRefinementPreflightV15Error("plan fingerprint mismatch")
    base = _base_receipt(receipt)
    if base.get("fingerprint") != value.get("v14_preflight_fingerprint"):
        raise HistoricalRefinementPreflightV15Error(
            "embedded v14 preflight fingerprint mismatch"
        )
    with _v19_context():
        v14.validate_receipt(base)
