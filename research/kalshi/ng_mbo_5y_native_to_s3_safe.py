...
The consolidator fences missing intervals by writing reservation receipts before
it submits the canonical Databento job. A native-pull lane must never interpret
such a reservation as a finished job receipt or submit a duplicate. It waits for
the reservation to be promoted to a receipt containing job_id, then resumes the
...
def migrate_legacy_receipts() -> dict:
    ...
    def putj(k, v):
        s3.put_object(
            Bucket=bucket,
            Key=k,
            Body=(json.dumps(v, indent=2, sort_keys=True) + "\n").encode(),
            ContentType="application/json",
        )

    result = {"aliased_jobs": [], "aliased_completed": [], "collisions": []}
    for a, b in _segments(start, end):
        exact = f"{a:%Y%m%d}_{b:%Y%m%d}"
        legacy = f"{a:%Y-%m}"
        ej = f"{prefix}/_jobs/{exact}.json"
        lj = f"{prefix}/_jobs/{legacy}.json"
        ed = f"{prefix}/_done/{exact}.done"
        ld = f"{prefix}/_done/{legacy}.done"
        em = f"{prefix}/manifests/{exact}.json"
        lm = f"{prefix}/manifests/{legacy}.json"
                ...
                result["collisions"].append(
                    {
                        ...
                        "exact_job_id": exact_job.get("job_id"),
                    }
                )
                ...
                s3.put_object(
                    ...
                    Body=(
                        json.dumps(
                            {
                                "segment": exact,
                                "job_id": legacy_manifest.get("job_id"),
                                "manifest": em,
                                "legacy_alias_from": ld,
                            }
                        )
                        + "\n"
                    ).encode(),
...
def wait_for_canonical_jobs(timeout_seconds: int = 1800) -> None:
    """Wait for our fenced receipts to gain job_id; never submit around them."""
    bucket = _required("BUCKET")
    prefix = _required("PREFIX").strip("/")
    start = dt.date.fromisoformat(_required("START"))
    end = dt.date.fromisoformat(_required("END"))
    ...
    while True:
        pending = []
        for a, b in _segments(start, end):
            segment = f"{a:%Y%m%d}_{b:%Y%m%d}"
            key = f"{prefix}/_jobs/{segment}.json"
            try:
                row = json.loads(s3.get_object(Bucket=bucket, Key=key)["Body"].read())
            except Exception as exc:
                r = getattr(exc, "response", {})
                code = r.get("Error", {}).get("Code")
                status = r.get("ResponseMetadata", {}).get("HTTPStatusCode")
                if str(code) in {"404", "NoSuchKey", "NotFound"} or status == 404:
                    continue
                raise
            if row.get("reservation_owner") and not row.get("job_id"):
                pending.append(segment)

        if not pending:
            print("[reservation-guard] all fenced receipts have canonical job_id", flush=True)
            return
        if time.time() >= deadline:
            raise RuntimeError(
                "canonical job reservations still pending after guard timeout: " + ",".join(pending[:8])
            )
        print(
            f"[reservation-guard] waiting on {len(pending)} canonical job receipts; first={pending[0]}",
            flush=True,
        )
        time.sleep(20)
