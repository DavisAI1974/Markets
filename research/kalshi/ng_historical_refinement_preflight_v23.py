#!/usr/bin/env python3
"""Bind branch-guarded execution to runtime-observed historical readiness v27."""
...
SCHEMA = "ng_historical_refinement_preflight.v23"
EXECUTOR_CONTRACT = "ng_historical_refinement_executor_v23"
...
class HistoricalRefinementPreflightV23Error(RuntimeError):
    pass
...
def _check_plan(plan: Mapping[str, Any]) -> None:
    ...
    if keys != STAGE_ORDER:
        raise HistoricalRefinementPreflightV23Error(
            "embedded plan does not use readiness-v27 stage order"
        )
    required = (
    ...
    try:
        ...
        raise HistoricalRefinementPreflightV23Error(
            "required runtime-observed corpus stage is missing"
        ) from error
    if [positions[key] for key in required] != sorted(positions[key] for key in required):
        raise HistoricalRefinementPreflightV23Error(
            "runtime-observed inventory capture is not ordered before materialization and G15 replay"
        )
    ...
    for key in required[:-1]:
        if by_key[key].get("requires_fixed_outcomes") is not False:
            raise HistoricalRefinementPreflightV23Error(f"{key} must remain pre-outcome")
...
def _base_receipt(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for field in (
        ...
        "execution_plan_v27_validated",
        "runtime_observed_inventory_capture_required",
        "runtime_observed_inventory_capture_pre_outcome",
        "runtime_capture_follows_product_finalization",
        "runtime_capture_precedes_materialization",
        "runtime_capture_bound_to_finalization_contract_v27",
        "runtime_capture_bound_to_paginated_inventory_v27",
        "runtime_capture_bound_to_materialization_spec_v27",
        "declared_observations_bound_to_runtime_interval_v27",
        "legacy_paginated_inventory_route_rejected_v27",
    ):
        base.pop(field, None)
...
def _finalize(plan: Mapping[str, Any], base: Mapping[str, Any]) -> dict[str, Any]:
    ...
    result["execution_plan_v27_validated"] = True
    result["runtime_observed_inventory_capture_required"] = True
    result["runtime_observed_inventory_capture_pre_outcome"] = True
    result["runtime_capture_follows_product_finalization"] = True
    result["runtime_capture_precedes_materialization"] = True
    result["runtime_capture_bound_to_finalization_contract_v27"] = True
    result["runtime_capture_bound_to_paginated_inventory_v27"] = True
    result["runtime_capture_bound_to_materialization_spec_v27"] = True
    result["declared_observations_bound_to_runtime_interval_v27"] = True
    result["legacy_paginated_inventory_route_rejected_v27"] = True
...
def validate_receipt(receipt: Mapping[str, Any]) -> None:
    ...
    if value.get("schema") != SCHEMA or observed != legacy._fingerprint(value):
        raise HistoricalRefinementPreflightV23Error(
            "preflight v23 schema or fingerprint mismatch"
        )
    if value.get("executor_contract") != EXECUTOR_CONTRACT:
        raise HistoricalRefinementPreflightV23Error("executor contract mismatch")
    if value.get("readiness_contract") != READINESS_CONTRACT:
        raise HistoricalRefinementPreflightV23Error("readiness contract mismatch")
    if value.get("readiness_stage_contract") != STAGE_CONTRACT:
        raise HistoricalRefinementPreflightV23Error("stage contract mismatch")
    if value.get("readiness_stage_contract_fingerprint") != STAGE_CONTRACT_FINGERPRINT:
        raise HistoricalRefinementPreflightV23Error("stage fingerprint mismatch")
    for field in (
        "execution_plan_v27_validated",
        "runtime_observed_inventory_capture_required",
        "runtime_observed_inventory_capture_pre_outcome",
    ...
    if not isinstance(plan, Mapping):
        raise HistoricalRefinementPreflightV23Error("exact plan snapshot is required")
    _check_plan(plan)
    if value.get("plan_fingerprint") != plan.get("fingerprint"):
        raise HistoricalRefinementPreflightV23Error("plan fingerprint mismatch")
