...
def emit(
    ...
    add("total zero (43,569 and 65,960 before the wiring). Report that verdict and both totals in")
    add("your findings artifact exactly as your own stream wrote them; a FAIL is a finding, not a")
    add("caveat, and a run that does not report F-20 did not consume the stream it claims to have.")
    add("")
    ...
    add("  (VERIFIED / NOT_TESTED_ON_THIS_SLICE / REFUTED against the stream - a lesson this")
    add("  slice did not exercise is not doubted, S124: NEW means recent, not unverified), citing the knowledge")
    add("  receipt;")
    add(f"- `{outputs.WHAT_HE_LEARNED_LEDGER}` - WHAT YOU LEARNED on this run, one entry per")
    add("  learning: `learning_id`, `statement`, `kind` (NEW / CONFIRMED / REVISED / REFUTED")
    add("  against the memory you were served, with `prior_lesson_id` unless NEW), the")
    add("  contract `sections` it came from, and `evidence` (exact `member_group_indices` with")
    add("  their `cutoff_recv_ns`, or a stated `basis` when it rests on absence);")
    add(f"- `{outputs.IN_HIS_OWN_WORDS_LEDGER}` - YOUR OWN WORDS about it, in depth, by `topic`:")
    add("  FINDINGS (what you found and what it means), BUILD (your opinion of the build you")
    add("  ran under), DATA (your opinion of the data you were fed), SUGGESTION (what should")
    add("  change); every topic at least once, each `statement` with `refers_to` naming the")
    add("  ledgers it is about. These two ledgers are printed as two documents beside your")
    add("  artifact and carried into your memory; a run without them taught nothing.")
    add("")
