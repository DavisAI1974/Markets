#!/usr/bin/env python3
"""Compile the lineage-bound dual-inspection corpus plan against readiness v17."""
...
import ng_corpus_executor_plan_compiler_v12 as v12
import ng_historical_refinement_executor_v14 as executor
import ng_historical_refinement_readiness_v17 as readiness

SCHEMA = "ng_corpus_executor_plan_compiler.v13"
STATUS = "LINEAGE_BOUND_DUAL_INSPECTION_CORPUS_EXECUTOR_PLAN_COMPILED"
CONFIGURED_STAGES = (
    "corpus_coverage",
...
class CorpusExecutorPlanCompilerV13Error(ValueError):
    pass
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CorpusExecutorPlanCompilerV13Error(f"cannot read JSON {path}: {error}") from error
    if not isinstance(value, dict):
        raise CorpusExecutorPlanCompilerV13Error(f"JSON artifact must be an object: {path}")
    return value
...
def _authority(value: Mapping[str, Any], *, label: str) -> None:
    for field in (
        ...
        if value.get(field) is not False:
            raise CorpusExecutorPlanCompilerV13Error(f"{label}: {field} must remain false")
    if value.get("one_signal_authority_preserved") is not True:
        raise CorpusExecutorPlanCompilerV13Error(f"{label}: one signal authority must be preserved")
    if value.get("blind_forecasts_immutable") is not True:
        raise CorpusExecutorPlanCompilerV13Error(f"{label}: blind forecasts must remain immutable")
    if value.get("cme_event_contracts_mode") != "SHADOW":
        raise CorpusExecutorPlanCompilerV13Error(f"{label}: CME event contracts must remain SHADOW")
    if value.get("brokerage_contract") != "tastytrade_not_ibkr":
        raise CorpusExecutorPlanCompilerV13Error(f"{label}: brokerage must remain tastytrade, not IBKR")
...
def _commands(
    *, artifact_dir: Path, inventory_receipt_path: Path,
    ...
    commands["target_slice_broad_lineage"] = [
        "python", "ng_target_slice_broad_lineage_gate.py", "build",
        "--broad-definition-byte-gate", str(root / "ng_corpus_definition_byte_binding_gate.json"),
        "--target-slice-bundle", str(slice_bundle_path.resolve(strict=False)),
        "--target-inspection-receipt", str(root / "ng_target_slice_inspection_receipt.json"),
        "--out", str(root / "ng_target_slice_broad_lineage_gate.json"),
    ]
...
def _validate_plan(
    ...
    if keys != expected or keys[: len(CONFIGURED_STAGES)] != list(CONFIGURED_STAGES):
        raise CorpusExecutorPlanCompilerV13Error(
            "execution plan is not the exact readiness-v17 lineage-bound contract"
        )
    if len(keys) != len(set(keys)):
        raise CorpusExecutorPlanCompilerV13Error("duplicate execution-plan stage keys")
    ...
    if not (
        ...
        raise CorpusExecutorPlanCompilerV13Error(
            "target-to-broad lineage is not between target inspection and basis regeneration"
        )
    rows = {str(row.get("key")): row for row in rows_list}
    for index, key in enumerate(CONFIGURED_STAGES):
        row = rows[key]
        if row.get("argv") != commands[key]:
            raise CorpusExecutorPlanCompilerV13Error(f"{key}: command vector mismatch")
        expected_enabled = index == 0 if compiled else True
        if row.get("enabled") is not expected_enabled:
            raise CorpusExecutorPlanCompilerV13Error(f"{key}: enabled-state mismatch")
        if row.get("requires_fixed_outcomes") is not False:
            raise CorpusExecutorPlanCompilerV13Error(f"{key}: corpus stage must remain pre-outcome")
...
def build_compiled_plan(
    *, artifact_dir: Path, working_directory: Path,
    ...
    receipt: dict[str, Any] = {
        ...
        "target_slice_broad_lineage_stage_present": True,
        "target_slice_broad_lineage_stage_enabled": False,
        "target_slice_broad_lineage_stage_pre_outcome": True,
        "basis_regeneration_blocked_until_target_lineage": True,
        "unrelated_target_slice_route_rejected": True,
        "target_slice_broad_lineage_output": str(
            root / "ng_target_slice_broad_lineage_gate.json"
        ),
...
def validate_receipt(
    ...
    if checked.get("schema") != SCHEMA or observed != fingerprinting._fp(checked):
        raise CorpusExecutorPlanCompilerV13Error(
            "compiler v13 receipt schema or fingerprint mismatch"
        )
    checked["fingerprint"] = observed
    _authority(checked, label="compiler v13 receipt")
    ...
    for field, item in expected.items():
        if checked.get(field) != item:
            raise CorpusExecutorPlanCompilerV13Error(
                f"compiler v13 field mismatch: {field}"
            )
    if rows["target_slice_broad_lineage"].get("enabled") is not False:
        raise CorpusExecutorPlanCompilerV13Error(
            "compiled plan may not pre-enable target-to-broad lineage"
        )
