...
import ng_corpus_executor_pipeline_arm_v4 as arm
...
import ng_historical_refinement_preflight_v5 as preflight
import ng_historical_refinement_readiness_v7 as readiness


class HistoricalRefinementExecutionV7Tests(unittest.TestCase):
    def _root(self) -> Path:
    ...
    def _base_preflight(
        ...
        plan: dict,
        *,
        stage: str = "broad_corpus_exact_partition",
    ) -> dict:
        value = {
    ...
    def test_executor_uses_readiness_v7_and_partition_entrypoint(self) -> None:
        self.assertEqual(
            readiness.SCHEMA,
            "ng_historical_refinement_readiness.v7",
        )
        self.assertEqual(
        ...
        self.assertLess(
            order.index("broad_corpus_exact_overlap"),
    ...
    def test_executor_plan_contains_partition_before_g15(self) -> None:
        root = self._root()
    ...
    def test_readiness_v6_plan_is_rejected(self) -> None:
        root = self._root()
        plan = executor.build_plan(
            root / "artifacts",
            root / "work",
        ...
        plan["fingerprint"] = executor.legacy_executor._fingerprint(
            plan
        )
        with self.assertRaises(Exception):
            executor.validate_plan(plan)

    def test_compiler_configures_six_pre_outcome_stages(self) -> None:
        plan, receipt, commands = self._compiled()
        self.assertEqual(
            receipt["configured_stages"],
        ...
        self.assertEqual(
            rows["broad_corpus_exact_partition"]["argv"],
            commands["broad_corpus_exact_partition"],
        )

    def test_partition_command_is_bound_to_overlap_output(self) -> None:
        ...
        argv = row["argv"]
        self.assertIn("--exact-overlap-gate", argv)
        self.assertIn(
    ...
    def test_compiler_rejects_partition_command_substitution(self) -> None:
        plan, receipt, commands = self._compiled()
        ...
        tampered["fingerprint"] = (
            executor.legacy_executor._fingerprint(tampered)
        )
        with self.assertRaises(Exception):
    ...
    def test_compiler_rejects_partition_requirement_removal(self) -> None:
        plan, receipt, commands = self._compiled()
        changed = copy.deepcopy(receipt)
        changed["broad_corpus_exact_partition_gate_required"] = False
        changed.pop("fingerprint")
        changed["fingerprint"] = fingerprinting._fp(changed)
        with self.assertRaises(
            compiler.CorpusExecutorPlanCompilerV4Error
        ):
            compiler.validate_receipt(
                changed,
                plan=plan,
    ...
    def test_arm_enables_only_six_corpus_stages(self) -> None:
        ...
        self.assertEqual(
            arm_receipt["stage_order_prefix"],
            list(arm.ARMED_STAGES),
        )
        self.assertTrue(
            arm_receipt[
                "broad_exact_partition_gate_armed_before_g15"
            ]
        )

    def test_arm_preserves_stable_deterministic_plan(self) -> None:
        plan, receipt, _ = self._compiled()
        first_plan, first_receipt = arm.build_armed_plan(
            plan,
            receipt,
        )
        second_plan, second_receipt = arm.build_armed_plan(
            plan,
            receipt,
        )
    ...
    def test_arm_rejects_refingerprinted_options_escalation(self) -> None:
        ...
        changed["fingerprint"] = fingerprinting._fp(changed)
        with self.assertRaises(
            arm.CorpusExecutorPipelineArmV4Error
        ):
    ...
    def test_preflight_contract_is_readiness_v7(self) -> None:
        self.assertEqual(
            preflight.READINESS_CONTRACT,
            readiness.SCHEMA,
        )
        self.assertEqual(
            preflight.EXECUTOR_CONTRACT,
            "ng_historical_refinement_executor_v5",
        )
        self.assertLess(
            preflight.STAGE_ORDER.index(
    ...
    def test_preflight_embeds_and_validates_partition_plan(self) -> None:
        root = self._root()
        plan = executor.build_plan(
            root / "artifacts",
            root / "work",
        ...
        self.assertTrue(result["execution_plan_v7_validated"])
        self.assertTrue(result["broad_corpus_scope_required"])
        self.assertTrue(
            result["broad_corpus_exact_overlap_required"]
        )
        self.assertTrue(
            result["broad_corpus_exact_partition_required"]
        )
        self.assertEqual(result["execution_plan_snapshot"], plan)

    def test_preflight_rejects_partition_requirement_removal(self) -> None:
        root = self._root()
        plan = executor.build_plan(
            root / "artifacts",
            root / "work",
        ...
        with mock.patch.object(
            ...
            result = preflight._finalize(plan, base)
        result["broad_corpus_exact_partition_required"] = False
        result.pop("fingerprint")
        result["fingerprint"] = legacy_preflight._fingerprint(
            result
        )
        with self.assertRaises(
            preflight.HistoricalRefinementPreflightV5Error
        ):
    ...
    def test_permanent_authority_contract(self) -> None:
        ...
        self.assertEqual(
            receipt["cme_event_contracts_mode"],
            "SHADOW",
        )
        self.assertEqual(
            receipt["brokerage_contract"],
            "tastytrade_not_ibkr",
        )
        self.assertFalse(receipt["options_lane_started"])
        self.assertEqual(
            arm_receipt["armed_plan_fingerprint"],
            armed["fingerprint"],
        )
