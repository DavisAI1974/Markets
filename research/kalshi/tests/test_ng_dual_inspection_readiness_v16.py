...
class DualInspectionReadinessV16Tests(unittest.TestCase):
    ...
    def _minimal_plan(self, *, compiled: bool = True) -> dict:
        ...
        for spec in readiness.STAGES:
            index = compiler.CONFIGURED_STAGES.index(spec.key) if spec.key in compiler.CONFIGURED_STAGES else None
            rows.append(
                {
                    "key": spec.key,
    ...
    def test_readiness_inserts_target_slice_inspection_before_basis(self) -> None:
        ...
        self.assertLess(keys.index("corpus_coverage"), keys.index("corpus_definition_byte_binding"))
        self.assertLess(keys.index("corpus_definition_byte_binding"), keys.index("target_slice_coverage"))
        self.assertLess(keys.index("target_slice_coverage"), keys.index("basis_inventory_regeneration"))
        self.assertLess(keys.index("basis_inventory_regeneration"), keys.index("broad_corpus_scope"))

    def test_target_slice_stage_contract_is_canonical(self) -> None:
        spec = next(spec for spec in readiness.STAGES if spec.key == "target_slice_coverage")
    ...
    def test_compiler_routes_artifacts_to_correct_receipts(self) -> None:
        commands = self._commands()
        broad = commands["corpus_coverage"]
        target = commands["target_slice_coverage"]
        basis = commands["basis_inventory_regeneration"]
        replay = commands["replay_catalog_export"]
        scope = commands["broad_corpus_scope"]
    ...
    def test_compiler_rejects_target_receipt_used_for_broad_scope(self) -> None:
        commands = self._commands()
        commands["broad_corpus_scope"] = copy.deepcopy(commands["broad_corpus_scope"])
        commands["broad_corpus_scope"][3] = str(Path("renders/ng_refine_s95/ng_target_slice_inspection_receipt.json").resolve())
        plan = self._minimal_plan(compiled=True)
        rows = {row["key"]: row for row in plan["stages"]}
        rows["broad_corpus_scope"]["argv"] = commands["broad_corpus_scope"]
        with mock.patch.object(compiler.executor, "validate_plan", return_value=None):
            compiler._validate_plan(plan, commands, compiled=True)
        self.assertNotEqual(commands["broad_corpus_scope"], self._commands()["broad_corpus_scope"])

    def test_compiler_rejects_missing_target_slice_stage(self) -> None:
        plan = self._minimal_plan(compiled=True)
        plan["stages"] = [row for row in plan["stages"] if row["key"] != "target_slice_coverage"]
        with mock.patch.object(compiler.executor, "validate_plan", return_value=None):
    ...
    def test_arm_enables_only_dual_inspection_corpus_prefix(self) -> None:
        ...
        with mock.patch.object(arm.executor, "validate_plan", return_value=None), mock.patch.object(
            arm.compiler, "_validate_plan", return_value={}
        ):
            rows = {row["key"]: row for row in plan["stages"]}
    ...
    def test_target_slice_linkage_is_bound_to_basis(self) -> None:
        ...
        self.assertEqual(target["receipt_fingerprint"], basis["inspection_receipt_fingerprint"])
        self.assertEqual(target["plan_fingerprint"], basis["inspection_plan_fingerprint"])
        self.assertEqual(target["catalog_fingerprint"], basis["coverage_catalog_fingerprint"])
        self.assertEqual(target["audit_fingerprint"], basis["coverage_audit_fingerprint"])
    ...
    def test_permanent_authority_wall(self) -> None:
        ...
        self.assertFalse(any("option" in key.lower() for key in executor.SUGGESTED_ENTRYPOINTS))
