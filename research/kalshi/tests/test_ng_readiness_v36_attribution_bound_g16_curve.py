...
def test_bound_curve_is_between_legacy_curve_and_lock() -> None:
    keys = _keys()
    index = keys.index("g16_counterfactual_curve_authorization")
    ...
    assert by_key["g16_attribution_bound_curve_authorization"].schema == gate.SCHEMA


def test_bound_curve_links_causal_curve_scores_and_lock() -> None:
...
def test_missing_bound_curve_blocks_lock(tmp_path: Path) -> None:
    ...
    for spec in readiness.STAGES:
        if spec.key == "g16_attribution_bound_curve_authorization":
            continue
    ...
    assert report["first_blocking_stage"] == "g16_attribution_bound_curve_authorization"
    assert report["status"] == (
        "G16_LEGACY_CURVE_READY_ATTRIBUTION_BOUND_CURVE_INCOMPLETE"
    )
    assert report["g16_curve_lock_opened_after_attribution_bound_curve"] is False


def test_link_substitution_blocks_bound_curve(tmp_path: Path) -> None:
    values = readiness._linked_fixture_chain()
    target = copy.deepcopy(values["g16_attribution_bound_curve_authorization"])
    target["attribution_bound_causal_authorization_fingerprint"] = "x" * 64
    target.pop("fingerprint", None)
    target["fingerprint"] = readiness._fingerprint(target)
    values["g16_attribution_bound_curve_authorization"] = target
    overrides = {spec.key: (lambda value: None) for spec in readiness.STAGES}
    for spec in readiness.STAGES:
        readiness._atomic_json(tmp_path / spec.filename, values[spec.key])
    report = readiness.build_readiness_report(tmp_path, validator_overrides=overrides)
    ...
    row = next(
        ...
        if item["key"] == "g16_attribution_bound_curve_authorization"
    )
    assert any("provenance link mismatch" in blocker for blocker in row["blockers"])


def test_lock_link_substitution_is_blocked(tmp_path: Path) -> None:
    values = readiness._linked_fixture_chain()
    lock = copy.deepcopy(values["g16_counterfactual_curve_lock"])
    lock["attribution_bound_curve_authorization_fingerprint"] = "x" * 64
    ...
    for spec in readiness.STAGES:
        readiness._atomic_json(tmp_path / spec.filename, values[spec.key])
    report = readiness.build_readiness_report(tmp_path, validator_overrides=overrides)
    assert report["first_blocking_stage"] == "g16_counterfactual_curve_lock"
    row = next(
        item
        for item in report["stages"]
...
def test_complete_fixture_preserves_g16_blind_wall(tmp_path: Path) -> None:
    ...
    assert bound["g16_curve_bound_to_attribution_scored_g15_lessons"] is True
    ...
    assert report["g16_outcomes_unavailable_during_curve_authorization"] is True


def test_executor_builds_fixed_g15_curve_binding_and_lock(tmp_path: Path) -> None:
    plan = executor.build_plan(tmp_path / "artifacts", tmp_path)
    executor.validate_plan(plan)
    rows = {row["key"]: row for row in plan["stages"]}
    assert rows["g16_counterfactual_curve_authorization"][
        "requires_fixed_outcomes"
    ] is True
    bound = rows["g16_attribution_bound_curve_authorization"]
    assert bound["expected_output"] == "g16_attribution_bound_curve_authorization.json"
...
def test_executor_rejects_removed_bound_curve(tmp_path: Path) -> None:
    plan = executor.build_plan(tmp_path / "artifacts", tmp_path)
    tampered = copy.deepcopy(plan)
    tampered["stages"] = [
        row
        for row in tampered["stages"]
        if row["key"] != "g16_attribution_bound_curve_authorization"
    ]
    _rehash(tampered)
...
def test_executor_rejects_pre_outcome_bound_curve(tmp_path: Path) -> None:
    plan = executor.build_plan(tmp_path / "artifacts", tmp_path)
    tampered = copy.deepcopy(plan)
    row = next(
        item
...
def test_executor_rejects_pre_outcome_legacy_curve(tmp_path: Path) -> None:
    plan = executor.build_plan(tmp_path / "artifacts", tmp_path)
    tampered = copy.deepcopy(plan)
    row = next(
        item
        for item in tampered["stages"]
        if item["key"] == "g16_counterfactual_curve_authorization"
    )
    row["requires_fixed_outcomes"] = False
    _rehash(tampered)
    with pytest.raises(Exception):
...
def test_executor_rejects_lock_that_hides_fixed_g15_outcome_lineage(
    tmp_path: Path,
) -> None:
    plan = executor.build_plan(tmp_path / "artifacts", tmp_path)
    tampered = copy.deepcopy(plan)
