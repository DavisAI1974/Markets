...
def test_watchdog_keeps_deadline_cleanup_after_storage_outage(monkeypatch, tmp_path):
    info = {'pod_id': host.lifecycle.POD_ID, 'intent': {}}
    lease = host.lifecycle.make_lease(info, start=1000., duration_seconds=600,
                                     request_sha256='a'*64)
    class Journal:
        def get(self, name):
            if name == 'retained-lease.json':
                return lease
    ...
    def stop(*args):
        stopped.append(clock[0])
        return dict(status='confirmed_stopped')
    monkeypatch.setattr(host.lifecycle, 'stop_owned_once', stop)
    host.watchdog(Journal(), object(), info)
    assert stopped == [1480.]
    assert (tmp_path/'lease.json').exists()
    assert (tmp_path/'watchdog-transient.json').exists()
