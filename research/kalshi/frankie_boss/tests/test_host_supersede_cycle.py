...
def test_moves_the_cycle_and_handoff_never_deletes_and_refuses_on_a_response_or_runner():
    ...
    assert "'session-response.json')) { throw 'refusing: a session-response.json is recorded" in CODE
    ...
    assert 'Get-ChildItem -LiteralPath $source -Recurse -File' in CODE and 'sha256 = (Digest $file.FullName)' in CODE
