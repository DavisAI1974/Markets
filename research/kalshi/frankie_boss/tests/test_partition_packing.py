...
old hardcoded 16 was the whole story (114,054 first-run entries / 16 = 7,129). PARTITION_ENTRIES
...
from journal_stack_execution import MigratingConformanceReader, PARTITION_ENTRIES   # noqa: E402
...
RECORDS = 60            # -> 120 entries, so 96 and 16 give different box counts on the same journal
...
def test_packing_changes_the_box_count_and_nothing_that_is_the_journal(tmp_path):
    packed, packed_entries, packed_blocks = run(tmp_path, 'packed', PARTITION_ENTRIES)
    ...
    assert packed_blocks[0] == -(-2 * RECORDS // PARTITION_ENTRIES)
    assert packed_blocks[0] < split_blocks[0]


def test_the_first_runs_box_count_under_this_value_is_about_twelve_hundred():
    entries = 114054                                  # the first run, 57,027 records x INPUT/APPLIED
    assert PARTITION_ENTRIES == 96
    assert -(-entries // 96) == 1189                  # Greg's "about 1,200 boxes"
    assert -(-entries // 16) == 7129                  # what the hardcoded 16 produced
