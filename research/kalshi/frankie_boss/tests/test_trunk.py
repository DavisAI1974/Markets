...
from trunk import GatedDeltaCell, TemporalGraphBranch, Trunk, TrunkConfig
...
def test_use_qsv_false_disables_branch_cleanly():
    c = cfg(use_qsv=False)
    m = Trunk(c).eval()
    b = batch(c)
    with torch.no_grad():
...
def test_trunk_contains_exactly_one_temporal_graph_branch():
    model = Trunk(cfg())

    assert sum(
        isinstance(module, TemporalGraphBranch) for module in model.modules()
    ) == 1
