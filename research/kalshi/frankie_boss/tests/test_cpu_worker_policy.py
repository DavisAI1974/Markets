...
from research.kalshi.frankie_boss.cpu_runtime import CpuRuntimePolicy, policy_from_environment
...
def test_parallel_reader_rejects_at_the_same_row_as_single_worker(tmp_path):
    """Corrupt one row deep inside a batch: both paths must raise at the same ordinal."""
    import sqlite3
    journal = EvidenceJournal(tmp_path / "journal.sqlite", create=True)
    ...
    raw.execute("UPDATE entries SET body = X'7b7d' WHERE ordinal = 37")
    raw.commit(); raw.close()
    seen = {}
    for workers in (1, 4):
        reader = VerifiedJournalReader(tmp_path / "journal.sqlite", expected_count=count,
