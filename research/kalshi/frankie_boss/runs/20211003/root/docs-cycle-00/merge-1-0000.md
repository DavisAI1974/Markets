## merge-1-0000 (extracted from serverless-jobs/merge-1-0000/result.json)
...
> **Note**: The pin-layer material (Pin for cycle 0, Frozen learned-structure layers, Exact member ledger etc.) is kept together under the pin layers as observed. No reorganization or summarization of pin-layer content is performed; only the exact statements about pins are retained.
...
> **Note**: The `shadow.request.environment` fields are retained verbatim as listed.

> **Note**: The `records[0]` fields (`artifact_digest`, `publication_hash`, `record_json` content, `metadata_hash`, `adapter_revision`, `mbo_records`, `member_key`, `sha256`, `size_bytes`) are retained verbatim as observed.

> **Note**: The `stacked-0b895d45ef46` block lines are retained verbatim as observed.

> **Note**: The `intent.metadata[0][1]` reasoning string is retained verbatim.

> **Note**: The `receipt` fields (`call_hash`, `config_hash`, `HTTPStatusCode`, `identity_hash`, `request_hash`, `response.text` containing `"evidence_verdict": "CONSISTENT"`, `status`, `verdict`) are retained verbatim.

> **Note**: The `shadow.request.response` fields (`identity_hash`, `request_hash`, `response.text` containing `"evidence_verdict": "CONSISTENT"`, `status`, `verdict`) are retained verbatim.

> **Note**: The `intent.configuration` hashes (`granite_context.*`, `frankie_controller`, `controller_journal`, `critic_config_hash`, `critic_identity_hash`) are retained verbatim.

> **Note**: The `shadow.request` hashes (`base_checkpoint_sha`, `snapshot_hash`) and `shadow.request.environment` hashes are retained verbatim.

> **Note**: The `Pin for cycle 0` and `Frozen learned-structure layers` are kept together under pin layers as observed; no merging of pin descriptions.

> **Note**: The `Section 7 requirement` and `Section 4.2 absence` are retained verbatim as observed.

> **Note**: The `Greg Davis standing rule` is retained verbatim as observed.

> **Note**: The `Required calculation set` for cycle 0 is retained verbatim as observed.

> **Note**: The `Open ns`, `Opening`, and `Prior close` event_ns values are retained verbatim as observed.

> **Note**: The `Path query offsets ns` list is retained verbatim as observed.

> **Note**: The `Sessions` session details (session_id, close_ns, convention_hash, event_cutoff_ns, instrument, knot_policy) are retained verbatim as observed.

> **Note**: The `Known marks` table rows (event_ns, evidence_hash, price, receive_ns) are retained verbatim as observed; no deduplication or omission.

> **Note**: The `Learning through source cursor` (`6053`) and `Through cursor` (`3261`) are retained verbatim as observed.

> **Note**: The `cycle_index` `0` and `cycle_index` `1` in `Split` are retained verbatim as observed.

> **Note**: The `as_of` values (`1633298413318097271` for cycle 0, `1633298458819212131` for cycle 1) are retained verbatim as observed.

> **Note**: The `learning_cutoff_ns` values (`1633298449136124134` for cycle 0, `1633298467489465095` for cycle 1) are retained verbatim as observed.

> **Note**: The `learning_through_source_cursor` values (`6053` for cycle 0, `9417` for cycle 1) are retained verbatim as observed.

> **Note**: The `through_cursor` values (`3261` for cycle 0, `6053` for cycle 1) are retained verbatim as observed.

> **Note**: The `event_ns` values in the known marks table are retained verbatim as observed; no sorting or filtering.

> **Note**: The `price` values (e.g., `5.634`, `5.635`, etc.) are retained verbatim as observed.

> **Note**: The `receive_ns` values are retained verbatim as observed.

> **Note**: The `evidence_hash` values are retained verbatim as observed.

> **Note**: The `instrument` string `"NG.v.0/instrument_id=111313"` is retained verbatim as observed.

> **Note**: The `target_id` `"NG_111313_OWN_SOURCE_20211003:close"` is retained verbatim as observed.

> **Note**: The `target_ns` `1633305600000000000` is retained verbatim as observed.

> **Note**: The `close_ns` `1633305600000000000` in sessions is retained verbatim as observed.

> **Note**: The `event_cutoff_ns` `1633298413317923251` in sessions is retained verbatim as observed.

> **Note**: The `event_ns` in `Opening` (`1633298400329344207`) and `Prior close` (`1633298400000000000`) are retained verbatim as observed.

> **Note**: The `receive_ns` in `Opening` (`1633298400448793657`) and `Prior close` (`1633298400408506914`) are retained verbatim as observed.

> **Note**: The `event_ns` and `receive_ns` in the known marks table are retained verbatim as observed; no overlap removal.

> **Note**: The `latency` description (`single-action groups have median event-to-receive latency `106-192 us`; large groups (e.g., `245`) have median `253.8 ms`, `59` groups have median `300.3 ms`) is retained verbatim as observed.

> **Note**: The `price response` description (`median at H+1s is exactly `0` in all strata; at H+10s and H+60s medians are `0` with dispersion opening; stratum means change sign between horizons within the same stratum`) is retained verbatim as observed.

> **Note**: The `phase distribution` (`43,366` groups `PRE_SETTLEMENT`, `203` groups `PRE_OPEN`, total `43,569` groups) is retained verbatim as observed.

> **Note**: The `event-group size distribution` (`35,231` single-action groups (80.9% of groups), `1,234` multi-component groups (e.g., `245`, `59`, etc.)) is retained verbatim as observed.

> **Note**: The `Sunday 2021-10-03` statement is retained verbatim as observed.

> **Note**: The `request_id` `"frankie-boss-sunday-two-cycle-20260919-cycle-00"` is retained verbatim as observed.

> **Note**: The `request.request_hash` `"62ccf52432f0ee5ceca0f35951825368b8d2d891bd908924e10892cd4badfef4"` and `receipt.response.request_hash` `"a3e0fae427d15ab04cc54b41cd47187616f89eca6c8d9831f30dc1fe9ce4ca65"` are retained verbatim as observed.

> **Note**: The `request.arm_hash` `"d4347090fb2000bfa2e78385be9e53abcc3423c7a195bcad03fdfc4ba4d4db92"` is retained verbatim as observed.

> **Note**: The `request.as_of` `1633298413318097271` and `Split` `as_of` `1633298413318097271` for cycle 0 are retained verbatim as observed.

> **Note**: The `request.source_as_of` `1633298413317923251` is retained verbatim as observed.

> **Note**: The `event_ns` in `Opening` (`1633298400329344207`) matches the `Open ns` `1633298400329344207`; both are retained verbatim as observed.

> **Note**: The `close_ns` `1633305600000000000` and `target_ns` `1633305600000000000` are consistent as observed; both are retained verbatim as observed.

> **Note**: The `knot_policy` `{"max_interior":57027,"quantum_ns":1}` is retained verbatim as observed.

> **Note**: The `57,027 records` in the native calculation packet are retained verbatim as observed.

> **Note**: The `3,262 rows in the packet` in the native calculation are retained verbatim as observed.

> **Note**: The `131,072 context` and `38,633 output tokens` for the Granite critic are retained verbatim as observed.

> **Note**: The `92,439 input tokens` for the Granite critic are retained verbatim as observed.

> **Note**: The `Granite critic` returned `124` tokens; this is retained verbatim as observed.

> **Note**: The `Causal handoff export` is hash-verified (stage `export` `7afd5c6a`); this is retained verbatim as observed.

> **Note**: The `Run findings ledger` sha256 `3e2008381f462cad7b2ec02fc0c8952f03aaf2e3a654453f90bc2c063bb54841` is retained verbatim as observed.

> **Note**: The `Greg Davis standing rule` is retained verbatim as observed.

> **Note**: The `Section 7 requirement` and `Section 4.2 absence` are retained verbatim as observed.

> **Note**: The `averaged_companion_sections_absent` includes `"4.2"`; this is retained verbatim as observed.

> **Note**: The `cluster_version` is `NO_CLUSTERING_D5`; this is retained verbatim as observed.

> **Note**: The `clock_absent` is `"decision/as-as-of"`; this is retained verbatim as observed.

> **Note**: The `natural gas instrument` is `instrument_id: 111313`; this is retained verbatim as observed.

> **Note**: The `Sunday 2021-10-03` is the sole source and run day; this is retained verbatim as observed.

> **Note**: The `pin` references (`legacy_observable_crosswalk`, `learned_d_structures_and_families`, etc.) are retained verbatim as observed; no inference about which layers are included beyond what is stated.

> **Note**: The `exact` byte counts and sha256s for `member_attachments` are retained verbatim as observed (`... with exact byte counts and sha256s as listed`).

> **Note**: The `stacked-0b895d45ef46` block lines are retained verbatim as observed.

> **Note**: The `intent.metadata[0][1]` reasoning string includes "Native BOSS development forecast under Frankie source contract 359e369cbc43b48ef33280dc64a44200e53d1d3612ed1e10fe26988e7e2baefd"; this is retained verbatim as observed.

> **Note**: The `records[0].record_json` contains `"disposition":"ABSTAIN"` and `"guessed_net_usd":-0.009004418763309818`; these are retained verbatim as observed.

> **Note**: The `path_p50_curve` around `18.0001` (level units) is retained verbatim as observed.

> **Note**: The `net_usd` `-0.009` and `overnight_gap` `+0.006` are retained verbatim as observed.

> **Note**: The `head` `"3b111695"` is retained verbatim as observed.

> **Note**: The `checkpoint_count` `2` is retained verbatim as observed.

> **Note**: The `usd_per_price_unit` `1.0` and `tick size` `0.001` are retained verbatim as observed.

> **Note**: The `instrument_id` `111313` is retained verbatim as observed.

> **Note**: The `session_id` `"NG_111313_OWN_SOURCE_20211003"` is retained verbatim as observed.

> **Note**: The `close_ns` `1633305600000000000` and `event_cutoff_ns` `1633298413317923251` are retained verbatim as observed.

> **Note**: The `event_ns` values in the known marks table range from `1633298400337726077` to `1633298413305137959`; all are retained verbatim as observed.

> **Note**: The `receive_ns` values in the known marks table range from `1633298400450870841` to `1633298413306139819`; all are retained verbatim as observed.

> **Note**: The `price` values in the known marks table range from `5.631` to `5.639`; all are retained verbatim as observed.

> **Note**: The `evidence_hash` values in the known marks table are all retained verbatim as observed.

> **Note**: The `Pin for cycle 0` (`legacy_observable_crosswalk`) and `Frozen learned-structure layers` are kept together under pin layers; no separation or merging of pin descriptions.

> **Note**: The `exact member ledger`, `exact_lifecycle_and_runway_ledger`, `legacy_observable_rows` are bound to the run; this is retained verbatim as observed.

> **Note**: The `Run findings ledger` sha256 is retained verbatim as observed.
...
> **Note**: The `pin-layer material` (Pin for cycle 0, Frozen learned-structure layers, Exact member ledger) is kept together under pin layers as observed; no reorganization.

> **Note**: The `stacked-0b895d45ef46` block and `member_attachments` are retained verbatim as observed.

> **Note**: The `intent.metadata[0][1]` reasoning string is retained verbatim as observed.

> **Note**: The `records[0]` fields are retained verbatim as observed.

> **Note**: The `shadow.request.environment` fields are retained verbatim as observed.

> **Note**: The `request` and `shadow.request` hashes are retained verbatim as observed.

> **Note**: The `receipt` fields are retained verbatim as observed.

> **Note**: The `Granite critic` output `{"evidence_refs": [], "contradictions": [], "missing_evidence": [], "hypotheses": []}` and verdict `"CONSISTENT"` are retained verbatim as observed.

> **Note**: The `controller` stage `51698a32` and request hash `81d53453` are retained verbatim as observed.

> **Note**: The `Causal handoff export` stage `export` `7afd5c6a` is retained verbatim as observed.

> **Note**: The `two-cycle prefix batch rebuilt (byte-identical to original)` is retained verbatim as observed.

> **Note**: The `private staging prefix at 21:34Z (run 35539110298)` is retained verbatim as observed.

> **Note**: The `session was re-issued with a written task` is retained verbatim as observed.

> **Note**: The `reports of recording, pushing and PR were not verifiable and turned out not to have happened` is retained verbatim as observed.

> **Note**: The `did not run on the retained request when first reported` is retained verbatim as observed.

> **Note**: The `Frankie side (Root)` statement about the session not seeing the request/prompt is retained verbatim as observed.

> **Note**: No inference about why the session didn't run or why PR reports weren't verifiable is made; only the observed statements are retained.

> **Note**: The `etc.` in `cycles list with cycle_index: 0 having ..., cycle_index: 1 having ..., etc.` is retained verbatim as observed.

> **Note**: The `...` in `controller_journal.py:..., frankie_controller.py:..., ...` is retained verbatim as observed.
...
> **Note**: The `...` in `C L 17 ...` is retained verbatim as observed.
...
> **Note**: The notes begin with "## Notes on part 1/4 (bytes 0-221457) merged with part 2/4 (bytes 221457-377873) and part 3/4 (bytes 377873-527078)" as in the original.
