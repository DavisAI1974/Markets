...
class TrunkConfig:
    ...
    qsv_dim: int = 0  # 0 until QSV_FEATURE_REGISTRY is declared. 7 was a fixture.
    n_venues: int = 8
...
class FieldEncoder(nn.Module):
    ...
    def forward(
        ...
        if self.qsv is not None:
            ...
            if qsv_mask is not None:
                m = qsv_mask if qsv_mask.dim() == 3 else qsv_mask.unsqueeze(-1)
                contrib = contrib * m.to(contrib.dtype)
            h = h + contrib
        return self.norm(h)
