...
def good_heads(**over):
    h = {
        ...
        "path_p50_curve": [0.0, 120.0, 340.0, 12500.0],
        "confidence": 0.82,
    }
    h.update(over)
    return h
...
def test_call_record_has_all_twelve_fields():
    r = proj().project("2026-08-24", good_heads(), PKT, "boss/1")
...
def test_low_confidence_stands_down_every_play():
    r = proj(confidence_floor=0.9).project(
        "2026-08-24", good_heads(confidence=0.6), PKT, "boss/1",
        plays_fired=["p1", "p2"], plays_stood_down=["p3"],
    )
    assert r.abstained
    assert r.payload["plays_fired"] == []
    assert r.payload["plays_stood_down"] == ["p1", "p2", "p3"]
...
def test_malformed_head_abstains_never_raises(bad):
    r = proj().project("2026-08-24", good_heads(**bad), PKT, "boss/1")
    assert r.abstained
...
def test_missing_head_field_abstains():
    ...
    r = proj().project("2026-08-24", h, PKT, "boss/1")
    assert r.abstained
...
def test_bad_date_is_a_caller_error_not_an_abstention():
    ...
    with pytest.raises(CallerError, match="ISO date"):
        proj().project("24-08-2026", good_heads(), PKT, "boss/1")
    with pytest.raises(CallerError):
        proj().abstain("24-08-2026", ["x"], PKT, "boss/1")


def test_disposition_must_be_call_or_abstain():
    payload = dict(proj().project("2026-08-24", good_heads(), PKT, "b").payload)
...
def test_a_real_change_does_move_the_digest():
    a = proj().project("2026-08-24", good_heads(), PKT, "b")
    b = proj().project("2026-08-24", good_heads(guessed_net_usd=12501.0), PKT, "b")
    assert a.determinism_digest() != b.determinism_digest()


def test_calibrator_version_moves_the_digest():
    a = proj(calibrator_version="c1").project("2026-08-24", good_heads(), PKT, "b")
    b = proj(calibrator_version="c2").project("2026-08-24", good_heads(), PKT, "b")
    assert a.determinism_digest() != b.determinism_digest()
...
def test_default_qsv_dim_is_zero():
    assert TrunkConfig().qsv_dim == 0, "7 was a unit-test fixture, not a width"
...
def test_wrong_qsv_width_is_caught_with_a_registry_hint():
    ...
    with pytest.raises(ValueError, match="QSV_FEATURE_REGISTRY"):
        m.represent(
            ...
            b["instrument_id"], b["parent"], torch.randn(2, 6, 5),
        )
...
def test_qsv_mask_gates_the_stream():
    ...
    with torch.no_grad():
        ...
        off_other = m.represent(*args, torch.randn(2, 6, 5), torch.zeros(2, 6))
...
def test_ablation_is_exact():
    ...
    with torch.no_grad():
        ...
        other = m.represent(*args, torch.randn(2, 6, 5))
