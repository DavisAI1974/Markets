...
It deliberately uses the checkout named by --repository so source(), prefix() and
_training() are the exact host/native implementations bound by that checkout.

The harness:
- verifies and opens the lawful source with ActualHost.source();
- binds the requested cycle from the trusted contract + trusted 19-step schedule;
- opens that cycle's verified prefix with ActualHost.prefix();
- removes only the scratch clone's training checkpoint/witnesses and calls
  ActualHost._training() so the seed checkpoint is fresh under the requested
  PyTorch intra-op thread count;
- reads the already-saved feedback stage from the scratch clone's cycles.sqlite,
  re-verifies the stage digest and feedback_hash, and reconstructs typed
  FrankieFeedback;
- rebuilds the session roster from bind_cycle exactly as SundayExecution does;
- calls NativeForecastLearner.step directly with the learner's diagnostic event.

It NEVER constructs a critic, calls Granite, invokes the Frankie principal, advances
CycleCoordinator, applies a completed checkpoint, or touches the original run directory.
The in-memory optimizer update is discarded when the process exits. Run this only against
a disposable clone whose path contains "scratch" or "bench".
"""
...
def _load_stage(db_path, request_id, stage, *, unpack, evidence_hash):
    with sqlite3.connect(db_path) as db:
        row = db.execute(
            'SELECT payload,digest FROM stages WHERE request=? AND stage=?',
            (request_id, stage),
        ).fetchone()
    if row is None:
        raise ValueError(f'missing retained {stage} stage for {request_id}')
    value = unpack(json.loads(row[0]))
    if evidence_hash(value) != row[1]:
        raise ValueError(f'retained {stage} stage digest changed')
    return value
...
def _typed_feedback(saved, learning):
    ...
    for item in body.get('sessions', ()):
        ...
        sessions.append(learning.SessionFeedback(
            session_id=item['session_id'], timing=timing, gap=gap, path=path))
    feedback = learning.FrankieFeedback(
        request_id=body['request_id'],
        input_hash=body['input_hash'],
        source_hash=body['source_hash'],
        available_ns=body['available_ns'],
        principal_receipt_hash=body['principal_receipt_hash'],
...
def _fresh_training(run_directory):
    (run_directory / 'training.sqlite').unlink(missing_ok=True)
    witnesses = run_directory / 'training-witnesses'
    if witnesses.exists():
        shutil.rmtree(witnesses)
...
def main():
    ...
    parser.add_argument('--configuration', required=True,
                        help='scratch-clone Sunday configuration; its run_directory must be disposable')
    parser.add_argument('--threads', type=int, required=True)
    parser.add_argument('--interop-threads', type=int, default=1)
    parser.add_argument('--cycle-index', type=int, default=0)
    parser.add_argument('--cycles-db', default=None,
                        help='retained cycles.sqlite; defaults to <run_directory>/cycles.sqlite')
    parser.add_argument('--log', required=True)
    parser.add_argument('--sample-seconds', type=float, default=0.25)
    args = parser.parse_args()

    if args.threads < 1 or args.interop_threads < 1:
        raise SystemExit('positive PyTorch thread counts required')
    if not 0 <= args.cycle_index < 19:
        raise SystemExit('cycle-index must be 0..18')
    if args.sample_seconds <= 0:
        raise SystemExit('positive sample interval required')

    repository = Path(args.repository).resolve()
    configuration_path = Path(args.configuration).resolve()
    configuration = json.loads(configuration_path.read_bytes())
    run_directory = Path(configuration['run_directory']).resolve()
    disposable = run_directory.as_posix().lower()
    if 'scratch' not in disposable and 'bench' not in run_directory.name.lower():
        raise SystemExit('refusing: run_directory does not look like a disposable scratch/bench clone')
    ...
    log.write('driver_start', repository=str(repository), configuration=str(configuration_path),
              run_directory=str(run_directory), cycle_index=args.cycle_index,
              threads=torch.get_num_threads(), interop_threads=torch.get_num_interop_threads(),
              torch=torch.__version__, python=sys.version.split()[0], cpus=os.cpu_count())

    _fresh_training(run_directory)
    log.write('fresh_training_removed')

    host = actual.ActualHost(configuration)
    ...
    try:
        host.source()
        log.write('source_verified')

        schedule_path = actual.verified(configuration['host_runtime']['schedule'])
        ...
        if len(steps) != 19:
            raise ValueError('complete 19-step schedule required')

        binding = bind_cycle(
            configuration['contract']['path'],
            configuration['contract']['sha256'],
            args.cycle_index,
            steps[args.cycle_index],
        )
        ...
        cycle_directory = run_directory / 'benchmark-native-learner' / f'cycle-{args.cycle_index:02d}'
        cycle_directory.mkdir(parents=True, exist_ok=True)
        host.prefix(binding, cycle_directory)
        log.write('prefix_verified', source_hash=binding['source_hash'])
        ...
        cycles_db = Path(args.cycles_db).resolve() if args.cycles_db else run_directory / 'cycles.sqlite'
        saved_binding = _load_stage(cycles_db, request_id, 'binding',
                                    unpack=c15_journal.unpack, evidence_hash=c15_journal.evidence_hash)
        saved_feedback = _load_stage(cycles_db, request_id, 'feedback',
                                     unpack=c15_journal.unpack, evidence_hash=c15_journal.evidence_hash)
        feedback = _typed_feedback(saved_feedback, learning)

        expected_learning = saved_binding.get('learning')
        if type(expected_learning) is not dict:
            raise ValueError('saved binding lacks learning identity')
        ...
        log.write('feedback_verified', feedback_hash=feedback.digest,
                  input_hash=feedback.input_hash, available_ns=feedback.available_ns)

        config = host.api.native.learning_config(
            host.optimizer,
            binding['sessions'],
            timing_policy_hash=binding['timing_policy_hash'],
            query_policy_hash=binding['query_policy_hash'],
        ...
        learner = learning.NativeForecastLearner(
            host.context, host.decoder, host.optimizer, config, event=learner_event)
        log.write('learner_step_start', feedback_hash=feedback.digest)
        ...
        result = learner.step(
            ...
            expected_sessions_hash=binding['expected_sessions_hash'],
            feedback=feedback,
            expected_feedback_hash=feedback.digest,
            learning_cutoff_ns=binding['learning_cutoff_ns'],
        )
        elapsed = time.perf_counter() - started
        result_hash = c15_journal.evidence_hash(result)
        ...
        print(json.dumps(dict(
            schema='FRANKIE_NATIVE_LEARNER_DIRECT_BENCHMARK_V1',
            request_id=request_id,
            threads=args.threads,
            interop_threads=args.interop_threads,
            wall_seconds=elapsed,
            result_hash=result_hash,
            feedback_hash=feedback.digest,
