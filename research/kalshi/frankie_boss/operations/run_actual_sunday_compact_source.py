...
def main():
    ...
    cycles=[] if args.cycles is None else ['--cycles', str(args.cycles)]
    sys.argv = [sys.argv[0], '--configuration', args.configuration] + cycles + (['--prepare-only'] if args.prepare_only else [])
