...
6. **CORRECTED. Frankie's own run (33746436209) DID file all 30 required output ledgers.** My
   first reading of this item said the outputs directory was empty and the cited receipt hash
   matched no file; that was wrong: I listed the run root and pointed the validator at it, while
   the bundle lives one level down in `principal_outputs/`. Measured properly with
   `native_principal_outputs.load_bundle` on `principal_outputs/`: `RECEIPT.json` present, its
   `receipt_sha256` `ac42944a…` equal to the hash Frankie cited in his artifact AND to the hash
   recomputed from the ledgers on disk; 30 of 30 required ledgers present (10 registry outputs,
   the 18 contract sections, raw-MBO classification, knowledge verification), `missing_ledger_ids`
   empty, every hash chain verified with zero failures. Entries: each of the 18 section ledgers 20,
   candidate discoveries 90, first locks 90, negative/sparse/inconclusive 50, knowledge retrieval
   receipts 76, provider invocation receipts 20, state and probability movies 20, reasoning movie
   21, raw-MBO classification 661, knowledge verification 93, answer-wall access 0 with the stated
   empty reason (no sealed surface was opened; a valid run's ledger is empty). So the 18
