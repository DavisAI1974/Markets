"""Explicit stacked V2 critic route: the V1 route with the token-minimal V2 codec, exact native inverse kept."""
...
from . import granite_context_stacked as v1_codec
from . import granite_context_stacked_v2 as codec
from . import granite_context_stacked_route as v1_route

SCHEMA = 'BOSS_GRANITE_STACKED_ROUTE_V2'
PROMPT_VERSION = 'BOSS_GRANITE_STACKED_ROUTE_PROMPT_V4'
...
class StackedContext:
    text: str
    limits: v1_codec.DecodeLimits = v1_codec.DEFAULT_LIMITS
...
def stacked_native_context(snapshot, *, scope_public=None, prefix_seed=None, limits=v1_codec.DEFAULT_LIMITS, knowledge=None):
    ...
    if result.native().text != checked.text:
        raise ValueError('stacked V2 context failed exact native inverse')
    return result


def parse_stacked_context(text, *, expected_hash, limits=v1_codec.DEFAULT_LIMITS):
    native.sha256_digest(expected_hash, 'stacked V2 snapshot')
    if type(limits) is not v1_codec.DecodeLimits or type(text) is not str:
    ...
    if len(text.encode()) > limits.max_input_bytes or hashlib.sha256(text.encode()).hexdigest() != expected_hash:
        raise ValueError('stacked V2 context byte admission/hash mismatch')
    ...
    if body is None or set(body) not in ({'schema', 'native_hash', 'codec'}, {'schema', 'native_hash', 'codec', 'knowledge'}) or body['schema'] != SCHEMA:
        raise ValueError('invalid stacked V2 context schema')
    if native._text(body) != text:
        raise ValueError('noncanonical stacked V2 context serialization')
    ...
    v1_route._knowledge(body, restored)
...
def build_stacked_prompt(snapshot, *, max_prompt_bytes=None):
    if type(snapshot) is not StackedContext:
        raise TypeError('stacked V2 context required')
    ...
    if 'knowledge' in body:
        ...
        body['knowledge_view'] = public_knowledge(body.pop('knowledge'))
    ...
    if max_prompt_bytes is not None:
        if type(max_prompt_bytes) is not int or max_prompt_bytes <= 0 or len(text.encode()) > max_prompt_bytes:
            raise ValueError('stacked V2 prompt exceeds configured byte capacity')
    return StackedPrompt(text, snapshot.hash)


def stacked_parser_code_hash():
    return hashlib.sha256(Path(__file__).read_bytes() + Path(codec.__file__).read_bytes()
        + v1_route.stacked_parser_code_hash().encode() + codec.grammar_hash().encode()).hexdigest()
