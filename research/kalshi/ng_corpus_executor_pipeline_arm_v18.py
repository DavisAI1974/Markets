#!/usr/bin/env python3
"""Arm the paginated latest-version stable corpus prefix under readiness v23."""
...
SCHEMA = "ng_corpus_executor_pipeline_arm.v18"
STATUS = "PAGINATED_LATEST_VERSION_CORPUS_PIPELINE_ARMED"
ARMED_STAGES = compiler.CONFIGURED_STAGES


class CorpusExecutorPipelineArmV18Error(ValueError):
    pass
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CorpusExecutorPipelineArmV18Error(
...
def _authority(value: Mapping[str, Any], *, label: str) -> None:
    try:
        ...
        raise CorpusExecutorPipelineArmV18Error(str(error)) from error
...
def _validated_compiled(
    ...
    if source_plan.get("outcome_paths") != []:
        raise CorpusExecutorPipelineArmV18Error(
            "corpus plan may not reference outcomes"
        )
...
def build_armed_plan(
    ...
    receipt: dict[str, Any] = {
        ...
        "complete_s3_service_pagination_armed_first": True,
        ...
        "paginated_resolution_armed_before_inventory_capture": True,
        ...
        "after_paginated_latest_version_resolution": (
            "CAPTURE_CHECKSUM_ENABLED_EXACT_S3_INVENTORY"
        ),
        "after_s3_inventory_capture": (
            "ATTEST_EXACT_S3_TO_LOCAL_MATERIALIZATION"
        ),
        "after_s3_materialization": (
            "RUN_BROAD_BYTE_INSPECTION_FROM_ATTESTED_PLAN"
        ),
        ...
        "next_permitted_stage": (
            "RUN_BRANCH_GUARDED_PAGINATED_LATEST_VERSION_CORPUS_PIPELINE"
        ),
    }
...
def validate_arm_receipt(
    ...
    *,
    ...
    if checked.get("schema") != SCHEMA or observed != fingerprinting._fp(checked):
        raise CorpusExecutorPipelineArmV18Error(
            "pipeline arm v18 schema or fingerprint mismatch"
        )
    checked["fingerprint"] = observed
    _authority(checked, label="pipeline arm v18 receipt")
    source_plan, source_receipt, commands = _validated_compiled(
        compiled_plan, compiler_receipt
    )
    ...
    for field, item in expected.items():
        if checked.get(field) != item:
            raise CorpusExecutorPipelineArmV18Error(
                f"pipeline arm v18 field mismatch: {field}"
            )
