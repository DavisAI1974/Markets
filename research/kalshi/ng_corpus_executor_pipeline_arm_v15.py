#!/usr/bin/env python3
"""Arm the S3-attested plan-bound corpus prefix under readiness v20."""
...
SCHEMA = "ng_corpus_executor_pipeline_arm.v15"
STATUS = "S3_ATTESTED_PLAN_BOUND_CORPUS_PIPELINE_ARMED"
ARMED_STAGES = compiler.CONFIGURED_STAGES


class CorpusExecutorPipelineArmV15Error(ValueError):
    pass
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CorpusExecutorPipelineArmV15Error(f"cannot read JSON {path}: {error}") from error
    if not isinstance(value, dict):
        raise CorpusExecutorPipelineArmV15Error(f"JSON artifact must be an object: {path}")
    return value
...
def _authority(value: Mapping[str, Any], *, label: str) -> None:
    for field in (
        ...
        if value.get(field) is not False:
            raise CorpusExecutorPipelineArmV15Error(f"{label}: {field} must remain false")
    if value.get("one_signal_authority_preserved") is not True:
        raise CorpusExecutorPipelineArmV15Error(f"{label}: one signal authority must be preserved")
    if value.get("blind_forecasts_immutable") is not True:
        raise CorpusExecutorPipelineArmV15Error(f"{label}: blind forecasts must remain immutable")
    if value.get("cme_event_contracts_mode") != "SHADOW":
        raise CorpusExecutorPipelineArmV15Error(f"{label}: CME event contracts must remain SHADOW")
    if value.get("brokerage_contract") != "tastytrade_not_ibkr":
        raise CorpusExecutorPipelineArmV15Error(f"{label}: brokerage must remain tastytrade, not IBKR")
...
def _commands(plan: Mapping[str, Any]) -> dict[str, list[str]]:
    ...
    for key in ARMED_STAGES:
        ...
        if not isinstance(argv, list) or not argv or not all(
            ...
            raise CorpusExecutorPipelineArmV15Error(
...
def _validated_compiled(
    ...
    if source_plan.get("outcome_paths") != []:
        raise CorpusExecutorPipelineArmV15Error("corpus plan may not reference outcomes")
...
def build_armed_plan(
    ...
    receipt: dict[str, Any] = {
        ...
        "s3_materialization_armed_before_broad_inspection": True,
        "remote_object_version_and_checksum_required": True,
        "materialized_bytes_must_match_remote_and_definition": True,
        "standalone_inventory_receipt_emitted_by_materialization_stage": True,
        "broad_inspection_armed_after_s3_materialization": True,
        "definition_byte_binding_armed_before_target_slice_inspection": True,
        "target_slice_inspection_armed_before_broad_lineage": True,
        "target_slice_broad_lineage_armed_before_derivation": True,
        "target_slice_derivation_armed_before_basis_regeneration": True,
        ...
        "after_target_slice_inspection": "BIND_TARGET_SLICES_TO_EXACT_BROAD_SOURCES",
        "after_target_slice_broad_lineage": "REDERIVE_TARGET_BYTES_FROM_BOUND_BROAD_SOURCES",
        "after_target_slice_derivation": "REGENERATE_DAILY_BASIS_FROM_PLAN_BOUND_DERIVATION",
        ...
        "next_permitted_stage": "RUN_BRANCH_GUARDED_S3_ATTESTED_CORPUS_PIPELINE",
    }
...
def validate_arm_receipt(
    ...
    *,
    ...
    if checked.get("schema") != SCHEMA or observed != fingerprinting._fp(checked):
        raise CorpusExecutorPipelineArmV15Error(
            "pipeline arm v15 schema or fingerprint mismatch"
        )
    checked["fingerprint"] = observed
    _authority(checked, label="pipeline arm v15 receipt")
    source_plan, source_receipt, commands = _validated_compiled(
        compiled_plan, compiler_receipt
    )
    ...
    for field, item in expected.items():
        if checked.get(field) != item:
            raise CorpusExecutorPipelineArmV15Error(
                f"pipeline arm v15 field mismatch: {field}"
            )
