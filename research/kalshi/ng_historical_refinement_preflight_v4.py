#!/usr/bin/env python3
"""Run readiness-v6 historical refinement behind live pre/post Git alignment.

Version 4 binds the branch-guarded executor to the full-window exact-overlap-first v6
stage contract. The exact plan is validated and embedded in the receipt; branch, remote,
HEAD, and worktree alignment are checked on both sides by the established preflight.
No G15 stage can run unless broad scope and exact L1/MBO event-time overlap both precede it.
"""
...
SCHEMA = "ng_historical_refinement_preflight.v4"
EXECUTOR_CONTRACT = "ng_historical_refinement_executor_v4"
READINESS_CONTRACT = readiness.SCHEMA
LEGACY_SCHEMA = legacy.SCHEMA


class HistoricalRefinementPreflightV4Error(RuntimeError):
    """Raised when branch-guarded execution is not bound to readiness v6."""
...
def _finalize(plan: Mapping[str, Any], base: Mapping[str, Any]) -> dict[str, Any]:
    ...
    result["execution_plan_v6_validated"] = True
...
def execute_preflight(
    ...
    **kwargs: Any,
    ...
    """Execute one readiness-v6 stage with branch alignment before and after."""
    executor.validate_plan(plan)
...
def validate_receipt(receipt: Mapping[str, Any]) -> None:
    ...
    if value.get("schema") != SCHEMA or observed != legacy._fingerprint(value):
        raise HistoricalRefinementPreflightV4Error("preflight v4 schema or fingerprint mismatch")
    if value.get("executor_contract") != EXECUTOR_CONTRACT:
        raise HistoricalRefinementPreflightV4Error("preflight v4 executor contract mismatch")
    if value.get("readiness_contract") != READINESS_CONTRACT:
        raise HistoricalRefinementPreflightV4Error("preflight v4 readiness contract mismatch")
    if value.get("readiness_stage_contract") != STAGE_CONTRACT:
        raise HistoricalRefinementPreflightV4Error("preflight v4 stage contract mismatch")
    if value.get("readiness_stage_contract_fingerprint") != STAGE_CONTRACT_FINGERPRINT:
        raise HistoricalRefinementPreflightV4Error("preflight v4 stage contract fingerprint mismatch")
    if value.get("execution_plan_v6_validated") is not True:
        raise HistoricalRefinementPreflightV4Error("preflight v4 must record v6 plan validation")
    if value.get("broad_corpus_scope_required") is not True:
        raise HistoricalRefinementPreflightV4Error("broad corpus scope must remain mandatory")
    if value.get("broad_corpus_exact_overlap_required") is not True:
        raise HistoricalRefinementPreflightV4Error("broad corpus exact overlap must remain mandatory")
    if value.get("lock_first_g15_scoring_required") is not True:
        raise HistoricalRefinementPreflightV4Error("G15 lock-first scoring must remain mandatory")
    if value.get("counterfactual_g16_lineage_required") is not True:
        raise HistoricalRefinementPreflightV4Error("G16 counterfactual lineage must remain mandatory")

    plan = value.get("execution_plan_snapshot")
    if not isinstance(plan, Mapping):
        raise HistoricalRefinementPreflightV4Error("preflight v4 requires the exact plan snapshot")
    with _v6_context():
        executor.validate_plan(plan)
    if value.get("plan_fingerprint") != plan.get("fingerprint"):
        raise HistoricalRefinementPreflightV4Error("receipt plan fingerprint does not match embedded v6 plan")
    stage_keys = [row.get("key") for row in plan.get("stages") or [] if isinstance(row, Mapping)]
    if stage_keys != STAGE_ORDER:
        raise HistoricalRefinementPreflightV4Error("embedded plan does not use readiness-v6 stage order")
    try:
        ...
        raise HistoricalRefinementPreflightV4Error(
            "broad scope, exact overlap, or G15 replay stage is missing"
        ) from error
    if not broad_index < overlap_index < replay_index:
        raise HistoricalRefinementPreflightV4Error(
            "broad scope and exact overlap must precede G15 replay in order"
        )
    for key in ("broad_corpus_scope", "broad_corpus_exact_overlap"):
        row = next(row for row in plan["stages"] if row.get("key") == key)
        if row.get("requires_fixed_outcomes") is not False:
            raise HistoricalRefinementPreflightV4Error(f"{key} must remain pre-outcome")
    ...
    if isinstance(executor_result, Mapping):
        ...
        if selected_stage is not None and selected_stage not in STAGE_ORDER:
            raise HistoricalRefinementPreflightV4Error("executor selected a stage outside readiness v6")

    base = _base_receipt(receipt)
    if base.get("fingerprint") != value.get("legacy_preflight_fingerprint"):
        raise HistoricalRefinementPreflightV4Error("embedded legacy preflight fingerprint mismatch")
    with _v6_context():
        legacy.validate_receipt(base)
