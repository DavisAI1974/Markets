#!/usr/bin/env python3
"""Run the guarded historical executor against runtime-observed readiness v27."""
...
def _check_v27_plan_contract(plan: Mapping[str, Any]) -> None:
    ...
    if third.get("expected_output") != (
        ...
        raise legacy_executor.HistoricalRefinementExecutionError(
            "paginated S3 resolution must follow finalization"
        )
    if fourth.get("expected_output") != (
        "ng_corpus_s3_runtime_observed_inventory_capture_attestation.json"
    ):
        raise legacy_executor.HistoricalRefinementExecutionError(
            "runtime-observed paginated inventory must follow version resolution"
        )
    if fifth.get("key") != "corpus_s3_materialization":
        raise legacy_executor.HistoricalRefinementExecutionError(
            "materialization must follow runtime-observed inventory capture"
        )


@contextmanager
def _v27_context() -> Iterator[None]:
