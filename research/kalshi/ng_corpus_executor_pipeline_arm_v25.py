#!/usr/bin/env python3
"""Arm the source-native-identity corpus prefix under readiness v30."""
...
SCHEMA = "ng_corpus_executor_pipeline_arm.v25"
STATUS = "SOURCE_NATIVE_IDENTITY_BOUND_CORPUS_PIPELINE_ARMED"
ARMED_STAGES = compiler.CONFIGURED_STAGES


class CorpusExecutorPipelineArmV25Error(ValueError):
    pass
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CorpusExecutorPipelineArmV25Error(f"cannot read JSON {path}: {error}") from error
    if not isinstance(value, dict):
        raise CorpusExecutorPipelineArmV25Error(f"JSON artifact must be an object: {path}")
    return value
...
def _authority(value: Mapping[str, Any], *, label: str) -> None:
    try:
        ...
        raise CorpusExecutorPipelineArmV25Error(str(error)) from error
...
def _commands(plan: Mapping[str, Any]) -> dict[str, list[str]]:
    ...
    for key in ARMED_STAGES:
        ...
        if not isinstance(argv, list) or not argv or not all(
            ...
            raise CorpusExecutorPipelineArmV25Error(
...
def _validated_compiled(
    ...
    if source_plan.get("outcome_paths") != []:
        raise CorpusExecutorPipelineArmV25Error("corpus plan may not reference outcomes")
...
def build_armed_plan(
    ...
    receipt: dict[str, Any] = {
        ...
        "runtime_observed_inventory_capture_armed_after_resolution": True,
        "exact_version_materialization_armed_before_recursive_provenance": True,
        "recursive_materializer_provenance_armed_before_source_identity": True,
        "source_native_identity_armed_before_broad_inspection": True,
        "source_native_identity_required_before_definition_binding": True,
        ...
        "after_recursive_materializer_provenance": "ATTEST_SOURCE_NATIVE_CORPUS_IDENTITY",
        "after_source_native_identity": "RUN_BROAD_BYTE_LEVEL_CORPUS_INSPECTION",
...
def validate_arm_receipt(
    ...
    *,
    ...
    if checked.get("schema") != SCHEMA or observed != fingerprinting._fp(checked):
        raise CorpusExecutorPipelineArmV25Error(
            "pipeline arm v25 schema or fingerprint mismatch"
        )
    checked["fingerprint"] = observed
    _authority(checked, label="pipeline arm v25 receipt")
    source_plan, source_receipt, commands = _validated_compiled(
        compiled_plan, compiler_receipt
    )
    ...
    for field, expected_value in expected.items():
        if checked.get(field) != expected_value:
            raise CorpusExecutorPipelineArmV25Error(
                f"pipeline arm v25 field mismatch: {field}"
            )
