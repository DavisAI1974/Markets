#!/usr/bin/env python3
"""Arm the runtime-observed stable corpus prefix under readiness v27."""
...
SCHEMA = "ng_corpus_executor_pipeline_arm.v22"
STATUS = "RUNTIME_OBSERVED_INVENTORY_BOUND_CORPUS_PIPELINE_ARMED"
ARMED_STAGES = compiler.CONFIGURED_STAGES


class CorpusExecutorPipelineArmV22Error(ValueError):
    pass
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CorpusExecutorPipelineArmV22Error(f"cannot read JSON {path}: {error}") from error
    if not isinstance(value, dict):
        raise CorpusExecutorPipelineArmV22Error(f"JSON artifact must be an object: {path}")
    return value
...
def _authority(value: Mapping[str, Any], *, label: str) -> None:
    try:
        ...
        raise CorpusExecutorPipelineArmV22Error(str(error)) from error
...
def _commands(plan: Mapping[str, Any]) -> dict[str, list[str]]:
    ...
    for key in ARMED_STAGES:
        ...
        if not isinstance(argv, list) or not argv or not all(
            ...
            raise CorpusExecutorPipelineArmV22Error(
...
def _validated_compiled(
    ...
    if source_plan.get("outcome_paths") != []:
        raise CorpusExecutorPipelineArmV22Error("corpus plan may not reference outcomes")
...
def build_armed_plan(
    ...
    receipt: dict[str, Any] = {
        ...
        "runtime_observed_inventory_armed_before_materialization": True,
        ...
        "after_paginated_latest_version_resolution": (
            "CAPTURE_RUNTIME_OBSERVED_PAGINATED_CHECKSUM_ENABLED_S3_INVENTORY"
        ),
...
def validate_arm_receipt(
    ...
    *,
    ...
    if checked.get("schema") != SCHEMA or observed != fingerprinting._fp(checked):
        raise CorpusExecutorPipelineArmV22Error(
            "pipeline arm v22 schema or fingerprint mismatch"
        )
    checked["fingerprint"] = observed
    _authority(checked, label="pipeline arm v22 receipt")
    source_plan, source_receipt, commands = _validated_compiled(
        compiled_plan, compiler_receipt
    )
    ...
    for field, item in expected.items():
        if checked.get(field) != item:
            raise CorpusExecutorPipelineArmV22Error(
                f"pipeline arm v22 field mismatch: {field}"
            )
