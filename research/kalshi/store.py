...
def render_open_items():
    ...
    L = ["# OPEN ITEMS - the tracked work registry", "",
         ...
         "By size: " + ", ".join("**%s** %d" % (s, sum(1 for i in live if i.get("size") == s))
                                 for s in ("XS", "S", "M", "L")), "", "---", "",
         "By tier: " + ", ".join("**%s** %d" % (t, sum(1 for i in live if i.get("tier") == t))
                                 for t in ("ESSENTIAL", "BIGGEST_WIN", "REST")), "",
         "> " + (tiers.get("note", "").replace("\n", " ")), "", "---", ""]
    for tier in ("ESSENTIAL", "BIGGEST_WIN", "REST"):
        rows = [i for i in live if i.get("tier") == tier]
        if not rows:
            continue
        L += ["## %s (%d)" % (tier.replace("_", " "), len(rows)), "",
              "*%s*" % tiers.get(tier, ""), "",
              "| id | size | title | why it is here |", "|---|---|---|---|"]
        for i in sorted(rows, key=lambda x: (order.get(x.get("size"), 4), x["id"])):
            L.append("| **%s** | %s | %s | %s |"
                     % (i["id"], i.get("size") or "?", i["title"].replace("|", "\\|"),
                        (i.get("tier_why") or "-").replace("|", "\\|").replace("\n", " ")))
        L.append("")
    L += ["---", "", "## EVERY LIVE ITEM, ONE TABLE", "",
          "| id | tier | size | status | raised | title | blocked by |",
          "|---|---|---|---|---|---|---|"]
    for i in sorted(live, key=lambda x: (tier_order.get(x.get("tier"), 3),
                                         order.get(x.get("size"), 4), x["id"])):
        L.append("| **%s** | %s | %s | %s | %s | %s | %s |"
                 % (i["id"], i.get("tier") or "?", i.get("size") or "?", i.get("status"),
...
def cmd_selftest(a):
    ...
    _reg = json.load(open(OPEN_ITEMS, encoding="utf-8"))
    _live = [i for i in _reg["items"] if i.get("status") in ("OPEN", "IN_PROGRESS")]
    check("every live item carries a tier - nothing falls outside the reading order",
          all(i.get("tier") in ("ESSENTIAL", "BIGGEST_WIN", "REST") for i in _live))
    check("every ESSENTIAL and BIGGEST_WIN item argues its own placement",
          all(i.get("tier_why") for i in _live
              if i.get("tier") in ("ESSENTIAL", "BIGGEST_WIN")))
    check("the render carries all three tiers and drops nothing",
          all(t.replace("_", " ") in om for t in ("ESSENTIAL", "BIGGEST_WIN", "REST")))
