#!/usr/bin/env python3
"""Arm the expected-day-bound stable corpus prefix under readiness v25."""
...
SCHEMA = "ng_corpus_executor_pipeline_arm.v20"
STATUS = "EXPECTED_DAY_BOUND_CORPUS_PIPELINE_ARMED"
ARMED_STAGES = compiler.CONFIGURED_STAGES


class CorpusExecutorPipelineArmV20Error(ValueError):
    pass
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CorpusExecutorPipelineArmV20Error(f"cannot read JSON {path}: {error}") from error
    if not isinstance(value, dict):
        raise CorpusExecutorPipelineArmV20Error(f"JSON artifact must be an object: {path}")
    return value
...
def _authority(value: Mapping[str, Any], *, label: str) -> None:
    try:
        ...
        raise CorpusExecutorPipelineArmV20Error(str(error)) from error
...
def _commands(plan: Mapping[str, Any]) -> dict[str, list[str]]:
    ...
    for key in ARMED_STAGES:
        ...
        if not isinstance(argv, list) or not argv or not all(
            ...
            raise CorpusExecutorPipelineArmV20Error(
...
def _validated_compiled(
    ...
    if source_plan.get("outcome_paths") != []:
        raise CorpusExecutorPipelineArmV20Error("corpus plan may not reference outcomes")
...
def build_armed_plan(
    ...
    receipt: dict[str, Any] = {
        ...
        "complete_calendar_partition_required": True,
        ...
        "s3_resolution_armed_only_after_expected_day_contract": True,
        "complete_resolution_pagination_armed_second": True,
        "complete_inventory_pagination_armed_third": True,
        ...
        "after_expected_day_contract": "RESOLVE_COMPLETE_PAGINATED_S3_LATEST_VERSIONS",
...
def validate_arm_receipt(
    ...
    *,
    ...
    if checked.get("schema") != SCHEMA or observed != fingerprinting._fp(checked):
        raise CorpusExecutorPipelineArmV20Error(
            "pipeline arm v20 schema or fingerprint mismatch"
        )
    checked["fingerprint"] = observed
    _authority(checked, label="pipeline arm v20 receipt")
    source_plan, source_receipt, commands = _validated_compiled(
        compiled_plan, compiler_receipt
    )
    ...
    for field, item in expected.items():
        if checked.get(field) != item:
            raise CorpusExecutorPipelineArmV20Error(
                f"pipeline arm v20 field mismatch: {field}"
            )
