#!/usr/bin/env python3
"""One-off ops (S100): pull the never-pulled July 2026 CL tape (S101; CL year store ends 20260630) (year-pull boundary gap) into
...
SCRATCH = os.environ.get("JUL_SCRATCH_CL", "/tmp/jul_scratch_cl")
...
def existing_fat_days():
    out = set()
    r = s3.list_objects_v2(Bucket=B, Prefix=PFX + "CL_202607", MaxKeys=1000)
...
def main():
    ...
    client = dbf._client()
    dbf.batch_pull(client, "CL", START, END, "mbp-10", max_cost=25.0,
                   out_dir=SCRATCH, flush_dir=SCRATCH)
    ok = fail = skip = 0
    for gz in sorted(glob.glob(os.path.join(SCRATCH, "CL_*.jsonl.gz"))):
        m = re.search(r"CL_(\d{8})", os.path.basename(gz))
        if not m:
            continue
        day = m.group(1)
        if day in have:
        ...
        key = PFX + f"CL_{day}_{DOW[d.weekday()]}.jsonl.gz"
