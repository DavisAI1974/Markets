#!/usr/bin/env python3
"""Arm the five readiness-v6 corpus stages under one stable executor plan.

The guarded executor still runs only the first blocking readiness stage. The order is byte
inspection, exact daily basis regeneration, replay catalog export, broad scope verification,
and exact full-window L1/MBO overlap verification. G15 and later stages stay disabled.
"""
...
SCHEMA = "ng_corpus_executor_pipeline_arm.v3"
ARMED_STAGES = compiler.CONFIGURED_STAGES
STATUS = "BROAD_CORPUS_EXACT_OVERLAP_PIPELINE_ARMED"


class CorpusExecutorPipelineArmV3Error(ValueError):
    """Raised when the exact-overlap corpus pipeline cannot be armed safely."""
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CorpusExecutorPipelineArmV3Error(f"cannot read JSON {path}: {error}") from error
    if not isinstance(value, dict):
        raise CorpusExecutorPipelineArmV3Error(f"JSON artifact must be an object: {path}")
    return value
...
def _authority(value: Mapping[str, Any], *, label: str) -> None:
    for field in (
        ...
        if value.get(field) is not False:
            raise CorpusExecutorPipelineArmV3Error(f"{label}: {field} must remain false")
    if value.get("one_signal_authority_preserved") is not True:
        raise CorpusExecutorPipelineArmV3Error(f"{label}: one signal authority must be preserved")
    if value.get("blind_forecasts_immutable") is not True:
        raise CorpusExecutorPipelineArmV3Error(f"{label}: blind forecasts must remain immutable")
    if value.get("cme_event_contracts_mode") != "SHADOW":
        raise CorpusExecutorPipelineArmV3Error(f"{label}: CME event contracts must remain SHADOW")
    if value.get("brokerage_contract") != "tastytrade_not_ibkr":
        raise CorpusExecutorPipelineArmV3Error(f"{label}: brokerage must remain tastytrade, not IBKR")
...
def _commands(plan: Mapping[str, Any]) -> dict[str, list[str]]:
    ...
    for key in ARMED_STAGES:
        ...
        if not isinstance(argv, list) or not argv or not all(
            ...
            raise CorpusExecutorPipelineArmV3Error(f"{key}: compiled command is missing or invalid")
...
def _validate_compiled(
    ...
    if receipt.get("schema") != compiler.SCHEMA:
        raise CorpusExecutorPipelineArmV3Error("compiler receipt schema mismatch")
    if receipt.get("configured_stages") != list(ARMED_STAGES):
        raise CorpusExecutorPipelineArmV3Error("compiler stage order mismatch")
    if plan.get("outcome_paths") != []:
        raise CorpusExecutorPipelineArmV3Error("corpus plan must not reference outcomes")
...
def _arm_plan(plan: Mapping[str, Any], commands: Mapping[str, list[str]]) -> dict[str, Any]:
    ...
    for key in ARMED_STAGES:
        if rows[key].get("enabled") is not True:
            raise CorpusExecutorPipelineArmV3Error(f"{key}: stage was not armed")
        if rows[key].get("requires_fixed_outcomes") is not False:
            raise CorpusExecutorPipelineArmV3Error(f"{key}: stage must remain pre-outcome")
    for spec in readiness.STAGES[len(ARMED_STAGES) :]:
        if rows[spec.key].get("enabled"):
            raise CorpusExecutorPipelineArmV3Error(f"{spec.key}: downstream stage must remain disabled")
    return armed
...
def build_armed_plan(
    ...
    receipt: dict[str, Any] = {
        ...
        "single_stable_plan_for_target_broad_and_overlap_stages": True,
        ...
        "next_permitted_stage": "RUN_BRANCH_GUARDED_EXACT_OVERLAP_CORPUS_PIPELINE",
        "after_exact_overlap_gate": "CONFIGURE_EXACT_G15_REPLAY_ONLY_IF_VERIFIED",
    }
...
def validate_arm_receipt(
    ...
    *,
    ...
    if checked.get("schema") != SCHEMA or observed != fingerprinting._fp(checked):
        raise CorpusExecutorPipelineArmV3Error("pipeline arm v3 schema or fingerprint mismatch")
    checked["fingerprint"] = observed
    _authority(checked, label="pipeline arm v3 receipt")
    source_plan, source_receipt, commands = _validate_compiled(compiled_plan, compiler_receipt)
    ...
    if fingerprinting._canonical(armed_plan) != fingerprinting._canonical(expected_plan):
        raise CorpusExecutorPipelineArmV3Error("armed plan differs from deterministic transformation")
    expected_fields = {
        "status": STATUS,
        "compiler_receipt_fingerprint": source_receipt["fingerprint"],
        "compiled_plan_fingerprint": source_plan["fingerprint"],
    ...
    for field, expected in expected_fields.items():
        if checked.get(field) != expected:
            raise CorpusExecutorPipelineArmV3Error(f"pipeline arm v3 field mismatch: {field}")
