#!/usr/bin/env python3
"""Arm the dual-inspection corpus prefix under readiness v16."""
...
SCHEMA = "ng_corpus_executor_pipeline_arm.v12"
STATUS = "DUAL_INSPECTION_CORPUS_PIPELINE_ARMED"
ARMED_STAGES = compiler.CONFIGURED_STAGES


class CorpusExecutorPipelineArmV12Error(ValueError):
    pass
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CorpusExecutorPipelineArmV12Error(f"cannot read JSON {path}: {error}") from error
    if not isinstance(value, dict):
        raise CorpusExecutorPipelineArmV12Error(f"JSON artifact must be an object: {path}")
    return value
...
def _authority(value: Mapping[str, Any], *, label: str) -> None:
    for field in (
        ...
        if value.get(field) is not False:
            raise CorpusExecutorPipelineArmV12Error(f"{label}: {field} must remain false")
    if value.get("one_signal_authority_preserved") is not True:
        raise CorpusExecutorPipelineArmV12Error(f"{label}: one signal authority must be preserved")
    if value.get("blind_forecasts_immutable") is not True:
        raise CorpusExecutorPipelineArmV12Error(f"{label}: blind forecasts must remain immutable")
    if value.get("cme_event_contracts_mode") != "SHADOW":
        raise CorpusExecutorPipelineArmV12Error(f"{label}: CME event contracts must remain SHADOW")
    if value.get("brokerage_contract") != "tastytrade_not_ibkr":
        raise CorpusExecutorPipelineArmV12Error(f"{label}: brokerage must remain tastytrade, not IBKR")
...
def _commands(plan: Mapping[str, Any]) -> dict[str, list[str]]:
    ...
    for key in ARMED_STAGES:
        ...
        if not isinstance(argv, list) or not argv or not all(isinstance(part, str) and part for part in argv):
            raise CorpusExecutorPipelineArmV12Error(f"{key}: compiled command is missing or invalid")
...
def _validated_compiled(
    ...
    if source_plan.get("outcome_paths") != []:
        raise CorpusExecutorPipelineArmV12Error("corpus plan may not reference outcomes")
...
def build_armed_plan(
    ...
    source_plan, source_receipt, commands = _validated_compiled(compiled_plan, compiler_receipt)
    ...
    receipt: dict[str, Any] = {
        ...
        "definition_byte_binding_armed_before_target_slice_inspection": True,
        "target_slice_inspection_armed_before_basis_regeneration": True,
        "all_downstream_g15_g16_stages_disabled": True,
        "after_broad_inspection": "BIND_BROAD_DEFINITION_BYTES",
        "after_definition_byte_binding": "INSPECT_TARGET_SLICES",
        "after_target_slice_inspection": "REGENERATE_DAILY_BASIS_ONLY_FROM_TARGET_RECEIPT",
        ...
        "next_permitted_stage": "RUN_BRANCH_GUARDED_DUAL_INSPECTION_CORPUS_PIPELINE",
    }
    receipt["fingerprint"] = fingerprinting._fp(receipt)
    validate_arm_receipt(
        receipt, compiled_plan=source_plan,
        compiler_receipt=source_receipt, armed_plan=armed,
    )
    return armed, receipt


def validate_arm_receipt(
    value: Mapping[str, Any], *, compiled_plan: Mapping[str, Any],
    compiler_receipt: Mapping[str, Any], armed_plan: Mapping[str, Any],
) -> dict[str, Any]:
    checked = copy.deepcopy(dict(value))
    observed = checked.pop("fingerprint", None)
    if checked.get("schema") != SCHEMA or observed != fingerprinting._fp(checked):
        raise CorpusExecutorPipelineArmV12Error("pipeline arm v12 schema or fingerprint mismatch")
    checked["fingerprint"] = observed
    _authority(checked, label="pipeline arm v12 receipt")
    source_plan, source_receipt, commands = _validated_compiled(compiled_plan, compiler_receipt)
    expected_plan = _arm(source_plan, commands)
    if fingerprinting._canonical(armed_plan) != fingerprinting._canonical(expected_plan):
        raise CorpusExecutorPipelineArmV12Error("armed plan differs from deterministic transformation")
    order = [spec.key for spec in readiness.STAGES]
    expected = {
    ...
    for field, item in expected.items():
        if checked.get(field) != item:
            raise CorpusExecutorPipelineArmV12Error(f"pipeline arm v12 field mismatch: {field}")
    ...
    for key in ARMED_STAGES:
        if rows[key].get("enabled") is not True or rows[key].get("requires_fixed_outcomes") is not False:
            raise CorpusExecutorPipelineArmV12Error(f"{key}: armed corpus stage contract mismatch")
    for spec in readiness.STAGES[len(ARMED_STAGES):]:
        if rows[spec.key].get("enabled"):
            raise CorpusExecutorPipelineArmV12Error(f"{spec.key}: downstream stage was prematurely armed")
...
def main() -> int:
    ...
    _write(args.plan_out, armed); _write(args.receipt_out, receipt)
