#!/usr/bin/env python3
"""Run readiness-v7 historical refinement behind live pre/post Git alignment.

Version 5 binds the branch-guarded executor to the exact-source-partition-first
v7 stage contract.  The exact plan is validated and embedded in the receipt;
branch, remote, HEAD, and worktree alignment are checked on both sides by the
established preflight.  No G15 stage can run unless broad scope, exact cross-lane
overlap, and exact same-lane source partitioning all precede it.
"""
...
SCHEMA = "ng_historical_refinement_preflight.v5"
EXECUTOR_CONTRACT = "ng_historical_refinement_executor_v5"
READINESS_CONTRACT = readiness.SCHEMA
LEGACY_SCHEMA = legacy.SCHEMA


class HistoricalRefinementPreflightV5Error(RuntimeError):
    """Raised when branch-guarded execution is not bound to readiness v7."""
...
def _finalize(
    ...
    result["readiness_stage_contract_fingerprint"] = (
        STAGE_CONTRACT_FINGERPRINT
    )
    result["execution_plan_snapshot"] = copy.deepcopy(dict(plan))
    result["execution_plan_v7_validated"] = True
...
def execute_preflight(
    ...
    **kwargs: Any,
    ...
    """Execute one readiness-v7 stage with branch alignment before and after."""
    executor.validate_plan(plan)
...
def validate_receipt(receipt: Mapping[str, Any]) -> None:
    ...
    if value.get("schema") != SCHEMA or observed != legacy._fingerprint(value):
        raise HistoricalRefinementPreflightV5Error(
            "preflight v5 schema or fingerprint mismatch"
        )
    if value.get("executor_contract") != EXECUTOR_CONTRACT:
        raise HistoricalRefinementPreflightV5Error(
            "preflight v5 executor contract mismatch"
        )
    if value.get("readiness_contract") != READINESS_CONTRACT:
        raise HistoricalRefinementPreflightV5Error(
            "preflight v5 readiness contract mismatch"
        )
    if value.get("readiness_stage_contract") != STAGE_CONTRACT:
        raise HistoricalRefinementPreflightV5Error(
            "preflight v5 stage contract mismatch"
        )
    if (
        ...
        raise HistoricalRefinementPreflightV5Error(
            "preflight v5 stage contract fingerprint mismatch"
        )
    if value.get("execution_plan_v7_validated") is not True:
        raise HistoricalRefinementPreflightV5Error(
            "preflight v5 must record v7 plan validation"
        )
    for field, message in (
        ...
        (
            "broad_corpus_exact_partition_required",
            "broad corpus exact source partition",
        ),
        ("lock_first_g15_scoring_required", "G15 lock-first scoring"),
        (
            "counterfactual_g16_lineage_required",
            "G16 counterfactual lineage",
        ),
    ):
    ...
    if not isinstance(plan, Mapping):
        raise HistoricalRefinementPreflightV5Error(
            "preflight v5 requires the exact plan snapshot"
        )
    with _v7_context():
        executor.validate_plan(plan)
    if value.get("plan_fingerprint") != plan.get("fingerprint"):
        raise HistoricalRefinementPreflightV5Error(
            "receipt plan fingerprint does not match embedded v7 plan"
        )
    stage_keys = [
        row.get("key")
    ...
    if stage_keys != STAGE_ORDER:
        raise HistoricalRefinementPreflightV5Error(
            "embedded plan does not use readiness-v7 stage order"
        )
    try:
        ...
        raise HistoricalRefinementPreflightV5Error(
            "broad scope, exact overlap, exact partition, or G15 replay stage is missing"
        ) from error
    if not broad_index < overlap_index < partition_index < replay_index:
        raise HistoricalRefinementPreflightV5Error(
            "broad scope, exact overlap, and exact partition must precede G15 replay in order"
        )
    for key in (
        "broad_corpus_scope",
        ...
        row = next(
            row for row in plan["stages"] if row.get("key") == key
        )
    ...
    if isinstance(executor_result, Mapping):
        ...
        if (
            ...
            and selected_stage not in STAGE_ORDER
        ):
            raise HistoricalRefinementPreflightV5Error(
                "executor selected a stage outside readiness v7"
            )

    base = _base_receipt(receipt)
    if base.get("fingerprint") != value.get(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--expected-branch",
        default=branch_alignment.DEFAULT_BRANCH,
    )
    parser.add_argument(
        "--expected-repository",
        default=branch_alignment.DEFAULT_REPOSITORY,
    )
    parser.add_argument(
        "--remote",
        default=branch_alignment.DEFAULT_REMOTE,
    )
    parser.add_argument(
        "--allow-dirty-prefix",
        action="append",
    ...
    summary = {
        ...
        "executor_status": (
            receipt.get("executor_result") or {}
        ).get("status"),
        "executor_stage": (
            receipt.get("executor_result") or {}
        ).get("stage"),
    ...
    return (
        0
        if summary["executor_status"]
        in legacy.EXECUTOR_OK_STATUSES
        else 2
    )
