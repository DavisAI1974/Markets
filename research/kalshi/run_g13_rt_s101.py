...
BASIS (recorded, load-bearing): G13 runs on the OI-continuous NG.n.0 tape (data/nymex_cont_n0,
...
ROLL: detected from the per-record instrument_id at the UTC file boundary (996 -> 1008 at 20260225 per the S101 roll-check subagent (March-expiry-day rank swap; identities inferred)); offset measured as new-instrument first trade minus
old-instrument last trade across the seam. RT prices stay REAL; the scorer roll-adjusts.

Output: renders/ng_refine_s95/g13_rt.json (continuous_score.py schema) + g13_blind.png
...
ANCHOR = "20260213"
DAYS = ["20260215", "20260216", "20260217", "20260218", "20260219", "20260220",
        "20260222", "20260223", "20260224", "20260225", "20260226", "20260227"]
...
def main():
    ...
    guess = json.load(open(os.path.join(HERE, "forecasts", "grp13.json")))
    ...
    rt_path = os.path.join(OUT, "g13_rt.json")
    json.dump(out, open(rt_path, "w"), indent=1)
    print(f"[g13_rt] wrote {rt_path}: {len(recs)} days, rolls={rolls}")
    ...
    ax.set_title(f"NG G13 blind (one-shot, brain {guess.get('brain_version')}) vs actual - Sun 2026-02-15 .. Fri 2026-02-27  "
    ...
    plt.tight_layout()
    png = os.path.join(OUT, "g13_blind.png")
    plt.savefig(png, dpi=120, bbox_inches="tight")
    print(f"[g13_rt] wrote {png}")
