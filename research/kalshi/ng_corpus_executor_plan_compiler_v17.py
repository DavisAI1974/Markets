#!/usr/bin/env python3
"""Compile the stable corpus executor plan against latest-version-resolved readiness v22."""
...
import ng_corpus_executor_plan_compiler_v16 as v16
...
SCHEMA = "ng_corpus_executor_plan_compiler.v17"
STATUS = "LATEST_VERSION_RESOLVED_CORPUS_EXECUTOR_PLAN_COMPILED"
CONFIGURED_STAGES = ("corpus_s3_latest_version_resolution", *v16.CONFIGURED_STAGES)


class CorpusExecutorPlanCompilerV17Error(ValueError):
    pass
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CorpusExecutorPlanCompilerV17Error(f"cannot read JSON {path}: {error}") from error
    if not isinstance(value, dict):
        raise CorpusExecutorPlanCompilerV17Error(f"JSON artifact must be an object: {path}")
    return value
...
def _authority(value: Mapping[str, Any], *, label: str) -> None:
    for field in (
        ...
        if value.get(field) is not False:
            raise CorpusExecutorPlanCompilerV17Error(f"{label}: {field} must remain false")
    if value.get("one_signal_authority_preserved") is not True:
        raise CorpusExecutorPlanCompilerV17Error(f"{label}: one signal authority must be preserved")
    if value.get("blind_forecasts_immutable") is not True:
        raise CorpusExecutorPlanCompilerV17Error(f"{label}: blind forecasts must remain immutable")
    if value.get("cme_event_contracts_mode") != "SHADOW":
        raise CorpusExecutorPlanCompilerV17Error(f"{label}: CME event contracts must remain SHADOW")
    if value.get("brokerage_contract") != "tastytrade_not_ibkr":
        raise CorpusExecutorPlanCompilerV17Error(f"{label}: brokerage must remain tastytrade, not IBKR")
...
def validate_inputs(
    *,
    ...
    if resolved.get("status") != resolution.READY_STATUS or resolved.get("blockers") != []:
        raise CorpusExecutorPlanCompilerV17Error(
            "latest S3 versions must be resolved without blockers before compiling the guarded plan"
        )
    if resolved.get("source_spec") != dict(resolution_spec):
        raise CorpusExecutorPlanCompilerV17Error(
            "resolution receipt is not derived from the supplied unversioned corpus specification"
        )
    if resolved.get("source_spec_fingerprint") != resolution._fp(resolution_spec):
        raise CorpusExecutorPlanCompilerV17Error("resolution source-spec fingerprint mismatch")
    if resolved.get("capture_spec") != dict(capture_spec):
        raise CorpusExecutorPlanCompilerV17Error(
            "inventory capture specification is not the exact output of latest-version resolution"
        )
    if resolved.get("capture_spec_fingerprint") != resolution._fp(capture_spec):
        raise CorpusExecutorPlanCompilerV17Error("resolved capture-spec fingerprint mismatch")

    captured, attested, compiled, broad, bundle, target = v16.validate_inputs(
    ...
    if captured.get("source_spec_fingerprint") != resolved.get("capture_spec_fingerprint"):
        raise CorpusExecutorPlanCompilerV17Error(
            "inventory capture is not bound to the exact latest-version-resolved capture specification"
        )
...
def _validate_plan(
    ...
    if keys != expected or keys[: len(CONFIGURED_STAGES)] != list(CONFIGURED_STAGES):
        raise CorpusExecutorPlanCompilerV17Error(
            "execution plan is not the exact readiness-v22 latest-version-resolved contract"
        )
    if len(keys) != len(set(keys)):
        raise CorpusExecutorPlanCompilerV17Error("duplicate execution-plan stage keys")
    ...
    if not (
        ...
        raise CorpusExecutorPlanCompilerV17Error(
            "latest-version resolution is not the first corpus-provenance stage"
        )
    rows = {str(row.get("key")): row for row in rows_list}
    for index, key in enumerate(CONFIGURED_STAGES):
        row = rows[key]
        if row.get("argv") != commands[key]:
            raise CorpusExecutorPlanCompilerV17Error(f"{key}: command vector mismatch")
        expected_enabled = index == 0 if compiled else True
        if row.get("enabled") is not expected_enabled:
            raise CorpusExecutorPlanCompilerV17Error(f"{key}: enabled-state mismatch")
        if row.get("requires_fixed_outcomes") is not False:
            raise CorpusExecutorPlanCompilerV17Error(f"{key}: corpus stage must remain pre-outcome")
...
def build_compiled_plan(
    *,
    ...
    receipt: dict[str, Any] = {
        ...
        "s3_latest_version_resolution_receipt_fingerprint": resolved["receipt_fingerprint"],
        "resolution_source_spec_fingerprint": resolved["source_spec_fingerprint"],
        "captured_list_evidence_fingerprint": resolved["captured_list_evidence_fingerprint"],
        "latest_version_resolutions_fingerprint": resolved["resolutions_fingerprint"],
        ...
        "latest_version_resolution_required_before_inventory_capture": True,
        ...
        "next_permitted_stage": "RUN_BRANCH_GUARDED_LATEST_VERSION_RESOLUTION",
        "s3_latest_version_resolution_output": str(
            root / "ng_corpus_s3_latest_version_resolution_attestation.json"
        ),
    }
...
def validate_receipt(
    ...
    if checked.get("schema") != SCHEMA or observed != fingerprinting._fp(checked):
        raise CorpusExecutorPlanCompilerV17Error(
            "compiler v17 receipt schema or fingerprint mismatch"
        )
    checked["fingerprint"] = observed
    _authority(checked, label="compiler v17 receipt")
    ...
    for field, item in expected.items():
        if checked.get(field) != item:
            raise CorpusExecutorPlanCompilerV17Error(f"compiler v17 field mismatch: {field}")
    if rows["corpus_s3_latest_version_resolution"].get("enabled") is not True:
        raise CorpusExecutorPlanCompilerV17Error(
            "compiled plan must enable only latest-version resolution first"
        )
    if rows["corpus_s3_inventory_capture"].get("enabled") is not False:
        raise CorpusExecutorPlanCompilerV17Error(
            "inventory capture may not be enabled before exact latest-version resolution"
        )
