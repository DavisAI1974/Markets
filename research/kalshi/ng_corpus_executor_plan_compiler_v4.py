#!/usr/bin/env python3
"""Compile exact target-day and broad-corpus partition stages for readiness v7.

Readiness v6 configured byte inspection, daily basis regeneration, replay
catalog export, broad-scope verification, and exact cross-lane overlap.  V7 adds
an exact same-lane source-partition gate so duplicate bytes, reused objects, or
positive-duration same-lane overlap cannot reach G15 replay.  Only inspection is
enabled initially; the stable v4 arm step may enable all six pre-outcome corpus
stages without changing command vectors or ledger lineage.
"""
...
import ng_corpus_executor_plan_compiler_v3 as v3
import ng_historical_refinement_executor_v5 as executor

SCHEMA = "ng_corpus_executor_plan_compiler.v4"
CONFIGURED_STAGES = (
    "corpus_coverage",
...
STATUS = "BROAD_CORPUS_EXACT_PARTITION_EXECUTOR_PLAN_COMPILED"


class CorpusExecutorPlanCompilerV4Error(ValueError):
    """Raised when a readiness-v7 corpus plan cannot be compiled safely."""
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CorpusExecutorPlanCompilerV4Error(f"cannot read JSON {path}: {error}") from error
    if not isinstance(value, dict):
        raise CorpusExecutorPlanCompilerV4Error(f"JSON artifact must be an object: {path}")
    return value
...
def _authority(value: Mapping[str, Any], *, label: str) -> None:
    for field in (
        ...
        if value.get(field) is not False:
            raise CorpusExecutorPlanCompilerV4Error(f"{label}: {field} must remain false")
    if value.get("one_signal_authority_preserved") is not True:
        raise CorpusExecutorPlanCompilerV4Error(f"{label}: one signal authority must be preserved")
    if value.get("blind_forecasts_immutable") is not True:
        raise CorpusExecutorPlanCompilerV4Error(f"{label}: blind forecasts must remain immutable")
    if value.get("cme_event_contracts_mode") != "SHADOW":
        raise CorpusExecutorPlanCompilerV4Error(f"{label}: CME event contracts must remain SHADOW")
    if value.get("brokerage_contract") != "tastytrade_not_ibkr":
        raise CorpusExecutorPlanCompilerV4Error(f"{label}: brokerage must remain tastytrade, not IBKR")


def _commands(artifact_dir: Path, slice_path: Path, inspection_path: Path) -> dict[str, list[str]]:
    commands = v3._commands(artifact_dir, slice_path, inspection_path)
    resolved = artifact_dir.resolve(strict=False)
    commands["broad_corpus_exact_partition"] = [
        "python",
        "ng_broad_corpus_exact_partition_gate.py",
        "--exact-overlap-gate",
        str(resolved / "ng_broad_corpus_exact_overlap_gate.json"),
        "--out",
        str(resolved / "ng_broad_corpus_exact_partition_gate.json"),
    ]
    return commands
...
def build_compiled_plan(
    *,
    ...
    receipt: dict[str, Any] = {
        ...
        "broad_corpus_exact_partition_output": str(
            artifact_dir.resolve(strict=False) / "ng_broad_corpus_exact_partition_gate.json"
        ),
...
def validate_receipt(
    ...
    *,
    ...
    if checked.get("schema") != SCHEMA or observed != fingerprinting._fp(checked):
        raise CorpusExecutorPlanCompilerV4Error(
            "compiler v4 receipt schema or fingerprint mismatch"
        )
    checked["fingerprint"] = observed
    _authority(checked, label="compiler v4 receipt")
    executor.validate_plan(plan)
    if checked.get("status") != STATUS:
        raise CorpusExecutorPlanCompilerV4Error("compiler v4 status mismatch")
    if checked.get("execution_plan_fingerprint") != plan.get("fingerprint"):
        raise CorpusExecutorPlanCompilerV4Error("execution-plan fingerprint mismatch")
    if checked.get("commands_fingerprint") != fingerprinting._fp(commands):
        raise CorpusExecutorPlanCompilerV4Error("command fingerprint mismatch")
    if checked.get("configured_stages") != list(CONFIGURED_STAGES):
        raise CorpusExecutorPlanCompilerV4Error("configured stage order mismatch")
    for field in (
        ...
        if checked.get(field) is not True:
            raise CorpusExecutorPlanCompilerV4Error(
                f"compiler v4 mandatory field mismatch: {field}"
            )
    rows = {str(row.get("key")): row for row in plan.get("stages") or []}
    if list(rows) != [str(row.get("key")) for row in plan.get("stages") or []]:
        raise CorpusExecutorPlanCompilerV4Error("duplicate execution-plan stage keys")
    for key in CONFIGURED_STAGES:
        if rows.get(key, {}).get("argv") != list(commands[key]):
            raise CorpusExecutorPlanCompilerV4Error(f"{key}: compiled argv mismatch")
        if rows[key].get("requires_fixed_outcomes") is not False:
            raise CorpusExecutorPlanCompilerV4Error(
                f"{key}: corpus stages must remain pre-outcome"
            )
    if rows["corpus_coverage"].get("enabled") is not True:
        raise CorpusExecutorPlanCompilerV4Error(
            "corpus inspection must be enabled initially"
        )
    for key in CONFIGURED_STAGES[1:]:
        if rows[key].get("enabled"):
            raise CorpusExecutorPlanCompilerV4Error(
                f"{key}: downstream stage must remain disabled"
            )
    later = [
        row for row in plan.get("stages") or []
        if row.get("key") not in CONFIGURED_STAGES
    ]
    if any(row.get("enabled") for row in later):
        raise CorpusExecutorPlanCompilerV4Error(
            "G15/G16 stages must remain disabled"
        )
