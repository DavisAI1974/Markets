# NG Exhaustion Frankie Source File and Document Inventory

Date: 2026-08-24
Companion: `NG_EXHAUSTION_FRANKIE_DATA_FEED_INVENTORY_20260824.md`

This is the curated source inventory for constructing the corrected Frankie knowledge and causal-data planes. The bridge must build a content-addressed manifest from the repository at launch time, discover superseding files, and record full coverage; this document is a governing map, not a substitute for runtime hashing and authority checks.

## A. Corrected operational handoff and review

- `research/kalshi/NG_EXHAUSTION_FRANKIE_FULL_STACK_OCTOBER_NEXT_CHAT_HANDOFF_20260824.md`
- `research/kalshi/NG_EXHAUSTION_FRANKIE_GPT56_SOL_STACK_SUFFICIENCY_REVIEW_20260824.md`
- `research/kalshi/NG_EXHAUSTION_FRANKIE_DATA_FEED_INVENTORY_20260824.md`
- `research/kalshi/NG_EXHAUSTION_FRANKIE_SOURCE_FILE_INVENTORY_20260824.md`
- `research/kalshi/NG_EXHAUSTION_OCTOBER_SHARDED_HANDOFF_20260824.md` — older handoff, subject to corrected scope

## B. Current brain and S120-S135 runtime

- `research/kalshi/knowledge/ng_brain.json`
- `research/kalshi/frankie_packet_compact_s120.py`
- `research/kalshi/frankie_causal_capture_gate_s126.py`
- `research/kalshi/frankie_specialist_parity_s126.py`
- `research/kalshi/frankie_s114_separation_metadata_s126.py`
- `research/kalshi/frankie_m13_recover_s126.py`
- `research/kalshi/frankie_aws_stage_s126.py`
- `research/kalshi/frankie_s128_contract_repairs.py`
- `research/kalshi/frankie_s128_decision_state.py`
- `research/kalshi/frankie_s128_handoff.py`
- `research/kalshi/frankie_s132_dynamic_curve.py`
- `research/kalshi/frankie_s132_runtime.py`
- `research/kalshi/frankie_s133_reasoning_runtime.py`
- `research/kalshi/frankie_s135_current_runtime.py`
- `research/kalshi/frankie_s135_date_driver.py`
- `research/kalshi/frankie_s135_date_render.py`
- `research/kalshi/frankie_s135_date_session.py`
- `research/kalshi/frankie_s135_group_runner.py`
- `research/kalshi/frankie_s135_handoff.py`
- `research/kalshi/frankie_s135_preflight.py`
- `research/kalshi/frankie_s135_specialist_authority.py`

Tests are verification references, not Frankie knowledge sources:

- `research/kalshi/tests/test_frankie_s120_canary_boundary.py`
- `research/kalshi/tests/test_frankie_s120_packet_compact.py`
- `research/kalshi/tests/test_frankie_causal_capture_gate_s126.py`
- `research/kalshi/tests/test_frankie_specialist_parity_s126.py`
- `research/kalshi/tests/test_frankie_s128_contract_repairs.py`

## C. Frozen 54/55-week structural corpus

Primary freezes and protocols:

- `research/NG_EXHAUSTION_CHAIN_PHASE1_54W_BASE_FREEZE_20260817.json`
- `research/NG_EXHAUSTION_CHAIN_PHASE1_54W_EXECUTION_PROTOCOL_20260817.json`
- `research/NG_EXHAUSTION_CHAIN_PHASE1_CAUSAL_PROTOCOL_20260817.json`
- `research/NG_EXHAUSTION_CHAIN_PHASE1_DISCOVERY_PROTOCOL_20260817.json`
- `research/NG_EXHAUSTION_CHAIN_PHASE1_MECHANISM_ADDENDUM_20260817.json`
- `research/NG_EXHAUSTION_CHAIN_PHASE1_55W_RECONCILE_LAUNCH_20260817.json`
- `research/NG_EXHAUSTION_CHAIN_PHASE2_FINAL_FREEZE_20260818.md`
- `research/NG_EXHAUSTION_CHAIN_PHASE2_ALL_AGENT_FINDINGS_20260818.md`
- `research/NG_EXHAUSTION_CHAIN_PHASE2_FINDINGS_20260817.md`
- `research/NG_EXHAUSTION_CHAIN_PHASE2_TIMING_CONTEXT_FINDINGS_20260818.md`
- `research/NG_EXHAUSTION_CHAIN_PHASE2_MODULE_NOVELTY_FINDINGS_20260818.md`
- `research/NG_EXHAUSTION_CHAIN_PHASE2_POSTEXIT_RECURRENCE_FINDINGS_20260818.md`
- `research/NG_EXHAUSTION_CHAIN_PHASE2_PARALLEL_RECURRENCE_RECONCILIATION_20260818.md`
- `research/NG_EXHAUSTION_CHAIN_PHASE2_FINDINGS_ADDENDUM_POX_20260818.md`
- `research/NG_EXHAUSTION_CHAIN_PHASE2_POX_BRANCH_RECONCILIATION_V2_20260818.md`
- `research/NG_EXHAUSTION_CHAIN_PHASE2_POX_SAME_POSTEXIT_REEXPRESSION_20260818.md`
- `research/NG_EXHAUSTION_CHAIN_PHASE2_REAPPEARANCE_WATCH_MAP_20260818.json`
- `research/NG_EXHAUSTION_CHAIN_PHASE2_FINALIZATION_CHECKLIST_20260818.json`

Construction and helper implementations:

- `research/ng_exhaustion_chain_canonical_54w_merge_20260817.py`
- `research/ng_exhaustion_chain_canonical_54w_shard_20260817.py`
- `research/ng_exhaustion_chain_canonical_table_20260817.py`
- `research/ng_exhaustion_chain_phase1_causal_54w_20260817.py`
- `research/ng_exhaustion_chain_phase1_continuity_54w_20260817.py`
- `research/ng_exhaustion_chain_phase1_falsifier_54w_20260817.py`
- `research/ng_exhaustion_chain_phase1_lineage_54w_20260817.py`
- `research/ng_exhaustion_chain_phase1_structural_54w_20260817.py`
- `research/ng_exhaustion_chain_phase1_reconcile_55w_20260817.py`
- `research/ng_exhaustion_chain_phase2_characterize_20260817.py`
- `research/ng_exhaustion_chain_phase2_parallel_agents_20260818.py`
- `research/ng_exhaustion_chain_state_enrich_20260817.py`
- `research/ng_exhaustion_week_chain_state_roster_20260817.py`
- `research/ng_exhaustion_week_continuous_roster_20260817.py`

## D. Brain proposal and learned-structure interpretation

- `research/NG_EXHAUSTION_BRAIN_PROPOSAL_INDEX_20260818.md`
- `research/NG_EXHAUSTION_BRAIN_PROPOSAL_INDEX_ADDENDUM_20260820.md`
- `research/NG_EXHAUSTION_V4_BRAIN_TRADE_PROPOSAL_CLEAN_SOURCE_CURRENT_20260820.md`
- `research/NG_EXHAUSTION_V3_V4_BRAIN_TRADE_PROPOSAL_ADDENDUM_20260820.md`
- `research/NG_EXHAUSTION_V3_V4_BRAIN_TRADE_PROPOSAL_FINAL_ADDENDUM_20260820.md`
- `research/kalshi/knowledge/ng_brain_exhaustion_chain_phase2_proposal_20260818.json`
- `research/kalshi/knowledge/ng_brain_exhaustion_chain_birth_v2_proposal_20260819.json`
- `research/kalshi/knowledge/ng_brain_exhaustion_entry_timing_extension_20260818.json`
- `research/kalshi/knowledge/ng_brain_exhaustion_pox_focused_proposal_20260819.json`

The clean current proposal and correction records control when older addenda conflict.

## E. V4 governing contracts and corrections

- `research/NG_EXHAUSTION_V4_INTERPRETATION_CORRECTION_20260820.md`
- `research/NG_EXHAUSTION_V4_CONTINUOUS_ADAPTIVE_WALKFORWARD_CONTRACT_20260820.md`
- `research/NG_EXHAUSTION_D0_D5_V4_GEOMETRIC_SELF_ADAPTATION_CONTRACT_20260820.md`
- `research/NG_EXHAUSTION_D0_D5_V4_GEOMETRIC_SELF_ADAPTATION_CONTRACT_20260820.json`
- `research/NG_EXHAUSTION_EVENT_MARK_CLOCK_OPEN_BOUNDARY_20260819.md`
- `research/NG_EXHAUSTION_V4_CLEAN_SOURCE_PRELAUNCH_GATES_20260820.md`
- `research/NG_EXHAUSTION_CONTINUOUS_INSTANCE_TIMING_CORRECTION_20260820.md`
- `research/NG_EXHAUSTION_CONTINUOUS_LIVE_INFORMATION_CORRECTION_20260819.md`
- `research/NG_EXHAUSTION_FULL_CAUSAL_INFORMATION_CORRECTION_20260819.md`
- `research/NG_EXHAUSTION_PRIOR_MODEL_PRICE_CORRECTION_20260819.md`
- `research/NG_EXHAUSTION_POLARITY_NOT_PRIMARY_TARGET_CORRECTION_20260819.md`

## F. Post-correction V3 carryforward and exclusion controls

Admissible controlling records:

- `research/NG_EXHAUSTION_V3_NONAUTHORITATIVE_RESULTS_EXTRA_AGENT_V4_CARRYFORWARD_20260820.md`
- `research/NG_EXHAUSTION_V3_EXTRA_AGENT_INFORMATION_FINDINGS_20260820.md`
- `research/NG_EXHAUSTION_V3_EXTRA_AGENT_INFORMATION_FINDINGS_20260820.json`

These records must mechanically exclude ordinary V3 findings and the pre-correction D1 ExtraTrees/predictability results. Older V3 runs, launch receipts, generated estimates, timing grids, and model results are archive-only unless one of the controlling records above explicitly carries a structural fact into V4.

## G. Canonical raw MBO replay and V4 state adapter

- `research/ng_exhaustion_mbo_v4_full_state_replay_20260820.py`
- `research/ng_exhaustion_mbo_v4_state_adapter_20260820.py`
- `research/test_ng_exhaustion_mbo_v4_state_adapter_20260820.py`
- `research/test_ng_exhaustion_mbo_v4_legacy_control_semantics_20260820.py`
- `research/kalshi/NG_EXHAUSTION_MBO_5Y_CANONICAL_OBJECT_MANIFEST_20260822.json`
- `research/ng_exhaustion_mbo_5y_canonical_manifest_20260822.py`
- `research/kalshi/NG_MBO_5Y_S3_STATUS_20260820.json`
- `research/kalshi/NG_MBO_5Y_COMPACT_AUDIT_20260820.json`
- `research/kalshi/NG_DATABENTO_MARKETS_NG_LIVE_MBO_RECEIPT_20260820.json`

Raw DBN object locations and canonical predecessor selection must come from the authoritative manifest/runtime environment, not be inferred from target answers.

## H. V4 causal runtime modules

- `research/kalshi/ng_exhaustion_v4_causal_clock.py`
- `research/kalshi/ng_exhaustion_v4_causal_entry_adapter.py`
- `research/kalshi/ng_exhaustion_v4_state_assembler.py`
- `research/kalshi/ng_exhaustion_v4_mechanics.py`
- `research/kalshi/ng_exhaustion_v4_detector_intensity.py`
- `research/kalshi/ng_exhaustion_v4_detector_intensity_semantics.py`
- `research/kalshi/ng_exhaustion_v4_history_support.py`
- `research/kalshi/ng_exhaustion_v4_lock_outcome.py`
- `research/kalshi/ng_exhaustion_v4_adapter_integration.py`
- `research/kalshi/ng_exhaustion_v4_end_to_end_adapter.py`
- `research/kalshi/ng_exhaustion_v4_unified_runtime.py`
- `research/kalshi/ng_exhaustion_v4_exact_candidate_freeze.py`
- `research/kalshi/ng_exhaustion_v4_gate_verifier.py`
- `research/kalshi/ng_exhaustion_v4_pilot_chunk_guard.py`
- `research/kalshi/ng_exhaustion_v4_pilot_chunk_guardrail.py`

Corresponding focused tests under `research/kalshi/tests/test_ng_exhaustion_v4_*.py` are verification references. Do not rerun them broadly unless a launch error requires a focused check.

## I. V4 receipts and current state records

- `research/kalshi/NG_EXHAUSTION_V4_MECHANICAL_CONTRACT_RECEIPT_20260820.json`
- `research/kalshi/NG_EXHAUSTION_V4_STATE_ASSEMBLER_RECEIPT_20260820.json`
- `research/kalshi/NG_EXHAUSTION_V4_UNIFIED_RUNTIME_RECEIPT_20260820.json`
- `research/kalshi/NG_EXHAUSTION_V4_PROTECTED_BASELINE_20260821.json`
- `research/NG_EXHAUSTION_V4_CURRENT_STATE_DECISION_20260822.md`
- `research/kalshi/FRANKIE_V4_CURRENT_STATE_AUDIT_20260822.md`
- `research/kalshi/FRANKIE_V4_UNIFIED_FRAMEWORK_AUDIT_20260820.md`

## J. Provisional shadow components

- `research/kalshi/FRANKIE_P0_GAP_CLOSURE_PROVISIONAL_20260820.md`
- `research/kalshi/FRANKIE_P0_GAP_CLOSURE_PROVISIONAL_MANIFEST_20260820.json`
- `research/kalshi/NG_EXHAUSTION_V4_PROVISIONAL_READINESS_20260821.json`
- `research/kalshi/frankie_s137_cognitive_runtime.py`
- `research/kalshi/frankie_s137_cognitive_experiment_runner.py`
- `research/kalshi/frankie_temporal_graph_p0_adapter.py`
- `research/kalshi/frankie_hipporag_p0_retrieval.py`
- `research/kalshi/frankie_lats_p0_search.py`
- `research/kalshi/frankie_meta_loop_s138.py`
- `research/kalshi/frankie_meta_loop_coordinator_s138.py`

These are `PROVISIONAL_SHADOW`, not replacements for S135 or sources of primary-lock authority.

## K. October Step-1 sealed answer material

- `research/ng_exhaustion_step1_october_shards_20260824.py`
- `research/test_ng_exhaustion_step1_october_shards_20260824.py`
- `research/kalshi/NG_EXHAUSTION_OCTOBER_SHARDED_LAUNCH_20260824.json`
- `research/kalshi/NG_EXHAUSTION_MBO_5Y_STEP1_FINALIZER_LOCK_20260823.json`
- Step-1 seconds, populations, classifications, crosswalks, receipts, result prefixes, and reconciliation products located by the Step-1 manifests/runtime.

All material in this section is `SEALED_TARGET_ANSWER` for Frankie until primary outputs freeze. Step-1 code may be inspected by the integration owner only when necessary to maintain the wall; its target outputs must not enter Frankie's knowledge or causal prompts.

## L. Obsolete transport bridge and canary records

- `research/kalshi/ng_exhaustion_october_frankie_v4_bridge_20260824.py`
- `research/kalshi/tests/test_ng_exhaustion_october_frankie_v4_bridge_20260824.py`
- `.github/workflows/ng_exhaustion_october_frankie_blind_canary_20260824.yml`
- `research/kalshi/NG_EXHAUSTION_OCTOBER_FRANKIE_BLIND_CANARY_LAUNCH_20260824.json`
- `.github/workflows/ng_exhaustion_october_frankie_blind_canary_probe_20260824.yml`, if present in the chosen worktree
- `research/kalshi/NG_EXHAUSTION_OCTOBER_FRANKIE_BLIND_CANARY_PROBE_20260824.json`, if present in the chosen worktree

These are forensic/transport references. The old bridge is not the corrected scientific runner and must not be used as its foundation.

## M. Explicitly forbidden substitute

- `research/kalshi/frankie_bounded_3mo_parallel.py`

This consumes prebuilt SQS Frankie events. It is not the raw-MBO/V4 path. Do not create a `frankie-events` queue to make it appear usable.

## Runtime manifest requirement

Before launch, convert this curated map into a machine-readable, content-addressed manifest that records every served or denied source, its SHA, bytes, authority, supersession, target relationship, access policy, and complete chunk coverage. Any newly discovered or superseding file must be added under the correct authority rather than silently omitted.
