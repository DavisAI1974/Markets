#!/usr/bin/env python3
"""Arm the six corpus stages under one stable readiness-v8 executor plan.

Only the corpus prefix is armed.  Exact G15 replay, replay-window authorization,
refinement, scoring, publication, G16, and every execution-authority stage remain
disabled.  The guarded executor continues to run only the first blocking stage.
"""
...
SCHEMA = "ng_corpus_executor_pipeline_arm.v5"
ARMED_STAGES = compiler.CONFIGURED_STAGES
STATUS = "BROAD_CORPUS_EXACT_PARTITION_REPLAY_WINDOW_PIPELINE_ARMED"
WINDOW_STAGE = compiler.WINDOW_STAGE


class CorpusExecutorPipelineArmV5Error(ValueError):
    """Raised when the readiness-v8 corpus pipeline cannot be armed safely."""
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CorpusExecutorPipelineArmV5Error(f"cannot read JSON {path}: {error}") from error
    if not isinstance(value, dict):
        raise CorpusExecutorPipelineArmV5Error(f"JSON artifact must be an object: {path}")
    return value
...
def _authority(value: Mapping[str, Any], *, label: str) -> None:
    for field in (
        ...
        if value.get(field) is not False:
            raise CorpusExecutorPipelineArmV5Error(f"{label}: {field} must remain false")
    if value.get("one_signal_authority_preserved") is not True:
        raise CorpusExecutorPipelineArmV5Error(f"{label}: one signal authority must be preserved")
    if value.get("blind_forecasts_immutable") is not True:
        raise CorpusExecutorPipelineArmV5Error(f"{label}: blind forecasts must remain immutable")
    if value.get("cme_event_contracts_mode") != "SHADOW":
        raise CorpusExecutorPipelineArmV5Error(f"{label}: CME event contracts must remain SHADOW")
    if value.get("brokerage_contract") != "tastytrade_not_ibkr":
        raise CorpusExecutorPipelineArmV5Error(f"{label}: brokerage must remain tastytrade, not IBKR")
...
def _commands(plan: Mapping[str, Any]) -> dict[str, list[str]]:
    ...
    for key in ARMED_STAGES:
        ...
        if not isinstance(argv, list) or not argv or not all(isinstance(part, str) and part for part in argv):
            raise CorpusExecutorPipelineArmV5Error(f"{key}: compiled command is missing or invalid")
...
def _validate_compiled(
    ...
    if receipt.get("schema") != compiler.SCHEMA:
        raise CorpusExecutorPipelineArmV5Error("compiler receipt schema mismatch")
    if receipt.get("readiness_contract") != readiness.SCHEMA:
        raise CorpusExecutorPipelineArmV5Error("compiler did not bind readiness v8")
    if receipt.get("configured_stages") != list(ARMED_STAGES):
        raise CorpusExecutorPipelineArmV5Error("compiler stage order mismatch")
    if plan.get("outcome_paths") != []:
        raise CorpusExecutorPipelineArmV5Error("corpus plan must not reference outcomes")
...
def _arm_plan(plan: Mapping[str, Any], commands: Mapping[str, list[str]]) -> dict[str, Any]:
    ...
    for key in ARMED_STAGES:
        if rows[key].get("enabled") is not True:
            raise CorpusExecutorPipelineArmV5Error(f"{key}: stage was not armed")
        if rows[key].get("requires_fixed_outcomes") is not False:
            raise CorpusExecutorPipelineArmV5Error(f"{key}: stage must remain pre-outcome")
    for spec in readiness.STAGES[len(ARMED_STAGES):]:
        if rows[spec.key].get("enabled"):
            raise CorpusExecutorPipelineArmV5Error(f"{spec.key}: downstream stage must remain disabled")
    window = rows[WINDOW_STAGE]
    if window.get("enabled") is not False or window.get("requires_fixed_outcomes") is not False:
        raise CorpusExecutorPipelineArmV5Error("exact replay-window authorization must remain disabled and pre-outcome")
    return armed
...
def build_armed_plan(
    ...
    receipt: dict[str, Any] = {
        ...
        "single_stable_plan_for_target_broad_overlap_partition_and_replay_window_contract": True,
        ...
        "after_exact_g15_replay": "RUN_EXACT_REPLAY_WINDOW_AUTHORIZATION_BEFORE_REFINEMENT",
    }
...
def validate_arm_receipt(
    ...
    *,
    ...
    if checked.get("schema") != SCHEMA or observed != fingerprinting._fp(checked):
        raise CorpusExecutorPipelineArmV5Error("pipeline arm v5 schema or fingerprint mismatch")
    checked["fingerprint"] = observed
    _authority(checked, label="pipeline arm v5 receipt")
    source_plan, source_receipt, commands = _validate_compiled(compiled_plan, compiler_receipt)
    ...
    if fingerprinting._canonical(armed_plan) != fingerprinting._canonical(expected_plan):
        raise CorpusExecutorPipelineArmV5Error("armed plan differs from deterministic transformation")
    order = [spec.key for spec in readiness.STAGES]
    expected_fields = {
        "status": STATUS,
        "readiness_contract": readiness.SCHEMA,
    ...
    for field, expected in expected_fields.items():
        if checked.get(field) != expected:
            raise CorpusExecutorPipelineArmV5Error(f"pipeline arm v5 field mismatch: {field}")
