#!/usr/bin/env python3
"""Bind branch-guarded execution to source-native-identity readiness v30."""
...
SCHEMA = "ng_historical_refinement_preflight.v26"
EXECUTOR_CONTRACT = "ng_historical_refinement_executor_v26"
...
class HistoricalRefinementPreflightV26Error(RuntimeError):
    pass
...
def _check_plan(plan: Mapping[str, Any]) -> None:
    ...
    if keys != STAGE_ORDER:
        raise HistoricalRefinementPreflightV26Error(
            "embedded plan does not use readiness-v30 stage order"
        )
    required = (
    ...
    try:
        ...
        raise HistoricalRefinementPreflightV26Error(
            "required source-identity corpus stage is missing"
        ) from error
    if [positions[key] for key in required] != sorted(positions[key] for key in required):
        raise HistoricalRefinementPreflightV26Error(
            "source-native identity is not ordered before inspection and G15 replay"
        )
    ...
    if by_key["corpus_source_identity_attestation"].get("argv", [])[:3] != [
        "python",
        "ng_corpus_source_identity_attestation.py",
        "build",
    ]:
        raise HistoricalRefinementPreflightV26Error(
            "source-native identity command was substituted"
        )
    if not (
        positions["corpus_s3_materialization_provenance"]
        < positions["corpus_source_identity_attestation"]
        < positions["corpus_coverage"]
        < positions["corpus_definition_byte_binding"]
    ):
        raise HistoricalRefinementPreflightV26Error(
            "source-native identity must remain between provenance and byte inspection"
        )
    for key in required[:-1]:
        if by_key[key].get("requires_fixed_outcomes") is not False:
            raise HistoricalRefinementPreflightV26Error(f"{key} must remain pre-outcome")
...
def _base_receipt(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for field in (
        ...
        "execution_plan_v30_validated",
        "source_native_identity_required",
        "source_native_identity_pre_outcome",
        "source_identity_follows_recursive_materializer_provenance",
        "source_identity_precedes_broad_inspection",
        "source_identity_precedes_definition_binding",
        "manifest_only_identity_rejected_v30",
        "filename_and_s3_key_identity_inference_forbidden_v30",
        "dataset_schema_publisher_instrument_symbol_period_and_chronology_attested_v30",
    ):
        base.pop(field, None)
...
def _finalize(plan: Mapping[str, Any], base: Mapping[str, Any]) -> dict[str, Any]:
    ...
    result["execution_plan_v30_validated"] = True
    result["source_native_identity_required"] = True
    result["source_native_identity_pre_outcome"] = True
    result["source_identity_follows_recursive_materializer_provenance"] = True
    result["source_identity_precedes_broad_inspection"] = True
    result["source_identity_precedes_definition_binding"] = True
    result["manifest_only_identity_rejected_v30"] = True
    result["filename_and_s3_key_identity_inference_forbidden_v30"] = True
    result[
        "dataset_schema_publisher_instrument_symbol_period_and_chronology_attested_v30"
    ] = True
...
def validate_receipt(receipt: Mapping[str, Any]) -> None:
    ...
    if value.get("schema") != SCHEMA or observed != legacy._fingerprint(value):
        raise HistoricalRefinementPreflightV26Error(
            "preflight v26 schema or fingerprint mismatch"
        )
    if value.get("executor_contract") != EXECUTOR_CONTRACT:
        raise HistoricalRefinementPreflightV26Error("executor contract mismatch")
    if value.get("readiness_contract") != READINESS_CONTRACT:
        raise HistoricalRefinementPreflightV26Error("readiness contract mismatch")
    if value.get("readiness_stage_contract") != STAGE_CONTRACT:
        raise HistoricalRefinementPreflightV26Error("stage contract mismatch")
    if value.get("readiness_stage_contract_fingerprint") != STAGE_CONTRACT_FINGERPRINT:
        raise HistoricalRefinementPreflightV26Error("stage fingerprint mismatch")
    for field in (
        "execution_plan_v30_validated",
        "source_native_identity_required",
        "source_native_identity_pre_outcome",
    ...
    if not isinstance(plan, Mapping):
        raise HistoricalRefinementPreflightV26Error("exact plan snapshot is required")
    _check_plan(plan)
    if value.get("plan_fingerprint") != plan.get("fingerprint"):
        raise HistoricalRefinementPreflightV26Error("plan fingerprint mismatch")
