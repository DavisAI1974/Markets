#!/usr/bin/env python3
"""Run readiness-v8 historical refinement behind live pre/post Git alignment.

Version 6 binds the branch-guarded executor to the exact G15 replay-window
contract. Every plan and receipt must place pre-outcome replay-window
authorization after exact replay and before causal refinement.
"""
...
SCHEMA = "ng_historical_refinement_preflight.v6"
EXECUTOR_CONTRACT = "ng_historical_refinement_executor_v6"
READINESS_CONTRACT = readiness.SCHEMA
LEGACY_SCHEMA = legacy.SCHEMA


class HistoricalRefinementPreflightV6Error(RuntimeError):
    """Raised when branch-guarded execution is not bound to readiness v8."""
...
def _base_receipt(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for field in (
        ...
        "g15_refinement_blocked_until_replay_window_authorized",
...
def _finalize(plan: Mapping[str, Any], base: Mapping[str, Any]) -> dict[str, Any]:
    ...
    result["execution_plan_v8_validated"] = True
    ...
    result["g15_refinement_blocked_until_replay_window_authorized"] = True
...
def execute_preflight(
    ...
    **kwargs: Any,
    ...
    """Execute one readiness-v8 stage with branch alignment before and after."""
    executor.validate_plan(plan)
...
def validate_receipt(receipt: Mapping[str, Any]) -> None:
    ...
    if value.get("schema") != SCHEMA or observed != legacy._fingerprint(value):
        raise HistoricalRefinementPreflightV6Error("preflight v6 schema or fingerprint mismatch")
    if value.get("executor_contract") != EXECUTOR_CONTRACT:
        raise HistoricalRefinementPreflightV6Error("preflight v6 executor contract mismatch")
    if value.get("readiness_contract") != READINESS_CONTRACT:
        raise HistoricalRefinementPreflightV6Error("preflight v6 readiness contract mismatch")
    if value.get("readiness_stage_contract") != STAGE_CONTRACT:
        raise HistoricalRefinementPreflightV6Error("preflight v6 stage contract mismatch")
    if value.get("readiness_stage_contract_fingerprint") != STAGE_CONTRACT_FINGERPRINT:
        raise HistoricalRefinementPreflightV6Error("preflight v6 stage contract fingerprint mismatch")
    if value.get("execution_plan_v8_validated") is not True:
        raise HistoricalRefinementPreflightV6Error("preflight v6 must record v8 plan validation")
    for field, message in (
        ...
        ("g15_refinement_blocked_until_replay_window_authorized", "G15 refinement replay-window wall"),
        ...
        if value.get(field) is not True:
            raise HistoricalRefinementPreflightV6Error(f"{message} must remain mandatory")

    plan = value.get("execution_plan_snapshot")
    if not isinstance(plan, Mapping):
        raise HistoricalRefinementPreflightV6Error("preflight v6 requires the exact plan snapshot")
    with _v8_context():
        executor.validate_plan(plan)
    if value.get("plan_fingerprint") != plan.get("fingerprint"):
        raise HistoricalRefinementPreflightV6Error(
            "receipt plan fingerprint does not match embedded v8 plan"
        )
    stage_keys = [
        row.get("key")
    ...
    if stage_keys != STAGE_ORDER:
        raise HistoricalRefinementPreflightV6Error(
            "embedded plan does not use readiness-v8 stage order"
        )
    try:
        ...
        raise HistoricalRefinementPreflightV6Error(
            "required broad-corpus, G15 replay, replay-window, or refinement stage is missing"
        ) from error
    if not broad_index < overlap_index < partition_index < replay_index < window_index < refine_index:
        raise HistoricalRefinementPreflightV6Error(
            "broad verification, replay, replay-window authorization, and refinement are out of order"
        )
    ...
    for key in (
        ...
        if rows[key].get("requires_fixed_outcomes") is not False:
            raise HistoricalRefinementPreflightV6Error(f"{key} must remain pre-outcome")
    ...
    if isinstance(executor_result, Mapping):
        ...
        if selected_stage is not None and selected_stage not in STAGE_ORDER:
            raise HistoricalRefinementPreflightV6Error(
                "executor selected a stage outside readiness v8"
            )
