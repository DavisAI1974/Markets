#!/usr/bin/env python3
"""Bind branch-guarded execution to S3-captured historical readiness v21."""
...
import ng_historical_refinement_preflight_v16 as v16
import ng_historical_refinement_readiness_v21 as readiness

SCHEMA = "ng_historical_refinement_preflight.v17"
EXECUTOR_CONTRACT = "ng_historical_refinement_executor_v17"
...
class HistoricalRefinementPreflightV17Error(RuntimeError):
    pass


@contextmanager
def _v21_context() -> Iterator[None]:
    saved = (
        v16.executor,
        v16.readiness,
        v16.EXECUTOR_CONTRACT,
        v16.READINESS_CONTRACT,
        v16.STAGE_CONTRACT,
        v16.STAGE_ORDER,
        v16.STAGE_CONTRACT_FINGERPRINT,
    )
    v16.executor = executor
    v16.readiness = readiness
    v16.EXECUTOR_CONTRACT = EXECUTOR_CONTRACT
    v16.READINESS_CONTRACT = READINESS_CONTRACT
    v16.STAGE_CONTRACT = STAGE_CONTRACT
    v16.STAGE_ORDER = STAGE_ORDER
    v16.STAGE_CONTRACT_FINGERPRINT = STAGE_CONTRACT_FINGERPRINT
    try:
        yield
    finally:
        (
...
def _base_receipt(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    prior = base.pop("v16_preflight_fingerprint", None)
    for field in (
        "execution_plan_v21_validated",
        "s3_inventory_capture_required",
        "s3_inventory_capture_pre_outcome",
        "s3_inventory_capture_precedes_materialization",
        "complete_latest_object_set_required",
        "checksum_enabled_head_required",
        "source_identity_not_inferred_from_s3_keys",
        "materialization_bound_to_captured_spec",
    ):
        base.pop(field, None)
    base.pop("fingerprint", None)
    base["schema"] = v16.SCHEMA
...
def _check_plan(plan: Mapping[str, Any]) -> None:
    ...
    if keys != STAGE_ORDER:
        raise HistoricalRefinementPreflightV17Error(
            "embedded plan does not use readiness-v21 stage order"
        )
    required = (
    ...
    try:
        ...
        raise HistoricalRefinementPreflightV17Error(
            "required S3-captured corpus stage is missing"
        ) from error
    if not (
        ...
        raise HistoricalRefinementPreflightV17Error(
            "live S3 inventory capture is not ordered before materialization, inspection, and G15 replay"
        )
    by_key = {str(row.get("key")): row for row in rows}
    for key in required[:-1]:
        if by_key[key].get("requires_fixed_outcomes") is not False:
            raise HistoricalRefinementPreflightV17Error(f"{key} must remain pre-outcome")


def _finalize(plan: Mapping[str, Any], base: Mapping[str, Any]) -> dict[str, Any]:
    _check_plan(plan)
    with _v21_context():
        v16.validate_receipt(base)
    result = copy.deepcopy(dict(base))
    result["schema"] = SCHEMA
    result["v16_preflight_fingerprint"] = base["fingerprint"]
    ...
    result["execution_plan_v21_validated"] = True
    result["s3_inventory_capture_required"] = True
    result["s3_inventory_capture_pre_outcome"] = True
    result["s3_inventory_capture_precedes_materialization"] = True
    result["complete_latest_object_set_required"] = True
    result["checksum_enabled_head_required"] = True
    result["source_identity_not_inferred_from_s3_keys"] = True
    result["materialization_bound_to_captured_spec"] = True
...
def validate_receipt(receipt: Mapping[str, Any]) -> None:
    ...
    if value.get("schema") != SCHEMA or observed != legacy._fingerprint(value):
        raise HistoricalRefinementPreflightV17Error(
            "preflight v17 schema or fingerprint mismatch"
        )
    if value.get("executor_contract") != EXECUTOR_CONTRACT:
        raise HistoricalRefinementPreflightV17Error("executor contract mismatch")
    if value.get("readiness_contract") != READINESS_CONTRACT:
        raise HistoricalRefinementPreflightV17Error("readiness contract mismatch")
    if value.get("readiness_stage_contract") != STAGE_CONTRACT:
        raise HistoricalRefinementPreflightV17Error("stage contract mismatch")
    if value.get("readiness_stage_contract_fingerprint") != STAGE_CONTRACT_FINGERPRINT:
        raise HistoricalRefinementPreflightV17Error("stage fingerprint mismatch")
    for field in (
        "execution_plan_v21_validated",
        "s3_inventory_capture_required",
        "s3_inventory_capture_pre_outcome",
    ...
    if not isinstance(plan, Mapping):
        raise HistoricalRefinementPreflightV17Error("exact plan snapshot is required")
    _check_plan(plan)
    if value.get("plan_fingerprint") != plan.get("fingerprint"):
        raise HistoricalRefinementPreflightV17Error("plan fingerprint mismatch")
    base = _base_receipt(receipt)
    if base.get("fingerprint") != value.get("v16_preflight_fingerprint"):
        raise HistoricalRefinementPreflightV17Error(
            "embedded v16 preflight fingerprint mismatch"
        )
    with _v21_context():
        v16.validate_receipt(base)
