#!/usr/bin/env python3
"""Bind branch-guarded execution to recursive-materializer-provenance readiness v29."""
...
SCHEMA = "ng_historical_refinement_preflight.v25"
EXECUTOR_CONTRACT = "ng_historical_refinement_executor_v25"
...
class HistoricalRefinementPreflightV25Error(RuntimeError):
    pass
...
def _check_plan(plan: Mapping[str, Any]) -> None:
    ...
    if keys != STAGE_ORDER:
        raise HistoricalRefinementPreflightV25Error(
            "embedded plan does not use readiness-v29 stage order"
        )
    required = (
    ...
    try:
        ...
        raise HistoricalRefinementPreflightV25Error(
            "required recursive-provenance corpus stage is missing"
        ) from error
    if [positions[key] for key in required] != sorted(positions[key] for key in required):
        raise HistoricalRefinementPreflightV25Error(
            "recursive materializer provenance is not ordered before inspection and G15 replay"
        )
    ...
    if by_key["corpus_s3_materialization_provenance"].get("argv", [])[:3] != [
        "python",
        "ng_corpus_s3_materializer_provenance_gate.py",
        "build",
    ]:
        raise HistoricalRefinementPreflightV25Error(
            "recursive materializer provenance command was substituted"
        )
    for key in required[:-1]:
        if by_key[key].get("requires_fixed_outcomes") is not False:
            raise HistoricalRefinementPreflightV25Error(f"{key} must remain pre-outcome")
...
def _base_receipt(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for field in (
        ...
        "execution_plan_v29_validated",
        "recursive_materializer_provenance_required",
        "recursive_materializer_provenance_pre_outcome",
        "provenance_follows_exact_materialization",
        "provenance_precedes_broad_inspection",
        "runtime_capture_recursively_validated_v29",
        "exact_materializer_recursively_validated_v29",
        "complete_runtime_inventory_embedded_v29",
        "definition_binding_waits_for_recursive_provenance_v29",
    ):
        base.pop(field, None)
...
def _finalize(plan: Mapping[str, Any], base: Mapping[str, Any]) -> dict[str, Any]:
    ...
    result["execution_plan_v29_validated"] = True
    result["recursive_materializer_provenance_required"] = True
    result["recursive_materializer_provenance_pre_outcome"] = True
    result["provenance_follows_exact_materialization"] = True
    result["provenance_precedes_broad_inspection"] = True
    result["runtime_capture_recursively_validated_v29"] = True
    result["exact_materializer_recursively_validated_v29"] = True
    result["complete_runtime_inventory_embedded_v29"] = True
    result["definition_binding_waits_for_recursive_provenance_v29"] = True
...
def validate_receipt(receipt: Mapping[str, Any]) -> None:
    ...
    if value.get("schema") != SCHEMA or observed != legacy._fingerprint(value):
        raise HistoricalRefinementPreflightV25Error(
            "preflight v25 schema or fingerprint mismatch"
        )
    if value.get("executor_contract") != EXECUTOR_CONTRACT:
        raise HistoricalRefinementPreflightV25Error("executor contract mismatch")
    if value.get("readiness_contract") != READINESS_CONTRACT:
        raise HistoricalRefinementPreflightV25Error("readiness contract mismatch")
    if value.get("readiness_stage_contract") != STAGE_CONTRACT:
        raise HistoricalRefinementPreflightV25Error("stage contract mismatch")
    if value.get("readiness_stage_contract_fingerprint") != STAGE_CONTRACT_FINGERPRINT:
        raise HistoricalRefinementPreflightV25Error("stage fingerprint mismatch")
    for field in (
        "execution_plan_v29_validated",
        "recursive_materializer_provenance_required",
        "recursive_materializer_provenance_pre_outcome",
    ...
    if not isinstance(plan, Mapping):
        raise HistoricalRefinementPreflightV25Error("exact plan snapshot is required")
    _check_plan(plan)
    if value.get("plan_fingerprint") != plan.get("fingerprint"):
        raise HistoricalRefinementPreflightV25Error("plan fingerprint mismatch")
