#!/usr/bin/env python3
"""Arm only the outcome-blind v33 historical prefix from one durable plan.

The compiler configures the fixed-outcome publication, recursive publication binding,
and scored lesson commands, but this arm deliberately enables only the pre-outcome
prefix through six-factor attribution authorization. Fixed G15 outcomes require a
separate explicit executor boundary. Every G16 stage remains disabled.
"""
...
SCHEMA = "ng_corpus_executor_pipeline_arm.v28"
STATUS = "G15_V33_PRE_OUTCOME_PREFIX_ARMED"
ARMED_STAGES = compiler.PRE_OUTCOME_STAGES


class CorpusExecutorPipelineArmV28Error(ValueError):
    pass
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CorpusExecutorPipelineArmV28Error(f"cannot read JSON {path}: {error}") from error
    if not isinstance(value, dict):
        raise CorpusExecutorPipelineArmV28Error(f"JSON artifact must be an object: {path}")
    return value
...
def _authority(value: Mapping[str, Any], *, label: str) -> None:
    try:
        ...
        raise CorpusExecutorPipelineArmV28Error(str(error)) from error
...
def _commands(plan: Mapping[str, Any]) -> dict[str, list[str]]:
    ...
    for key in compiler.CONFIGURED_STAGES:
        ...
        if not isinstance(argv, list) or not argv or not all(
            ...
            raise CorpusExecutorPipelineArmV28Error(
...
def _validated_compiled(
    ...
    if source_plan.get("outcome_paths") != []:
        raise CorpusExecutorPipelineArmV28Error(
            "pre-outcome arm may not reference fixed outcome paths"
        )
...
def build_armed_plan(
    ...
    receipt: dict[str, Any] = {
        ...
        "six_factor_attribution_authorization_armed_pre_outcome": True,
        "g15_publication_command_configured_but_disabled": True,
        "g15_attribution_bound_publication_command_configured_but_disabled": True,
        "g15_counterfactual_lesson_command_configured_but_disabled": True,
        "fixed_outcomes_require_explicit_executor_authorization": True,
        "g16_outcomes_forbidden": True,
        "all_g16_stages_disabled": True,
...
def validate_arm_receipt(
    ...
    *,
    ...
    if checked.get("schema") != SCHEMA or observed != fingerprinting._fp(checked):
        raise CorpusExecutorPipelineArmV28Error(
            "pipeline arm v28 schema or fingerprint mismatch"
        )
    checked["fingerprint"] = observed
    _authority(checked, label="pipeline arm v28 receipt")
    source_plan, source_receipt, commands = _validated_compiled(
        compiled_plan, compiler_receipt
    )
    ...
    for field, expected_value in expected.items():
        if checked.get(field) != expected_value:
            raise CorpusExecutorPipelineArmV28Error(
                f"pipeline arm v28 field mismatch: {field}"
            )
    ...
    for key in (
        ...
        if rows[key].get("enabled") is not False or rows[key].get(
            "requires_fixed_outcomes"
        ) is not True:
            raise CorpusExecutorPipelineArmV28Error(
                f"{key}: fixed-outcome stage must remain configured and disabled"
            )
    for spec in readiness.STAGES[len(compiler.CONFIGURED_STAGES) :]:
        if rows[spec.key].get("enabled"):
            raise CorpusExecutorPipelineArmV28Error(
                f"{spec.key}: G16/downstream stage was prematurely armed"
            )
