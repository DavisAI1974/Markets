#!/usr/bin/env python3
"""Bind branch-guarded execution to latest-version-resolved historical readiness v22."""
...
import ng_historical_refinement_preflight_v17 as v17
import ng_historical_refinement_readiness_v22 as readiness

SCHEMA = "ng_historical_refinement_preflight.v18"
EXECUTOR_CONTRACT = "ng_historical_refinement_executor_v18"
...
class HistoricalRefinementPreflightV18Error(RuntimeError):
    pass


@contextmanager
def _v22_context() -> Iterator[None]:
    saved = (
        v17.executor,
        v17.readiness,
        v17.EXECUTOR_CONTRACT,
        v17.READINESS_CONTRACT,
        v17.STAGE_CONTRACT,
        v17.STAGE_ORDER,
        v17.STAGE_CONTRACT_FINGERPRINT,
    )
    v17.executor = executor
    v17.readiness = readiness
    v17.EXECUTOR_CONTRACT = EXECUTOR_CONTRACT
    v17.READINESS_CONTRACT = READINESS_CONTRACT
    v17.STAGE_CONTRACT = STAGE_CONTRACT
    v17.STAGE_ORDER = STAGE_ORDER
    v17.STAGE_CONTRACT_FINGERPRINT = STAGE_CONTRACT_FINGERPRINT
    try:
        yield
    finally:
        (
...
def _base_receipt(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    prior = base.pop("v17_preflight_fingerprint", None)
    for field in (
        "execution_plan_v22_validated",
        "s3_latest_version_resolution_required",
        "s3_latest_version_resolution_pre_outcome",
        "s3_latest_version_resolution_precedes_inventory_capture",
        "operator_guessed_version_ids_forbidden",
        "complete_latest_object_set_required_after_resolution",
        "resolved_capture_spec_bound_to_inventory_capture",
    ):
        base.pop(field, None)
    base.pop("fingerprint", None)
    base["schema"] = v17.SCHEMA
...
def _check_plan(plan: Mapping[str, Any]) -> None:
    ...
    if keys != STAGE_ORDER:
        raise HistoricalRefinementPreflightV18Error(
            "embedded plan does not use readiness-v22 stage order"
        )
    required = (
    ...
    try:
        ...
        raise HistoricalRefinementPreflightV18Error(
            "required latest-version-resolved corpus stage is missing"
        ) from error
    if not (
        ...
        raise HistoricalRefinementPreflightV18Error(
            "latest-version resolution is not ordered before inventory capture, materialization, and G15 replay"
        )
    by_key = {str(row.get("key")): row for row in rows}
    for key in required[:-1]:
        if by_key[key].get("requires_fixed_outcomes") is not False:
            raise HistoricalRefinementPreflightV18Error(f"{key} must remain pre-outcome")


def _finalize(plan: Mapping[str, Any], base: Mapping[str, Any]) -> dict[str, Any]:
    _check_plan(plan)
    with _v22_context():
        v17.validate_receipt(base)
    result = copy.deepcopy(dict(base))
    result["schema"] = SCHEMA
    result["v17_preflight_fingerprint"] = base["fingerprint"]
    ...
    result["execution_plan_v22_validated"] = True
    result["s3_latest_version_resolution_required"] = True
    result["s3_latest_version_resolution_pre_outcome"] = True
    result["s3_latest_version_resolution_precedes_inventory_capture"] = True
    result["operator_guessed_version_ids_forbidden"] = True
    result["complete_latest_object_set_required_after_resolution"] = True
    result["resolved_capture_spec_bound_to_inventory_capture"] = True
...
def validate_receipt(receipt: Mapping[str, Any]) -> None:
    ...
    if value.get("schema") != SCHEMA or observed != legacy._fingerprint(value):
        raise HistoricalRefinementPreflightV18Error(
            "preflight v18 schema or fingerprint mismatch"
        )
    if value.get("executor_contract") != EXECUTOR_CONTRACT:
        raise HistoricalRefinementPreflightV18Error("executor contract mismatch")
    if value.get("readiness_contract") != READINESS_CONTRACT:
        raise HistoricalRefinementPreflightV18Error("readiness contract mismatch")
    if value.get("readiness_stage_contract") != STAGE_CONTRACT:
        raise HistoricalRefinementPreflightV18Error("stage contract mismatch")
    if value.get("readiness_stage_contract_fingerprint") != STAGE_CONTRACT_FINGERPRINT:
        raise HistoricalRefinementPreflightV18Error("stage fingerprint mismatch")
    for field in (
        "execution_plan_v22_validated",
        "s3_latest_version_resolution_required",
        "s3_latest_version_resolution_pre_outcome",
    ...
    if not isinstance(plan, Mapping):
        raise HistoricalRefinementPreflightV18Error("exact plan snapshot is required")
    _check_plan(plan)
    if value.get("plan_fingerprint") != plan.get("fingerprint"):
        raise HistoricalRefinementPreflightV18Error("plan fingerprint mismatch")
    base = _base_receipt(receipt)
    if base.get("fingerprint") != value.get("v17_preflight_fingerprint"):
        raise HistoricalRefinementPreflightV18Error(
            "embedded v17 preflight fingerprint mismatch"
        )
    with _v22_context():
        v17.validate_receipt(base)
