#!/usr/bin/env python3
"""Compile an S3-attested, plan-bound corpus execution plan against readiness v20."""
...
import ng_corpus_executor_plan_compiler_v14 as v14
...
SCHEMA = "ng_corpus_executor_plan_compiler.v15"
STATUS = "S3_ATTESTED_PLAN_BOUND_CORPUS_EXECUTOR_PLAN_COMPILED"
CONFIGURED_STAGES = ("corpus_s3_materialization", *v14.CONFIGURED_STAGES)


class CorpusExecutorPlanCompilerV15Error(ValueError):
    pass
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CorpusExecutorPlanCompilerV15Error(f"cannot read JSON {path}: {error}") from error
    if not isinstance(value, dict):
        raise CorpusExecutorPlanCompilerV15Error(f"JSON artifact must be an object: {path}")
    return value
...
def _authority(value: Mapping[str, Any], *, label: str) -> None:
    for field in (
        ...
        if value.get(field) is not False:
            raise CorpusExecutorPlanCompilerV15Error(f"{label}: {field} must remain false")
    if value.get("one_signal_authority_preserved") is not True:
        raise CorpusExecutorPlanCompilerV15Error(f"{label}: one signal authority must be preserved")
    if value.get("blind_forecasts_immutable") is not True:
        raise CorpusExecutorPlanCompilerV15Error(f"{label}: blind forecasts must remain immutable")
    if value.get("cme_event_contracts_mode") != "SHADOW":
        raise CorpusExecutorPlanCompilerV15Error(f"{label}: CME event contracts must remain SHADOW")
    if value.get("brokerage_contract") != "tastytrade_not_ibkr":
        raise CorpusExecutorPlanCompilerV15Error(f"{label}: brokerage must remain tastytrade, not IBKR")
...
def validate_inputs(
    *,
    ...
    attested = materialization.validate_receipt(materialization_receipt)
    if attested.get("status") != materialization.READY_STATUS or attested.get("blockers") != []:
        raise CorpusExecutorPlanCompilerV15Error(
            "S3 materialization must be blocker-free before compiling the guarded plan"
        )
    compiled, broad, bundle, target = v14.validate_inputs(
        inventory_receipt=inventory_receipt,
        broad_plan=broad_plan,
        slice_bundle=slice_bundle,
        target_plan=target_plan,
    )
    if attested.get("inventory_compiler_receipt_fingerprint") != compiled.get("receipt_fingerprint"):
        raise CorpusExecutorPlanCompilerV15Error(
            "materialization and standalone inventory compiler receipt fingerprints differ"
        )
    if attested.get("plan_fingerprint") != broad.get("plan_fingerprint"):
        raise CorpusExecutorPlanCompilerV15Error(
            "materialization and broad inspection plan fingerprints differ"
        )
    if attested.get("compiled_plan") != broad:
        raise CorpusExecutorPlanCompilerV15Error(
            "broad inspection plan is not the exact S3-attested compiled plan"
        )
    return attested, compiled, broad, bundle, target
...
def _validate_plan(
    ...
    if keys != expected or keys[: len(CONFIGURED_STAGES)] != list(CONFIGURED_STAGES):
        raise CorpusExecutorPlanCompilerV15Error(
            "execution plan is not the exact readiness-v20 S3-attested contract"
        )
    if len(keys) != len(set(keys)):
        raise CorpusExecutorPlanCompilerV15Error("duplicate execution-plan stage keys")
    ...
    if not (
        ...
        raise CorpusExecutorPlanCompilerV15Error(
            "S3 materialization is not the first corpus-provenance stage"
        )
    rows = {str(row.get("key")): row for row in rows_list}
    for index, key in enumerate(CONFIGURED_STAGES):
        row = rows[key]
        if row.get("argv") != commands[key]:
            raise CorpusExecutorPlanCompilerV15Error(f"{key}: command vector mismatch")
        expected_enabled = index == 0 if compiled else True
        if row.get("enabled") is not expected_enabled:
            raise CorpusExecutorPlanCompilerV15Error(f"{key}: enabled-state mismatch")
        if row.get("requires_fixed_outcomes") is not False:
            raise CorpusExecutorPlanCompilerV15Error(f"{key}: corpus stage must remain pre-outcome")
...
def build_compiled_plan(
    *,
    ...
    attested, compiled, broad, bundle, target = validate_inputs(
    ...
    receipt: dict[str, Any] = {
        ...
        "enabled_stage": "corpus_s3_materialization",
        "commands_fingerprint": fingerprinting._fp(commands),
        "s3_materialization_required_before_inspection": True,
        "s3_materialization_stage_pre_outcome": True,
        "remote_object_version_and_checksum_required": True,
        "materialized_bytes_must_match_remote_and_definition": True,
        "standalone_inventory_receipt_emitted_by_same_stage": True,
        "broad_inspection_uses_s3_attested_plan": True,
        "definition_binding_uses_same_inventory_receipt": True,
        ...
        "next_permitted_stage": "RUN_BRANCH_GUARDED_S3_MATERIALIZATION_ATTESTATION",
        "s3_materialization_output": str(
            root / "ng_corpus_s3_materialization_attestation.json"
        ),
    }
...
def validate_receipt(
    ...
    if checked.get("schema") != SCHEMA or observed != fingerprinting._fp(checked):
        raise CorpusExecutorPlanCompilerV15Error(
            "compiler v15 receipt schema or fingerprint mismatch"
        )
    checked["fingerprint"] = observed
    _authority(checked, label="compiler v15 receipt")
    ...
    for field, item in expected.items():
        if checked.get(field) != item:
            raise CorpusExecutorPlanCompilerV15Error(
                f"compiler v15 field mismatch: {field}"
            )
    if rows["corpus_s3_materialization"].get("enabled") is not True:
        raise CorpusExecutorPlanCompilerV15Error(
            "compiled plan must enable only the S3 materialization stage first"
        )
    if rows["corpus_coverage"].get("enabled") is not False:
        raise CorpusExecutorPlanCompilerV15Error(
            "broad inspection may not be enabled before S3 materialization"
        )
