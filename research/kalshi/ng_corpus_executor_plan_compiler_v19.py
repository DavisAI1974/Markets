#!/usr/bin/env python3
"""Compile the stable corpus executor plan against paginated inventory readiness v24."""
...
import ng_corpus_executor_plan_compiler_v18 as v18
import ng_corpus_s3_paginated_inventory_capture as inventory
...
SCHEMA = "ng_corpus_executor_plan_compiler.v19"
STATUS = "PAGINATED_INVENTORY_CORPUS_EXECUTOR_PLAN_COMPILED"
CONFIGURED_STAGES = v18.CONFIGURED_STAGES


class CorpusExecutorPlanCompilerV19Error(ValueError):
    pass
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CorpusExecutorPlanCompilerV19Error(f"cannot read JSON {path}: {error}") from error
    if not isinstance(value, dict):
        raise CorpusExecutorPlanCompilerV19Error(f"JSON artifact must be an object: {path}")
    return value
...
def _authority(value: Mapping[str, Any], *, label: str) -> None:
    try:
        v18._authority(value, label=label)
    except Exception as error:
        raise CorpusExecutorPlanCompilerV19Error(str(error)) from error
...
def validate_inputs(
    *,
    ...
    if (
        ...
        raise CorpusExecutorPlanCompilerV19Error(
    ...
    if resolved.get("source_spec_fingerprint") != resolution._fp(resolution_spec):
        raise CorpusExecutorPlanCompilerV19Error("paginated resolution source-spec fingerprint mismatch")
    ...
    if resolved.get("capture_spec_fingerprint") != resolution._fp(capture_spec):
        raise CorpusExecutorPlanCompilerV19Error("paginated resolved capture-spec fingerprint mismatch")

    captured = inventory.validate_receipt(capture_receipt)
    if (
        captured.get("status") != inventory.READY_STATUS
        ...
        raise CorpusExecutorPlanCompilerV19Error(
            "paginated checksum-enabled inventory capture must be blocker-free before plan compilation"
        )
    if captured.get("source_spec") != dict(capture_spec):
        raise CorpusExecutorPlanCompilerV19Error(
            "paginated inventory receipt is not derived from the supplied capture specification"
        )
    if captured.get("source_spec_fingerprint") != resolved.get("capture_spec_fingerprint"):
        raise CorpusExecutorPlanCompilerV19Error(
            "paginated inventory capture is not bound to the resolved capture specification"
        )
    if captured.get("materialization_spec") != dict(materialization_spec):
        raise CorpusExecutorPlanCompilerV19Error(
            "materialization specification is not the exact paginated inventory output"
        )
    if captured.get("materialization_spec_fingerprint") != inventory._fp(materialization_spec):
        raise CorpusExecutorPlanCompilerV19Error(
            "paginated inventory materialization-spec fingerprint mismatch"
        )
    nested = captured.get("legacy_capture_receipt")
    if not isinstance(nested, Mapping):
        raise CorpusExecutorPlanCompilerV19Error(
            "paginated inventory receipt is missing its exact legacy capture proof"
        )

    legacy_resolved, legacy_captured, attested, compiled, broad, bundle, target = v18.validate_inputs(
        resolution_spec=resolution_spec,
        resolution_receipt=resolution_receipt,
        capture_spec=capture_spec,
        capture_receipt=nested,
    ...
    if legacy_resolved != resolved:
        raise CorpusExecutorPlanCompilerV19Error(
            "paginated resolution changed while validating nested inventory proof"
        )
    if legacy_captured != dict(nested):
        raise CorpusExecutorPlanCompilerV19Error(
            "nested exact inventory proof changed during validation"
        )
    if captured.get("legacy_capture_receipt_fingerprint") != nested.get("receipt_fingerprint"):
        raise CorpusExecutorPlanCompilerV19Error(
            "paginated inventory does not fingerprint-bind its nested exact capture proof"
        )
...
def _validate_plan(
    ...
    if keys != expected or keys[: len(CONFIGURED_STAGES)] != list(CONFIGURED_STAGES):
        raise CorpusExecutorPlanCompilerV19Error(
            "execution plan is not the exact readiness-v24 paginated inventory contract"
        )
    if len(keys) != len(set(keys)):
        raise CorpusExecutorPlanCompilerV19Error("duplicate execution-plan stage keys")
    ...
    if not (
        ...
        raise CorpusExecutorPlanCompilerV19Error(
            "paginated inventory capture is not ordered before materialization and G15 work"
        )
    rows = {str(row.get("key")): row for row in rows_list}
    if rows["corpus_s3_latest_version_resolution"].get("expected_output") != (
        "ng_corpus_s3_paginated_latest_version_resolution_attestation.json"
    ):
        raise CorpusExecutorPlanCompilerV19Error(
            "legacy non-paginated resolution artifact may not enter compiler v19"
        )
    inventory_row = rows["corpus_s3_inventory_capture"]
    if inventory_row.get("expected_output") != (
        "ng_corpus_s3_paginated_inventory_capture_attestation.json"
    ):
        raise CorpusExecutorPlanCompilerV19Error(
            "legacy single-page inventory artifact may not enter compiler v19"
        )
    if inventory_row.get("suggested_entrypoint") != [
        "python",
        "ng_corpus_s3_paginated_inventory_capture.py",
        "capture",
    ]:
        raise CorpusExecutorPlanCompilerV19Error(
            "paginated inventory suggested entrypoint was substituted"
        )
    for index, key in enumerate(CONFIGURED_STAGES):
        row = rows[key]
        if row.get("argv") != commands[key]:
            raise CorpusExecutorPlanCompilerV19Error(f"{key}: command vector mismatch")
        expected_enabled = index == 0 if compiled else True
        if row.get("enabled") is not expected_enabled:
            raise CorpusExecutorPlanCompilerV19Error(f"{key}: enabled-state mismatch")
        if row.get("requires_fixed_outcomes") is not False:
            raise CorpusExecutorPlanCompilerV19Error(f"{key}: corpus stage must remain pre-outcome")
...
def build_compiled_plan(
    *,
    ...
    resolved, captured, attested, compiled_inventory, broad, bundle, target = validate_inputs(
    ...
    receipt: dict[str, Any] = {
        ...
        "resolution_source_spec_fingerprint": resolved["source_spec_fingerprint"],
        ...
        "inventory_pages_fingerprint": captured["captured_pages_fingerprint"],
        "inventory_pagination_summaries_fingerprint": captured[
            "pagination_summaries_fingerprint"
        ],
        "inventory_heads_fingerprint": captured["captured_head_objects_fingerprint"],
        "combined_inventory_evidence_fingerprint": captured[
            "combined_inventory_evidence_fingerprint"
        ],
        "legacy_capture_receipt_fingerprint": captured[
        ...
        "next_permitted_stage": "RUN_BRANCH_GUARDED_PAGINATED_S3_CORPUS_PIPELINE",
        "s3_inventory_capture_output": str(
            root / "ng_corpus_s3_paginated_inventory_capture_attestation.json"
        ),
    }
...
def validate_receipt(
    ...
    if checked.get("schema") != SCHEMA or observed != fingerprinting._fp(checked):
        raise CorpusExecutorPlanCompilerV19Error(
            "compiler v19 receipt schema or fingerprint mismatch"
        )
    checked["fingerprint"] = observed
    _authority(checked, label="compiler v19 receipt")
    ...
    for field, item in expected.items():
        if checked.get(field) != item:
            raise CorpusExecutorPlanCompilerV19Error(
                f"compiler v19 field mismatch: {field}"
            )
    ...
    if rows["corpus_s3_inventory_capture"].get("enabled") is not False:
        raise CorpusExecutorPlanCompilerV19Error(
            "paginated inventory capture may not be enabled before resolution"
        )
