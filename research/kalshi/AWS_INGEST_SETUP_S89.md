# AWS ingest setup — full-raw MBP-10 year to a Lightsail/S3 bucket (S89)

Runbook to pull the continuous full-raw MBP-10 year (CL+NG, 2025-07..2026-07) straight to an AWS
(Lightsail or S3) bucket. The driver + writer are done and pushed to the trunk
`claude/kalshi-s79-kickoff-ij8t9o`. This is what to do in a fresh session or on the Lightsail box.
...
## Run it — option A: on the Lightsail INSTANCE (durable, keys stay local, RECOMMENDED)
```bash
# one-time on the box
...
# creds (aws configure if the CLI is set up, or just export)
export DATABENTO_API_KEY=db-...
export AWS_ACCESS_KEY_ID=...  AWS_SECRET_ACCESS_KEY=...  AWS_DEFAULT_REGION=us-east-1

# ALL 12 months -> the bucket (resumable; safe to re-run if interrupted)
nohup python research/kalshi/pull_year_mbp10.py \
    --start 2025-07 --end 2026-07 --dest s3://davisai-nymex/nymex \
...
- Verify a sample: `aws s3 cp s3://davisai-nymex/nymex/nymex_cont/CL_20260514.jsonl.gz - | zcat | head -1`
