#!/usr/bin/env python3
"""Arm the exact-materialization stable corpus prefix under readiness v28."""
...
SCHEMA = "ng_corpus_executor_pipeline_arm.v23"
STATUS = "EXACT_S3_MATERIALIZATION_BOUND_CORPUS_PIPELINE_ARMED"
ARMED_STAGES = compiler.CONFIGURED_STAGES


class CorpusExecutorPipelineArmV23Error(ValueError):
    pass
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CorpusExecutorPipelineArmV23Error(f"cannot read JSON {path}: {error}") from error
    if not isinstance(value, dict):
        raise CorpusExecutorPipelineArmV23Error(f"JSON artifact must be an object: {path}")
    return value
...
def _authority(value: Mapping[str, Any], *, label: str) -> None:
    try:
        ...
        raise CorpusExecutorPipelineArmV23Error(str(error)) from error
...
def _commands(plan: Mapping[str, Any]) -> dict[str, list[str]]:
    ...
    for key in ARMED_STAGES:
        ...
        if not isinstance(argv, list) or not argv or not all(
            ...
            raise CorpusExecutorPipelineArmV23Error(
...
def _validated_compiled(
    ...
    if source_plan.get("outcome_paths") != []:
        raise CorpusExecutorPipelineArmV23Error("corpus plan may not reference outcomes")
...
def build_armed_plan(
    ...
    receipt: dict[str, Any] = {
        ...
        "runtime_capture_bound_to_product_finalization_lags": True,
        "declared_inventory_observations_bound_to_runtime_interval": True,
        "complete_paginated_inventory_embedded_and_validated": True,
        "exact_version_materialization_armed_before_broad_inspection": True,
        ...
        "after_runtime_observed_s3_inventory_capture": (
            "DOWNLOAD_AND_ATTEST_EXACT_VERSIONED_S3_OBJECT_BYTES"
        ),
        "after_exact_s3_materialization": "RUN_BROAD_BYTE_LEVEL_CORPUS_INSPECTION",
...
def validate_arm_receipt(
    ...
    *,
    ...
    if checked.get("schema") != SCHEMA or observed != fingerprinting._fp(checked):
        raise CorpusExecutorPipelineArmV23Error(
            "pipeline arm v23 schema or fingerprint mismatch"
        )
    checked["fingerprint"] = observed
    _authority(checked, label="pipeline arm v23 receipt")
    source_plan, source_receipt, commands = _validated_compiled(
        compiled_plan, compiler_receipt
    )
    ...
    for field, item in expected.items():
        if checked.get(field) != item:
            raise CorpusExecutorPipelineArmV23Error(
                f"pipeline arm v23 field mismatch: {field}"
            )
