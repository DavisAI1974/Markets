#!/usr/bin/env python3
"""Arm the recursive-materializer-provenance corpus prefix under readiness v29."""
...
SCHEMA = "ng_corpus_executor_pipeline_arm.v24"
STATUS = "RECURSIVE_S3_MATERIALIZER_PROVENANCE_BOUND_CORPUS_PIPELINE_ARMED"
ARMED_STAGES = compiler.CONFIGURED_STAGES


class CorpusExecutorPipelineArmV24Error(ValueError):
    pass
...
def _load(path: Path) -> dict[str, Any]:
    try:
        ...
        raise CorpusExecutorPipelineArmV24Error(f"cannot read JSON {path}: {error}") from error
    if not isinstance(value, dict):
        raise CorpusExecutorPipelineArmV24Error(f"JSON artifact must be an object: {path}")
    return value
...
def _authority(value: Mapping[str, Any], *, label: str) -> None:
    try:
        ...
        raise CorpusExecutorPipelineArmV24Error(str(error)) from error
...
def _commands(plan: Mapping[str, Any]) -> dict[str, list[str]]:
    ...
    for key in ARMED_STAGES:
        ...
        if not isinstance(argv, list) or not argv or not all(
            ...
            raise CorpusExecutorPipelineArmV24Error(
...
def _validated_compiled(
    ...
    if source_plan.get("outcome_paths") != []:
        raise CorpusExecutorPipelineArmV24Error("corpus plan may not reference outcomes")
...
def build_armed_plan(
    ...
    receipt: dict[str, Any] = {
        ...
        "recursive_materializer_provenance_armed_before_broad_inspection": True,
        ...
        "definition_binding_waits_for_recursive_materializer_provenance": True,
        "all_corpus_stages_pre_outcome": True,
        "all_downstream_g15_g16_stages_disabled": True,
        "after_exact_s3_materialization": "BIND_COMPLETE_RUNTIME_INVENTORY_AND_MATERIALIZER_PROVENANCE",
        "after_recursive_materializer_provenance": "RUN_BROAD_BYTE_LEVEL_CORPUS_INSPECTION",
...
def validate_arm_receipt(
    ...
    *,
    ...
    if checked.get("schema") != SCHEMA or observed != fingerprinting._fp(checked):
        raise CorpusExecutorPipelineArmV24Error(
            "pipeline arm v24 schema or fingerprint mismatch"
        )
    checked["fingerprint"] = observed
    _authority(checked, label="pipeline arm v24 receipt")
    source_plan, source_receipt, commands = _validated_compiled(
        compiled_plan, compiler_receipt
    )
    ...
    for field, expected_value in expected.items():
        if checked.get(field) != expected_value:
            raise CorpusExecutorPipelineArmV24Error(
                f"pipeline arm v24 field mismatch: {field}"
            )
