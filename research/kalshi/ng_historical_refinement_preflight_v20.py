#!/usr/bin/env python3
"""Bind branch-guarded execution to paginated-inventory historical readiness v24."""
...
import ng_historical_refinement_preflight_v19 as v19
import ng_historical_refinement_readiness_v24 as readiness

SCHEMA = "ng_historical_refinement_preflight.v20"
EXECUTOR_CONTRACT = "ng_historical_refinement_executor_v20"
...
class HistoricalRefinementPreflightV20Error(RuntimeError):
    pass
...
def _v24_context() -> Iterator[None]:
    saved = (
        v19.executor,
        v19.readiness,
        v19.EXECUTOR_CONTRACT,
        v19.READINESS_CONTRACT,
        v19.STAGE_CONTRACT,
        v19.STAGE_ORDER,
        v19.STAGE_CONTRACT_FINGERPRINT,
    )
    v19.executor = executor
    v19.readiness = readiness
    v19.EXECUTOR_CONTRACT = EXECUTOR_CONTRACT
    v19.READINESS_CONTRACT = READINESS_CONTRACT
    v19.STAGE_CONTRACT = STAGE_CONTRACT
    v19.STAGE_ORDER = STAGE_ORDER
    v19.STAGE_CONTRACT_FINGERPRINT = STAGE_CONTRACT_FINGERPRINT
    try:
        yield
    finally:
        (
...
def _base_receipt(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    prior = base.pop("v19_preflight_fingerprint", None)
    for field in (
        "execution_plan_v24_validated",
        "inventory_service_pagination_required",
        "paginated_inventory_capture_pre_outcome",
        "paginated_inventory_capture_precedes_materialization",
        "inventory_page_request_response_evidence_bound",
        "checksum_enabled_head_inventory_bound",
        "legacy_single_page_inventory_capture_rejected",
        "materialization_bound_to_paginated_inventory_capture_v24",
    ):
        base.pop(field, None)
    base.pop("fingerprint", None)
    base["schema"] = v19.SCHEMA
...
def _check_plan(plan: Mapping[str, Any]) -> None:
    ...
    if keys != STAGE_ORDER:
        raise HistoricalRefinementPreflightV20Error(
            "embedded plan does not use readiness-v24 stage order"
        )
    required = (
    ...
    try:
        ...
        raise HistoricalRefinementPreflightV20Error(
            "required paginated-inventory corpus stage is missing"
        ) from error
    if not (
    ...
    if second.get("suggested_entrypoint") != [
        "python",
        "ng_corpus_s3_paginated_inventory_capture.py",
        "capture",
    ]:
        raise HistoricalRefinementPreflightV20Error(
            "paginated S3 inventory entrypoint was substituted"
        )
    for key in required[:-1]:
        if by_key[key].get("requires_fixed_outcomes") is not False:
            raise HistoricalRefinementPreflightV20Error(f"{key} must remain pre-outcome")


def _finalize(plan: Mapping[str, Any], base: Mapping[str, Any]) -> dict[str, Any]:
    _check_plan(plan)
    with _v24_context():
        v19.validate_receipt(base)
    result = copy.deepcopy(dict(base))
    result["schema"] = SCHEMA
    result["v19_preflight_fingerprint"] = base["fingerprint"]
    ...
    result["execution_plan_v24_validated"] = True
    result["inventory_service_pagination_required"] = True
    result["paginated_inventory_capture_pre_outcome"] = True
    result["paginated_inventory_capture_precedes_materialization"] = True
    result["inventory_page_request_response_evidence_bound"] = True
    result["checksum_enabled_head_inventory_bound"] = True
    result["legacy_single_page_inventory_capture_rejected"] = True
    result["materialization_bound_to_paginated_inventory_capture_v24"] = True
...
def validate_receipt(receipt: Mapping[str, Any]) -> None:
    ...
    if value.get("schema") != SCHEMA or observed != legacy._fingerprint(value):
        raise HistoricalRefinementPreflightV20Error(
            "preflight v20 schema or fingerprint mismatch"
        )
    if value.get("executor_contract") != EXECUTOR_CONTRACT:
        raise HistoricalRefinementPreflightV20Error("executor contract mismatch")
    if value.get("readiness_contract") != READINESS_CONTRACT:
        raise HistoricalRefinementPreflightV20Error("readiness contract mismatch")
    if value.get("readiness_stage_contract") != STAGE_CONTRACT:
        raise HistoricalRefinementPreflightV20Error("stage contract mismatch")
    if value.get("readiness_stage_contract_fingerprint") != STAGE_CONTRACT_FINGERPRINT:
        raise HistoricalRefinementPreflightV20Error("stage fingerprint mismatch")
    for field in (
        "execution_plan_v24_validated",
        "inventory_service_pagination_required",
        "paginated_inventory_capture_pre_outcome",
    ...
    if not isinstance(plan, Mapping):
        raise HistoricalRefinementPreflightV20Error("exact plan snapshot is required")
    _check_plan(plan)
    if value.get("plan_fingerprint") != plan.get("fingerprint"):
        raise HistoricalRefinementPreflightV20Error("plan fingerprint mismatch")
    base = _base_receipt(receipt)
    if base.get("fingerprint") != value.get("v19_preflight_fingerprint"):
        raise HistoricalRefinementPreflightV20Error(
            "embedded v19 preflight fingerprint mismatch"
        )
    with _v24_context():
        v19.validate_receipt(base)
