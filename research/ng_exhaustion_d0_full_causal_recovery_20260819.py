...
def run_model(model, events, lineage, raw_dir):
    ...
    return {
        "status": "NG_D0_FULL_CAUSAL_RECOVERY_MODEL_AGENT_COMPLETE",
        "date": DATE,
        "model": model,
        ...
        "first_chain_type_label_contract": "FROZEN_FIRST_DESCENDANT_P_O_S_X_PLUS_SAME_FLIP",
        "D1_complement_crosscheck": "AFTER_BOTH_DURABLE_RESULTS_EXIST_COMPARE_DIRECT_D0_WITH_ONE_MINUS_D1_CONTINUATION_ON_MATCHED_MODEL_VIEW_CHECKPOINT_ROWS",
        "root_information_deeper_crosscheck": "AFTER_D1_D3_RECOVERY_RECONCILES_COMPARE_FULL_ROOT_RETAINED_VS_ROOT_ABLATED_DEEPER_STAGE_MODELS_ON_IDENTICAL_ROWS",
        "results": results,
        "censored": censored,
...
def main():
    ...
    assert dict(sorted(exact.items())) == EXPECTED_EXACT, (
        dict(sorted(exact.items())), EXPECTED_EXACT
    )
