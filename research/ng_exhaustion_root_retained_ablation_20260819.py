...
def zero_root(c):
    ...
    if conf is None:
        raise RuntimeError(f"root confirmation unavailable for ablation case={c['id']}")
    dummy = {
        'event_id': root.get('event_id', 'ROOT_ABLATED'),
        'week_sunday': root['week_sunday'],
        'sequence_index': root['sequence_index'],
        # Keep confirmation solely for the identical causal checkpoint. Setting
        # t0 far in the future makes the feature engine emit a zero root block.
        't0_idx': 10**12,
        'polarity': 0,
        'family': None,
        ...
        'seed_state': None,
        'feature': {},
        'dynamic_endpoint': {'causal_confirmation_idx': int(conf)},
        'time_context': {},
    }
    z['preds'][0] = dummy
    return z
...
def validated_increment(delta):
    for b in ('validation', 'confirmation'):
        ...
        if q.get('n', 0) <= 0:
            return False
        if q.get('root_retained_minus_ablated_log_loss_improvement', 0) <= 0:
            return False
        if q.get('root_retained_minus_ablated_brier_improvement', 0) <= 0:
            return False
    h = delta.get('held', {})
    if h.get('n', 0) >= 20:
        if h.get('root_retained_minus_ablated_log_loss_improvement', 0) < 0:
            return False
        if h.get('root_retained_minus_ablated_brier_improvement', 0) < 0:
            return False
    return True
...
def main():
    ...
    out = {
        'status': 'NG_ROOT_RETAINED_VS_ABLATED_AGENT_COMPLETE',
        'date': DATE,
        'stage': int(a.stage),
        'model': a.model,
