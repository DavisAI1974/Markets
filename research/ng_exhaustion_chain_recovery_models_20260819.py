...
def metrics(y, p, nullp, labels, weeks):
    ...
    for w, idx in by.items():
        yy = y[idx]; pp = p[idx]; nn = nullp[idx]
        labs = labels
        gains[w] = multiclass_brier(yy, nn, labs) - multiclass_brier(yy, pp, labs)
...
def support_ok(target: str, stage: int, block: str, q: dict[str, Any]):
    ...
    floor = {1: {"validation": 200, "confirmation": 100}, 2: {"validation": 30, "confirmation": 15}, 3: {"validation": 10, "confirmation": 5}}[stage][block]
