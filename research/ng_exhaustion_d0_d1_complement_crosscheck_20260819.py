...
def probability_rows(model, param, trainset, testset, age, cache):
    ...
    if not len(yte): return None
    raw, cls=predict_probs(model,param,Xtr,ytr,Xte)
    if raw is None: return None
    labels=['0','1']
    p=align_probs(raw,cls,labels)[:,1]
...
def main():
    ...
    d1,censored=build_cases(events,lineage,1); d0=invert_cases(d1)
    assert sum(c['continuation'] for c in d0)==135823 and len(censored)==37
    cache=load_price_cache(d1,a.raw_dir)
    findings=[]
    for model in MODELS:
        fit1=split_cases(d1,('discovery_fit',)); tune1=split_cases(d1,('discovery_tune',)); train1=fit1+tune1
        fit0=split_cases(d0,('discovery_fit',)); tune0=split_cases(d0,('discovery_tune',)); train0=fit0+tune0
        for age in PRIOR_AGES:
            p1=tune_param(model,fit1,tune1,1,'PRIOR',age,cache,'FULL_CAUSAL','CONTINUATION')
            p0=tune_param(model,fit0,tune0,1,'PRIOR',age,cache,'FULL_CAUSAL','CONTINUATION')
            point={'model':model,'root_age_seconds_after_confirmation':age,'d1_param':p1,'d0_param':p0,'blocks':{}}
            if p1 is None or p0 is None:
                findings.append(point); continue
            for block in ('validation','confirmation','held'):
                r1=probability_rows(model,p1,train1,split_cases(d1,(block,)),age,cache)
                r0=probability_rows(model,p0,train0,split_cases(d0,(block,)),age,cache)
                if r1 is None or r0 is None:
                    point['blocks'][block]={'n':0}; continue
                assert r1['ids']==r0['ids']
                y1=np.asarray([int(x) for x in r1['y']]); y0=np.asarray([int(x) for x in r0['y']])
    ...
    out={
        'status':'NG_D0_D1_COMPLEMENT_CROSSCHECK_COMPLETE','date':DATE,
        ...
        'primary_information_view':'FULL_CAUSAL','root_clock':'ROOT_AGE_MATCHED_TO_D1_PRIOR_AGE',
        'cross_model_vote_used':False,'frozen_exact_depth_counts':dict(sorted(exact.items())),
        'executable_rows':len(d1),'censored_d0':len(censored),'findings':findings,
