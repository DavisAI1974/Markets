...
def model_stage(stage: int, model: str, events, lineage, raw_dir: str):
    ...
    for target in TARGETS:
        ...
        for age in PRIOR_AGES:
            ...
            point = {
                ...
                "views": views,
                "incremental_price_value": price_increment(views["FULL_CAUSAL"], views["NO_PRICE_CAUSAL"]),
            }
    ...
    return {
        "status": "NG_CHAIN_BIRTH_DEPTH_TYPE_MODEL_AGENT_V2_COMPLETE",
        "date": DATE,
        "stage": int(stage),
        "model": model,
...
def main():
    ...
    print(json.dumps({
        "status": result["status"], "stage": a.stage, "model": a.model,
        "earliest": {k: v.get("earliest") for k, v in result.get("results", {}).items()},
    }, indent=2))
