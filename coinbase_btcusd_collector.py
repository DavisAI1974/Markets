...
Minimal Coinbase BTC-USD WS collector. Same bin schema as
...
Run: python coinbase_btcusd_collector.py --duration 14400 --bins-path btc_coinbase_bins.json
"""
...
def _load_existing_bins(path: str) -> dict[float, dict]:
    ...
    try:
        ...
        print(f"[cb-btc] could not load existing bins: {e}", flush=True)
        return {}
...
async def collect(duration_s: float, save_path: str) -> dict[float, dict]:
    ...
    print(f"[cb-btc] starting {duration_s:.0f}s on {PRODUCT}"
        ...
        try:
            async with websockets.connect(WS_URI, ping_interval=20, close_timeout=5) as ws:
                ...
                if reconnect_count > 0:
                    print(f"[cb-btc] reconnected ({reconnect_count})", flush=True)

                while time.time() - t0 < duration_s:
                    try:
                        raw = await asyncio.wait_for(ws.recv(), timeout=30.0)
                    except asyncio.TimeoutError:
                        print(f"[cb-btc] recv timeout t={time.time()-t0:.0f}s", flush=True)
                        continue
                    ...
                    if now - last_save >= 30.0:
                        ...
                        last_save = now
                        print(f"[cb-btc] t={now-t0:.0f}s bins={len(bins)}", flush=True)

        except (websockets.exceptions.ConnectionClosed,
                websockets.exceptions.WebSocketException,
                ConnectionResetError, OSError) as e:
            reconnect_count += 1
            print(f"[cb-btc] WS issue ({type(e).__name__}: {e}); retry 5s (count={reconnect_count})",
                  flush=True)
            _save_bins(bins, save_path)
            try:
    ...
    _save_bins(bins, save_path)
    print(f"[cb-btc] done. bins={len(bins)} reconnects={reconnect_count}", flush=True)
    return bins
...
def main():
    ...
    p.add_argument("--bins-path", type=str, default="btc_coinbase_bins.json")
    args = p.parse_args()
