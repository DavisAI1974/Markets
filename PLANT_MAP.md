...
| `failure_localization.py` | the frozen interaction taxonomy (Scale AI arXiv:2607.28802): 9 components, 41 modes, the root-cause rule, our classified history, and TWO DECLARED LOCAL EXTENSIONS — the paper's `model-context` edge carries no harness-side mode, which is where 16 of our 28 failures live. `validate()` REFUSES any (edge, mode, fault_side) triple outside the table; it found 19 borrowed labels on its first run | the session |
...
| `path_contract.py` | THE CURVE IS THE PRODUCT — checks every emitted `path_p50_curve` against the contract: the 2-hourly clock from the 20:00 REOPEN through the close, first point 0, last == (day-move minus gap). ANNOUNCE in the blind, HARD in the refine (same reasoning as `due_gate`). Measured: the committed g22 blind violates it 10 of 10, every day starting at hour 08 — g21 was 8 of 10 correct, so g22 is a REGRESSION | the session |
