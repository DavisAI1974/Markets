...
> ALREADY FOLDED IN — do not re-upload/re-apply. The S20 delta below was merged into this
> master in-repo (commit on `claude/session-upload-handoff-cy7wQ`, 2026-06-03). Per the
> START-HERE workflow rule (lines 14-21): never re-upload the whole master and never re-paste
> an already-folded session note. Next session: read the handoff + kickoff, then fold only the
> NEW (S21+) delta and bump the header line.
