...
# reads the SSM credential parameter. The host role cannot read the staging prefix (run 35522665856:
# 403 on HeadObject), so the delivery workflow signs a one-hour GET URL with its own credentials and
# the host fetches it with Invoke-WebRequest; the URL is never printed here or in the workflow log.
#
# $Day, $RunRoot, $Source (the s3 URI, for the receipt), $Url (the presigned GET), $ExpectedSha256,
# $ExpectedBytes arrive from ssm_run_ps1.py --set; no path literal here.
$ErrorActionPreference = 'Stop'
foreach ($required in 'Day', 'RunRoot', 'Source', 'Url', 'ExpectedSha256', 'ExpectedBytes') {
...
if (Test-Path $target) {
    ...
    Write-Output ("downloading " + $Source + " over the presigned URL (not printed)")
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    $previous = $ProgressPreference; $ProgressPreference = 'SilentlyContinue'
    try { Invoke-WebRequest -Uri $Url -OutFile $incoming -UseBasicParsing } finally { $ProgressPreference = $previous }
    if (-not (Test-Path $incoming)) { throw 'download produced no file' }
