...
#
# Why these files and no others (probe runs 35510320789 / 35510506738 / 35510597019 + code read):
...
#   (initial_checkpoint_hash), host-ready-<instance>.c15.json (checkpoint_hash) and
#   host-context-cache.c15.json (prepared_context_cache.py 142); actual-critic-request.json must be
#   byte-identical to the re-prepared body or the runner refuses, so it is superseded with them.
#   sunday_execution._save refuses any of these on differing bytes ('retained Sunday execution
#   evidence changed'). Everything else is data-derived and stays: host-instance.c15.json (the
...
#   the classroom package, cycles.sqlite.
#
...
# to run at the commit the configuration names) and unless host-identity.c15.json is present and
# actually stale (stored boss_commit differs). Idempotent: nothing to move -> receipt says so.
...
# The stored boss_commit, read through the host's own unpack so the c15 shape is honoured.
$identityPath = Join-Path $runDirectory 'host-identity.c15.json'
$stored = ''
if (Test-Path $identityPath) {
    $env:PYTHONDONTWRITEBYTECODE = '1'; $env:PYTHONPATH = $ToolsRoot
    $code = "import json,sys; sys.path.insert(0, r'$ToolsRoot'); from research.kalshi.frankie_boss.c15_journal import unpack; print(unpack(json.loads(open(r'$identityPath','rb').read()))['configuration']['host_runtime']['boss_commit'])"
    $stored = (& $Python -c $code 2>&1 | Select-Object -Last 1).ToString().Trim()
    if ($stored -notmatch '^[0-9a-f]{40}$') { throw ("could not read the stored boss_commit: " + $stored) }
    if ($stored -eq $head) { throw 'refusing: host-identity.c15.json already matches the current commit; nothing is stale' }
} else { $stored = 'absent' }
Write-Output ("stored boss_commit: " + $stored + "  current: " + $head)

$cycle = Join-Path (Join-Path $runDirectory 'execution') ('cycle-' + $CycleIndex)
$candidates = @(
    'host-identity.c15.json', 'initialization.c15.json', 'training.sqlite', 'training-witnesses',
    ('execution/cycle-' + $CycleIndex + '/host-preparation.c15.json'),
    ('execution/cycle-' + $CycleIndex + '/host-context-cache.c15.json'),
    ('execution/cycle-' + $CycleIndex + '/actual-critic-request.json'))
if (Test-Path $cycle) {
    Get-ChildItem $cycle -Filter 'host-ready-*.c15.json' | ForEach-Object { $candidates += ('execution/cycle-' + $CycleIndex + '/' + $_.Name) }
}
