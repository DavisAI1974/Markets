...
echo "### tooling"; for t in python3 python3.13 python3.12 python3.11 python3.10 pip3 git aws node npm claude docker; do printf '%-10s ' "$t"; command -v "$t" >/dev/null 2>&1 && { "$t" --version 2>&1 | head -1; } || echo "(absent)"; done
echo "### python packages (system python3)"; python3 -c "import importlib;[print(n, getattr(importlib.import_module(n),'__version__','?')) for n in ('boto3','numpy','torch','zstandard','databento_dbn','cryptography','openai','anthropic') if importlib.util.find_spec(n)]" 2>/dev/null; python3 -c "import importlib.util as u;print('absent:',[n for n in ('boto3','numpy','torch','zstandard','databento_dbn','cryptography','openai','anthropic') if not u.find_spec(n)])" 2>/dev/null
