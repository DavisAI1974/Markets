...
status() {
  ...
  echo "### kernel: processes killed in the last 6 hours (out of memory)"; journalctl -k --since "6 hours ago" 2>/dev/null | grep -iE "out of memory|killed process|oom" | tail -n 8 || echo "(none or journalctl unavailable)"
  echo "### memory now"; free -g | head -2
  for d in "$ROOT"/work/ingest-*/; do [ -s "$d/journal.compact.sqlite" ] && { echo "### $d/journal.compact.sqlite seal"; "$PY" -c "import sqlite3,sys; db=sqlite3.connect('file:'+sys.argv[1]+'?mode=ro', uri=True); print(list(db.execute('SELECT format,count,head FROM seal')) or 'UNSEALED')" "$d/journal.compact.sqlite"; }; done
