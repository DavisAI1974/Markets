...
- `research/kalshi/frankie_boss/SUNDAY_SITE_MAP_20260922.md` - every code site where the first run's Sunday-slice identity is a literal or a gate, with what it guards, and the schedule/binding/seed/host shapes: the input to modules 2-4 (advisory sweep, chat 9).
- `research/kalshi/frankie_boss/CODEX_HANDOFF_20260922.md` - the handoff for Codex: every change since 2026-09-21 morning chat by chat, the Monday ingest running on the box, Greg's directive to build modules 2-4 with the ingest-dependent values blank (chat 9 close).
- `research/kalshi/frankie_boss/SHIP_REVIEW_20260922_CHAT9_INGEST.md` - the /ship decision on the ingest-speed commits (the incremental observation, the box standard in the writer): GO, required items fixed (chat 9).
- `research/kalshi/frankie_boss/SPEC-ingest-parallel-replay.md` - the next lever after 1.77 ms per record: bodies produced in parallel from book checkpoints, the parent keeping only the digest chain; not built, Greg's call.
...
- `deploy/aws/host/frankie_host_supersede_classroom_package.ps1` + `.github/workflows/frankie_host_supersede_classroom_package.yml` (2026-09-20): moves aside (never deletes) the four retained `host-dipole-classroom-*.c15.json` of one cycle with sha256 + receipt so the runner rebuilds the package from the current code; refuses when the cycle holds a classroom completion. Frames of pipeline run 35517121900 named the refusal (`run_actual_sunday_classroom.py:191 prime_cache` -> `sunday_execution.py:48 _save`).
- `deploy/aws/host/frankie_host_rebuild_prefix_batch.ps1` + `.github/workflows/frankie_host_rebuild_prefix_batch.yml` (2026-09-20): after the host's line-ending normalization, the retained two-cycle prefix batch pinned CRLF-era code hashes; the script compares the binding's pins to the checkout, stops when nothing is stale, otherwise moves the code-pinned batch files aside (prefix-00 never touched), re-runs the gold-standard builder unchanged, rewrites only the configuration's `prefix_manifest` witness (dated backup) and moves the configuration-bound `host-identity.c15.json` aside. One receipt.
- `deploy/aws/host/frankie_host_supersede_readiness.ps1` + `.github/workflows/frankie_host_supersede_readiness.yml` (2026-09-20): moves aside (never deletes) a cycle request's trigger directory, delivered readiness directory and `host-service.c15.json` with per-file sha256 + receipt so a readiness re-pinned to the host's current request can be delivered; refuses when the cycle holds a critic dispatch.
- `research/kalshi/frankie_boss/operations/active_run_supersede.py` + `.github/workflows/frankie_active_run_supersede.yml` (2026-09-20): inspect or close the retained Pod's S3 active-run claim when the claiming observer's world is dead (copy aside server-side, conditional `phase closed`, receipt; no Runpod call). Used once: run 35519639227.
- `deploy/aws/host/frankie_host_cycle_status.ps1` + `.github/workflows/frankie_host_cycle_status.yml` (2026-09-20): read-only cycle status that fits under the SSM output cap while a run is in progress (runner status lines, log tail, runner process, cycle-directory files with mtimes, run progress record). Use this, not the seven-section binding probe, to watch a running cycle.
- `deploy/aws/host/frankie_host_restore_mapping_index.ps1` + `.github/workflows/frankie_host_restore_mapping_index.yml` (2026-09-20): deliver the source mapping index (`mapping/index.jsonl`, 16,121,079 bytes, pinned by mapping.json) that the 20260915 package never carried: takes `frankie_boss_ledger_mapping.yml`'s rebuilt artifact, verifies it against the committed pin, stages it on the granite bucket, starts the host, tags KeepRunning=true for the run, and the host script re-verifies the pin off the host's own mapping.json before a same-directory rename (receipt `FRANKIE_MAPPING_INDEX_RESTORED_V1`; nothing deleted). Built after pipeline run 35520104563 stopped at frankie_source_mapping.py:242.
- `research/kalshi/frankie_boss/operations/declare_identity_supersede.py` + `deploy/aws/host/frankie_host_declare_identity_supersede.ps1` + `.github/workflows/frankie_host_declare_identity_supersede.yml` (2026-09-20): the operator's receipted declaration that one cycle's saved binding may be superseded after a host code advance. `feedback_cycle.CycleCoordinator._binding_supersede` accepts ONLY a changed `training_identities.code_hash` and, when the controller result is already retained, the spent `controller.arm_hash`; `_export_verified` accepts a moved retained-export `boss_commit`/`agent_commit` only against the OLD value the declaration names. The helper reads the old values from `cycles.sqlite` read-only, appends to `cycles.sqlite.identity-supersede.json` and writes a receipt; acceptances append to `cycles.sqlite.identity-supersede-accepted.json`. Run AFTER the advance. Tests: `tests/test_feedback_cycle.py`, `tests/test_declare_identity_supersede.py` (torch, `frankie_cycle_identity_ci.yml`), `tests/test_host_declare_identity_supersede.py` (text).
- `deploy/aws/host/frankie_host_binding_diff.ps1` + `.github/workflows/frankie_host_binding_diff.yml` (2026-09-20): read-only probe that rebuilds a cycle's binding from what is on disk (initialization identities, the request plan's controller/learning kwargs, the configuration's memory sha256) and prints only the differing key paths against the saved binding; found `controller.arm_hash` (run 35529695569). Python embedded, so no host advance is needed to run it. Text test `tests/test_host_binding_diff.py`.
- `.github/workflows/frankie_cycle_identity_ci.yml` (2026-09-20): torch runner for `test_feedback_cycle.py` + `test_declare_identity_supersede.py`, which the pipeline's fixed family list does not name.
